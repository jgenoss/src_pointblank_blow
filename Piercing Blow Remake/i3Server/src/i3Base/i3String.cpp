...
void i3String::MakePointFix( REAL64 Value, wchar_t *Result, INT32 UnderCount, bool FillZero)
{ 
	wchar_t conv[128];
    int i, j, FractCount;
    bool UnderFlag, fFindNull;

    _swprintf( conv, L"%f", Value);
...
void i3String::ParseMultiSelectFileName( char *FileName, i3StringList *List)
{
	char *p;
	char conv[MAX_PATH];
	int idx;
...
INT32 i3String::Compare( const wchar_t* str1, const wchar_t* str2)
{
#ifdef USE_C_LIBRARY_FUNCS
...
INT32 i3String::Length( const wchar_t * pStr )
{
	INT32 Count = 0;
...
// 앞에서 부터 찾는다.
// 없으면 -1 반환
INT32 i3String::ContainChar( const wchar_t * pStr, wchar_t ch )
{
	INT32 idx = 0;
...
INT32 i3String::ContainNoCase( const char * pszStr, const char * pszKey)
{
	INT32 Len;
	INT32 idxKey = 0, idxStr = 0, StartIdx = 0;
	INT32 C1 = 0, C2 = 0;
...
void i3String::Copy( wchar_t * pDest, const wchar_t * pSrc, INT32 DestLength)
{
	if ( DestLength <= 0 )
		return;
...
void i3String::Concat( wchar_t * pDest, const wchar_t * pSrc)
{
	while( *pDest)
	{
		pDest++;
...
int i3String::UCS2toUTF8( char *pDest, const WCHAR16 *pSrc, int nDestBufSize )
{
	...
	int count = (int)( i3String::Length( pSrc ) );
	WCHAR16 unicode;
...
void i3String::ResolvePathToken( const char * pszPath, i3StringList * pList)
{
	char * pTemp;
	char szBuf[MAX_PATH];
	INT32 i;
...
{
	i3StringList BaseToken, AbsToken;
	char * pszBase, *pszAbs;
	INT32 sidx, didx;
	...
	if( AbsToken.GetCount() <= 0)
		...
		// 반환한다.
		if( i3String::Compare( BaseToken.Items[0], AbsToken.Items[0]) != 0)
		{
			i3String::Copy( pszRelPath, pszAbsPath, nRelPathMax);
			return;
		}
...
I3_EXPORT_BASE INT32 i3String::Format( char * strDest, INT32 i32Length, const char * strFormat, ... )
{
	va_list marker;
...
I3_EXPORT_BASE INT32 i3String::Format( wchar_t * strDest, INT32 i32Length, const wchar_t * strFormat, ...)
{
	va_list marker;
	va_start( marker, strFormat);
