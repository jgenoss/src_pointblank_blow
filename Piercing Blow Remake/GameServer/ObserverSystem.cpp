...
bool ObserverSystem::OnObserverJoin(GameSession* pSession, int i32SlotIdx)
{
	...
	// Notify all players in room that an observer joined
	{
		...
		i3NetworkPacket packet(PROTOCOL_OBSERVER_JOIN_NTF);
		packet.WriteData(&slotByte, sizeof(uint8_t));
		packet.WriteData(&obsCount, sizeof(uint8_t));
	...
	if (m_pRoom->GetRoomState() >= ROOM_STATE_BATTLE)
	{
		...
		i3NetworkPacket scorePacket(PROTOCOL_OBSERVER_SCORE_UPDATE_NTF);
		scorePacket.WriteData(&redScore, sizeof(int32_t));
		scorePacket.WriteData(&blueScore, sizeof(int32_t));
		scorePacket.WriteData(&round, sizeof(int32_t));
	...
	for (int i = 0; i < MAX_OBSERVERS_PER_ROOM; i++)
	{
		if (m_Observers[i].bActive && m_Observers[i].i32SessionIdx == pSession->GetIndex())
		{
			...
			// Notify room
			{
				...
				uint8_t obsCount = (uint8_t)m_i32ObserverCount;
				i3NetworkPacket packet(PROTOCOL_OBSERVER_LEAVE_NTF);
				packet.WriteData(&slotByte, sizeof(uint8_t));
				packet.WriteData(&obsCount, sizeof(uint8_t));
				m_pRoom->SendToAll(&packet);
			}
...
bool ObserverSystem::OnViewModeChange(int i32SlotIdx, ObserverViewMode eMode)
{
	...
	// Send ACK back to observer
	{
		...
		i3NetworkPacket packet(PROTOCOL_OBSERVER_VIEW_MODE_ACK);
		packet.WriteData(&result, sizeof(int32_t));
		packet.WriteData(&modeVal, sizeof(uint8_t));
		packet.WriteData(&followSlot, sizeof(int8_t));
...
bool ObserverSystem::OnFollowPlayer(int i32SlotIdx, int i32TargetSlot)
{
	...
	// Send ACK
	{
		...
		i3NetworkPacket packet(PROTOCOL_OBSERVER_FOLLOW_PLAYER_ACK);
		packet.WriteData(&result, sizeof(int32_t));
		packet.WriteData(&targetByte, sizeof(int8_t));
		packet.WriteData(&modeVal, sizeof(uint8_t));
		SendToObserver(i32SlotIdx, &packet);
	}
...
bool ObserverSystem::OnToggleScoreboard(int i32SlotIdx, bool bShow)
{
	...
	// Send ACK with full scoreboard data if showing
	{
		...
		i3NetworkPacket packet(PROTOCOL_OBSERVER_SCOREBOARD_ACK);
		packet.WriteData(&result, sizeof(int32_t));
		packet.WriteData(&showByte, sizeof(uint8_t));
		...
		if (bShow && m_pRoom)
		{
			...
			for (int i = 0; i < SLOT_MAX_COUNT; i++)
			{
				...
				packet.WriteData(&kills, sizeof(int16_t));
				packet.WriteData(&deaths, sizeof(int16_t));
				packet.WriteData(&assists, sizeof(int16_t));
				packet.WriteData(&alive, sizeof(uint8_t));
			}
		}
		SendToObserver(i32SlotIdx, &packet);
	}
		...
		if (obs.eViewMode == OBSERVER_VIEW_FOLLOW || obs.eViewMode == OBSERVER_VIEW_FIRST_PERSON)
		{
			if (!m_pRoom->IsPlayerAlive(obs.i32FollowSlot))
			{
				...
				if (nextSlot >= 0)
				{
					...
					i3NetworkPacket pkt(PROTOCOL_OBSERVER_FOLLOW_PLAYER_ACK);
					pkt.WriteData(&res, sizeof(int32_t));
					pkt.WriteData(&tgt, sizeof(int8_t));
					pkt.WriteData(&mode, sizeof(uint8_t));
...
void ObserverSystem::SendPositionUpdate(DWORD dwNow)
{
	...
	if (slotCount > 0)
	{
		i3NetworkPacket packet(PROTOCOL_OBSERVER_POSITION_UPDATE_NTF);
		packet.WriteData(&slotCount, sizeof(uint8_t));
		for (int i = 0; i < SLOT_MAX_COUNT; i++)
		{
			if (!latestPos[i].bValid)
				continue;
			...
			packet.WriteData(&latestPos[i].ui8Team, 1);
			packet.WriteData(&alive, 1);
			packet.WriteData(&latestPos[i].fX, 4);
			packet.WriteData(&latestPos[i].fY, 4);
			packet.WriteData(&latestPos[i].fZ, 4);
			packet.WriteData(&latestPos[i].fYaw, 4);
		}
...
void ObserverSystem::SendKillFeedUpdate(DWORD dwNow)
{
	...
	while (m_i32KillFeedReadIdx != m_i32KillFeedWriteIdx)
	{
		...
		i3NetworkPacket packet(PROTOCOL_OBSERVER_KILLFEED_NTF);
		packet.WriteData(&killerSlot, sizeof(int8_t));
		packet.WriteData(&deadSlot, sizeof(int8_t));
		packet.WriteData(&entry.ui32WeaponId, sizeof(uint32_t));
		packet.WriteData(&entry.ui8HitPart, sizeof(uint8_t));
		packet.WriteData(&entry.ui8MultiKill, sizeof(uint8_t));
		SendToObservers(&packet);
...
void ObserverSystem::SendScoreUpdate()
{
	...
	i3NetworkPacket packet(PROTOCOL_OBSERVER_SCORE_UPDATE_NTF);
	packet.WriteData(&redScore, sizeof(int32_t));
	packet.WriteData(&blueScore, sizeof(int32_t));
	packet.WriteData(&round, sizeof(int32_t));
	SendToObservers(&packet);
}
...
void ObserverSystem::SendRoundEvent(DWORD dwNow)
{
	...
	while (m_i32RoundEventReadIdx != m_i32RoundEventWriteIdx)
	{
		...
		i3NetworkPacket packet(PROTOCOL_OBSERVER_ROUND_EVENT_NTF);
		packet.WriteData(&entry.ui8EventType, sizeof(uint8_t));
		packet.WriteData(&winner, sizeof(int8_t));
		packet.WriteData(&actor, sizeof(int8_t));
		packet.WriteData(&entry.ui8Extra, sizeof(uint8_t));
		SendToObservers(&packet);
