...
void ModuleControlAdmin::CmdAnnounce(AdminSessionInfo* pSession, const char* pszArgs)
{
	...
	if (g_pGameSessionManager)
	{
		// Use KNOCK warning protocol so client shows it as a server maintenance notice
		g_pGameSessionManager->BroadcastKnockWarning(pszArgs, (uint16_t)strlen(pszArgs));
...
void ModuleControlAdmin::CmdShutdown(AdminSessionInfo* pSession, const char* pszArgs)
{
	...
	// Broadcast shutdown notice via KNOCK warning, then KNOCK shutdown
	char notice[128];
	snprintf(notice, sizeof(notice), "[SERVER] Server shutting down in %d seconds...", i32Delay);
	if (g_pGameSessionManager)
	{
		g_pGameSessionManager->BroadcastKnockWarning(notice, (uint16_t)strlen(notice));
		g_pGameSessionManager->BroadcastKnockShutdown();
	}
