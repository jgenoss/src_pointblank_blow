...
static void UpdateObjectiveProgress(GameQuestObjective& obj,
	int kills, int deaths, int headshots, int assists, bool won,
...
void GameSession::OnGetAllQuestInfoReq(char* pData, INT32 i32Size)
{
	...
	packet.WriteData(&cardSetCount, 1);
	packet.WriteData(&setIdx, 1);
	packet.WriteData(&complete, 1);
	packet.WriteData(&eventCount, 1);
	packet.WriteData(&completed, 1);
	packet.WriteData(&claimed, 1);
	packet.WriteData(&periodCount, 1);
	packet.WriteData(&completed, 1);
	packet.WriteData(&claimed, 1);
	packet.WriteData(&selected, 1);
...
void GameSession::OnGetHonorQuestInfoReq(char* pData, INT32 i32Size)
{
	...
	i3NetworkPacket packet(PROTOCOL_GET_HONOT_QUEST_INFO_ACK);
	packet.WriteData(&result, sizeof(int32_t));
	packet.WriteData(&rankId, 4);
	packet.WriteData(&honorCount, 1);
	packet.WriteData(&completed, 1);
	packet.WriteData(&claimed, 1);
	SendPacketMessage(&packet);
...
void GameSession::OnPeriodQuestSettingReq(char* pData, INT32 i32Size)
{
	...
	i3NetworkPacket packet;
	offset += sizeof(uint16_t);
	i3NetworkPacket packet(PROTOCOL_PERIOD_QUEST_SETTING_ACK);
	packet.WriteData(&result, sizeof(int32_t));
	packet.WriteData(&questId, 4);
...
// ============================================================================
void GameSession::UpdateEventQuestProgress(int kills, int deaths, int headshots, int assists, bool won,
	uint32_t weaponClass, uint32_t stageId,
	int bombPlants, int bombDefuses, int multiKills,
	int consecutiveKills, int playTimeMinutes, int damageDealt)
{
	...
	for (int i = 0; i < m_EventQuestData.i32EventQuestCount && i < MAX_EVENT_QUEST_COUNT; i++)
	{
		...
		for (int o = 0; o < eq.ui8ObjectiveCount && o < MAX_QUEST_OBJECTIVES; o++)
		{
			UpdateObjectiveProgress(eq.objectives[o],
				kills, deaths, headshots, assists, won,
				weaponClass, stageId,
