...
CXTrapModule::CXTrapModule()
{
	...
	m_i32XTrapMapIdx	= 0;
	i3mem::FillZero( m_ui8XTrapMap, sizeof(m_ui8XTrapMap) );
}
...
void	CXTrapModule::OnRunning()
{
	...
	while( m_bIsRunning )
	{
		...
		i3String::Format( strTemp, MAX_PATH, "[GAMEGUARD]Check Dir:%s", m_strDirName );
		g_pModuleLogFile->Write_M_Log( MODULE_GAMEGUARD_THREAD_IDX, NULL, strTemp );
		...
		{
			g_pModuleLogFile->Write_M_Log( MODULE_GAMEGUARD_THREAD_IDX, NULL, "[GAMEGUARD]FindFirstChangeNotification Fail" );
		...
		{
			char strTemp[ MAX_PATH ];
			i3String::Format( strTemp, MAX_PATH, "[GAMEGUARD]File Open Success count:%d", m_i32XTrapMapIdx );
			g_pModuleLogFile->Write_M_Log( MODULE_GAMEGUARD_THREAD_IDX, NULL, strTemp );
		}
		else
		{
			g_pModuleLogFile->Write_M_Log( MODULE_GAMEGUARD_THREAD_IDX, NULL, "[GAMEGUARD]File Open Fail" );
		}
...
BOOL	CXTrapModule::_FileOpen()
{
	...
	{
		i3mem::Copy( m_ui8XTrapMap[i+1], m_ui8XTrapMap[i], sizeof(UINT8)*XTRAP_CS4_BUFSIZE_MAP );
	}
