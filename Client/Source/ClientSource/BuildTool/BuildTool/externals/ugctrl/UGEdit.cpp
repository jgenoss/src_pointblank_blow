...
void CUGEdit::OnEditChanged()
{
	const int col = m_ctrl->GetCurrentCol();
	const long row = m_ctrl->GetCurrentRow();

	m_ctrl->OnEditChanged(col, row, this);
}
...
HBRUSH CUGEdit::CtlColor(CDC* pDC, UINT nCtlColor)
{
