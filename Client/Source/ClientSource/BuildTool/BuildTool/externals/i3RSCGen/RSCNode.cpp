...
void	RSCNode::GatherAllNodeToPathMap(std::unordered_map<std::string, RSCNode*>& inout)
{
	// 일단 insert 실패는 무시한다고 가정하고 처리한다..
	//i3::fixed_string key = CalcKeyFixedString(m_Entry.m_strFilePath);
	//i3::to_upper(key);
	if (m_Entry.m_strFilePath.empty())
		return;

	Path fileRelPathWindow(m_Entry.m_strFilePath.c_str());	fileRelPathWindow.make_preferred();
	std::string strRelPathUpperWindow(fileRelPathWindow.string());	i3::to_upper(strRelPathUpperWindow);

	inout.insert(std::unordered_map<std::string, RSCNode*>::value_type(strRelPathUpperWindow, this));		// 리턴값은 무시한다..

	for( INT32 i = 0; i < GetChildCount(); ++i)
		GetChild( i)->GatherAllNodeToPathMap( inout);		
}

void	RSCNode::AddThisToPathMap(std::unordered_map<std::string, RSCNode*>& inout)
{
	//i3::fixed_string key = CalcKeyFixedString(m_Entry.m_strFilePath);
	if (m_Entry.m_strFilePath.empty())
		return;
