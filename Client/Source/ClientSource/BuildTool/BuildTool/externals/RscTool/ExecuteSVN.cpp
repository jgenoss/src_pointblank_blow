...
namespace
{
	...
	bool GetStatusText(const i3::rc_string& strCmd, i3::vector<i3::string>& outTextList)
	{
		i3::vector<BYTE> buff;

		LoadDataFromPipeSync(strCmd, buff);
		...
		while (!rng.empty())
		{
			...
			if (!line_rng.empty())
			{
				i3::string text_line(line_rng.begin(), line_rng.end());
				outTextList.push_back(text_line);
			}
		}
...
bool GetConflictStatusFiles(const i3::const_char_range& full_path, i3::vector<i3::rc_string>& outInfos)
{
	i3::vector<i3::string> statusTextList;
	
	i3::string strCmd;
	strCmd = i3::format_string("svn.exe status -q \"%s\"", full_path);	// 변경된 파일만 검색.

	if (GetStatusText(strCmd, statusTextList))
	{
		boost::char_separator<char> sep(" \t");

		using tok = boost::tokenizer < boost::char_separator<char> >;

		for (size_t i = 0; i < statusTextList.size(); i++)
		{
			std::string text(statusTextList[i].c_str());

			tok tok_text(text, sep);

			tok::const_iterator it = tok_text.begin();
			if (i3::generic_is_iequal("C", *it))	// 'C' 는 Conflicted 를 의미함.
			{
				outInfos.push_back(*(++it));
			}
		}
	}

	return (outInfos.size() > 0);
}

void Cleanup(const i3::rc_string& target_fullpath, const i3::rc_string& sqlite3_fullpath)
{
	// 우선 sqlite3.exe 를 실행할 대상 경로를 구한다. 대상 경로는 .svndb 파일이 있는 폴더.
	Path path_svndb(target_fullpath.c_str());
	
	if (path_svndb.has_extension())	// 파일일 경우 폴더 대상으로.
	{
		if (path_svndb.has_parent_path())
			path_svndb = path_svndb.parent_path();
	}

	bool findSvnDBPathOK = false;
	while (true)
	{
		if (fs::exists(path_svndb / ".svndb"))
		{
			findSvnDBPathOK = true;
			break;
		}
	...
	// sqlite3.exe 를 실행해서 svn 큐를 비운다.
	if (findSvnDBPathOK && fs::exists(sqlite3_fullpath.c_str()))
	{
		i3::rc_string strCmd(sqlite3_fullpath);
		strCmd += " ";
		strCmd += path_svndb.string().c_str();
		strCmd += "/.svn/wc.db \"select * from work_queue\"";
		::system(strCmd.c_str());

		strCmd = sqlite3_fullpath;
		strCmd += " ";
		strCmd += path_svndb.string().c_str();
		strCmd += "/.svn/wc.db \"delete from work_queue\"";
		::system(strCmd.c_str());
	}

	// svn cleanup 처리.
	tsvn::ExecuteSync(tsvn::CMD_CLEANUP, target_fullpath);
}
...
{
	svn::GetConflictStatusFiles(full_path, out_conflict_abspaths);

	// svn 충돌 파일들 처리.
	if (!out_conflict_abspaths.empty())
	{
		for (auto &abspath : out_conflict_abspaths)
		{
			if (fs::exists(abspath.c_str()))
				fs::remove(abspath.c_str());
		}

		tsvn::ExecuteSync(tsvn::CMD_UPDATE, i3::as_literal(full_path));

		i3::vector<i3::rc_string> conflictCheckList;
		svn::GetConflictStatusFiles(full_path, conflictCheckList);
		if (!conflictCheckList.empty())	// 클린업 처리가 안되어 업데이트가 안되었음. 클린업 처리 후 재업데이트.
		{
			svn::Cleanup(full_path, sqlite3_fullpath);
			tsvn::ExecuteSync(tsvn::CMD_UPDATE, i3::as_literal(full_path));
		}
	}
}
