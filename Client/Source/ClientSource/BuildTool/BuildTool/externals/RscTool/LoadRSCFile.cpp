...
bool LoadRscFilePreprocess(i3Stream* stream, const i3::string& base_path,
	const i3::rc_string& rel_path, INT32& outCount)
{
	...
	if (!static_cast<i3FileStream*>(stream)->Open(rel_path.c_str(), STREAM_READ | STREAM_SHAREREAD))
		return false;
...
bool CheckRSCFile(const i3::string& media_base_path, const i3::rc_string& rel_path)
{
	// RSC 파일이 존재하지 않으면 RSG 파일을 읽어서 RSC 파일을 만든다.
	Path i3RSCAbsPath(media_base_path.c_str());	i3RSCAbsPath /= rel_path.c_str();
	if (!fs::exists(i3RSCAbsPath))
	{
		Path i3RSGRelPath(rel_path.c_str());
		i3RSGRelPath.replace_extension(".RSG");

		i3RSCGen* i3RSG = RSCFileMgr::i()->FindOrLoadRSCGen(media_base_path, i3RSGRelPath.string().c_str());
		if (i3RSG == nullptr)
			return false;

		Path i3RSGAbsPath(media_base_path.c_str());	i3RSGAbsPath /= i3RSGRelPath;

		char szi3RSGAbsPath[MAX_PATH] = { 0, };
		strcpy(szi3RSGAbsPath, i3RSGAbsPath.string().c_str());

		i3RSG->ExportRSCBin(szi3RSGAbsPath);
	}
...
{
	if (!CheckRSCFile(media_base_path, rel_path))
		return false;
