...
bool	Read_i3AI(i3Stream* stream, i3::vector<OBJREF>& state_list)
{
	...
	stream->SetPosition(sizeof(data), STREAM_CURRENT);		// 관심사가 안되서 그냥 통째로 넘긴다..
...
namespace
{
	void MakeAnimPathNodeMap_Recursive(const i3::rc_string& filename, const i3Rsc_AnimPathNodePtr& node, i3Rsc_AnimPathNodeMap& inout)
	{
		...
		i3::string node_name_upper(node->m_name);
		i3::to_upper(node_name_upper);
		map_type::iterator it = m.find(node_name_upper);

		if (it == m.end())
		{
			i3::pair<map_type::iterator, bool> pr = m.insert(i3::make_pair(node_name_upper, vector_i3Rsc_AnimPathNode_Filename()));
			it = pr.first;
