...
i3GuiEditBox * CBattleGUI_BOSS_PreStart::_InitEditBox( const char * pszPath, INT32 maxLength, INT32 pt, INT32 alignX, INT32 alignY, const char * pszText)
{
	i3GuiEditBox * pCtrl;

	if((pszText != NULL) && (maxLength == -1))
	{
		maxLength = i3String::Length( pszText);
	}
	...
	// Static Text 초기화
	{
		for( i = 0; s_STATIC_TEXT_INIT[i].m_szName[0] != 0; i++)
		{
			...
			if( pInfo->m_idxStrInit != -1)
			{
				...
				if( strlen( pszText ) == 0 )
				{
					I3FATAL("탈출 미션에 들어가는 스트링[ %s ] 이 번역이 안 되어 있다.", s_STATIC_TEXT_INIT[i].m_szName );
				}
			}
			...
			case GAME_GUI_PRESTART_ROUND :
				if( this->isEnable() == FALSE)
				{
					...
					g_pFramework->GetEquipShapeManager()->SetBossStageShape(
						...
						g_pGameContext->getStageID() );

					{
						m_pST_Gauge->ResetProgressBar();
						m_pST_Gauge->Start( 5.0f, I3GUI_PROGRESSBAR_LEFT_TO_RIGHT);

						CGameCharaBase * pMyChara = g_pCharaManager->getCharaByNetIdx( g_pGameContext->getMySlotIdx());
						I3ASSERT( pMyChara != NULL);
						

						
						if(g_pGameContext->IsPlayingSlot(g_pGameContext->getMySlotIdx()) == false )
						{				
						}
						else
						{
