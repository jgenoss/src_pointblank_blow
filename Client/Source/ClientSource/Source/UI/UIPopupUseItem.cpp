...
I3_CLASS_INSTANCE( UIPopupUseItem, UIPopupBase);





UIPopupUseItem::UIPopupUseItem(void)
{
	m_ItemID = 0;
	m_AuthWareDBIndex = 0;
...
/*virtual*/ void UIPopupUseItem::OnCreate( i3GameNode * pParent)
{
...
BOOL UIPopupUseItem::OnLinkControl(i3BinList * plist, i3GameNode * pControl)
{
	...
	//	컬러 변경
	{
		...
		{
			sprintf(strName, "useItemChangeColorSelectColorBox%d", i + 1);
	...
	//	컬러 크로스헤어
	{
		...
		{
			sprintf(strName, "useItemChangeColorCrosshairSelectColorBox%d", i + 1);
...
void UIPopupUseItem::OnInitControl(void)
{
	//	아이템사용 팝업
	{
...
void UIPopupUseItem::ProcessGameEvent( INT32 event, INT32 arg)
{
	...
	case EVENT_CLAN_PERSON_PLUS:	// 클랜원 증가 캐시 아이템 (CASHITEM_GROUP_CLAN_PERSON_PLUS_50) 사용 결과
		{
			...
			{				
				...
				if( event == EVENT_ITEM_AUTH && 
					m_ItemID == MAKE_ITEM_FLAG(ITEM_CLASS_EXPENDABLE, CASHITEM_USAGE_EXPENDABLES, CASHITEM_GROUP_CLAN_PERSON_PLUS_50, CASHITEM_DAY_NO))
					break;

				_Open(USE_ITEM_COMPLETE);

				if( m_Style == USE_ITEM_STYLE_BUYING)		// 상점의 경우, 셀렉션 업데이트를 한번 해준다..
				{
					UIPhaseShop* phase = static_cast<UIPhaseShop*>(GetMainFrame()->GetSubPhase(USP_SHOP));
...
void UIPopupUseItem::OnGuiNotify(I3GUI_CONTROL_NOTIFY* pNotify)
{
	...
	case GCI_USE_ITEM_B_CONFIRM_OK:
		...
		{
		//	OnExitStart();										// _FocusPopup처리를 거치지 않으므로 무효...(11.07.07.수빈)
			GetMainFrame()->ClosePopup(UPW_USEITEM);			
		}
		break;
...
bool UIPopupUseItem::OnEntranceStart(void * pArg1, void * pArg2)
{
	m_ItemID = 0;
	m_AuthWareDBIndex = 0;
	...
	{
		if( UIPopupBase::OnEntranceStart( pArg1, pArg2) == false)		//	enable
			return false;
...
bool UIPopupUseItem::OnExitStart()
{
...
void UIPopupUseItem::OnCancel(void)
{
...
void UIPopupUseItem::OnUpdate(RT_REAL32 rDeltaSeconds)
{
	UIPopupBase::OnUpdate( rDeltaSeconds);
}

void UIPopupUseItem::OnPreGuiNotify(void)
{

}

void UIPopupUseItem::OnExitKey(void)
{
	GetMainFrame()->ClosePopup( UPW_USEITEM);
...
bool UIPopupUseItem::_Open( USE_ITEM_POPUP_TYPE type)
{
	// 사용여부
	UINT8 Type = 0;
	UINT32 Arg = 0;
...
void UIPopupUseItem::_SetInputLock(BOOL bValue)
{
	m_pRoot->setInputDisable( bValue);
	i3GuiRoot::setGuiDisableInput( bValue);

	GetMainFrame()->GetTopMenu()->SetInputDisable( bValue?true:false);
}

void UIPopupUseItem::_InitQueryPopup(void)
{
	...
	INT32 itemUsage = GET_ITEM_FLAG_USAGE(m_ItemID);
	...
	//	속성 :
	if ( GET_ITEM_FLAG_CLASS(m_ItemID) != ITEM_CLASS_POINT )
	{
		char Attr[MAX_STRING_COUNT] = INVALID_TEXT;
		switch( itemUsage)
		{
		case CASHITEM_USAGE_MAINTENANCE:	i3String::Format(Attr, MAX_STRING_COUNT, GAME_STRING("STR_POPUP_USABLE_TERM"), itemNum);/*%d일권*/	break;
		case CASHITEM_USAGE_EXPENDABLES:	i3String::Copy(Attr, GAME_STRING("STR_POPUP_DISPOSABLE"));/*1회용*/	break;
		}
...
void UIPopupUseItem::_InitCompletePopup(void)
{
	...
	case RESULT_METHOD_CLAN_NAME		:
		{
			...
			pszResultValue = g_pFramework->GetClanContext()->GetClanNameText();
		}
		break;
	case RESULT_METHOD_CLAN_PERSON_PLUS	:
		{
			...
			sprintf( temp, "%d", pClanInfo->_maxPerson);
...
void UIPopupUseItem::_QueryOK(void)
{
	// 개수제 아이템일 경우엔 바로 사용한다.
	if ( CASHITEM_USAGE_EXPENDABLES == GET_ITEM_FLAG_USAGE(m_ItemID) )
	{
		_UseItem( m_ItemID);
		return;
	}
	...
	switch( GET_ITEM_FLAG_TYPE( m_ItemID))
	{
...
void UIPopupUseItem::CbQueryOk(void * pThis, INT32 nParam)
{
	if( nParam == MBR_OK)
	{
		((UIPopupUseItem*) pThis)->OnUseItem();
	}
	else
	{
		((UIPopupUseItem*) pThis)->OnCancel();
	}
}

void UIPopupUseItem::OnUseItem(void)
{
	_UseItem( m_ItemID);
}

BOOL UIPopupUseItem::_CheckItem(INT32 nItemFlag)
{
	switch( GET_ITEM_FLAG_USAGE(nItemFlag))
	{
		//	지속성
	case CASHITEM_USAGE_MAINTENANCE:
		{
			...
			case CASHITEM_GROUP_MEGA_HP10:
				{
					INT32 useItem = MAKE_ITEM_FLAG(ITEM_CLASS_MAINTENANCE, CASHITEM_USAGE_MAINTENANCE, CASHITEM_GROUP_MEGA_HP5, CASHITEM_DAY_NO);
			...
			case CASHITEM_GROUP_MEGA_HP5:
				{
					INT32 useItem = MAKE_ITEM_FLAG(ITEM_CLASS_MAINTENANCE, CASHITEM_USAGE_MAINTENANCE, CASHITEM_GROUP_MEGA_HP10, CASHITEM_DAY_NO);
			...
			case CASHITEM_GROUP_HOLLOW_POINT_AMMO_PLUS:
				{
					INT32 checkItemID[] =
					{
						MAKE_ITEM_FLAG(ITEM_CLASS_MAINTENANCE, CASHITEM_USAGE_MAINTENANCE, CASHITEM_GROUP_FULLMETALJACKETAMMO, CASHITEM_DAY_NO),
							MAKE_ITEM_FLAG(ITEM_CLASS_MAINTENANCE, CASHITEM_USAGE_MAINTENANCE, CASHITEM_GROUP_HOLLOW_POINT_AMMO, CASHITEM_DAY_NO),
							MAKE_ITEM_FLAG(ITEM_CLASS_MAINTENANCE, CASHITEM_USAGE_MAINTENANCE, CASHITEM_GROUP_JACKETED_HELLOW_POINT_AMMO, CASHITEM_DAY_NO),
							MAKE_ITEM_FLAG(ITEM_CLASS_MAINTENANCE, CASHITEM_USAGE_MAINTENANCE, CASHITEM_GROUP_HOLLOW_POINT_AMMO_PLUS, CASHITEM_DAY_NO),
					};
					...
					{
						// 체크하지 않습니다.
						if ( GET_ITEM_FLAG_TYPE(checkItemID[i]) == GET_ITEM_FLAG_TYPE( nItemFlag) )
							continue;
			...
			case CASHITEM_GROUP_BULLET_PROOF_VEST_METAL:
				{
					INT32 checkItemID[] =
					{
						MAKE_ITEM_FLAG(ITEM_CLASS_MAINTENANCE, CASHITEM_USAGE_MAINTENANCE, CASHITEM_GROUP_BULLET_PROOF_VEST, CASHITEM_DAY_NO),
							MAKE_ITEM_FLAG(ITEM_CLASS_MAINTENANCE, CASHITEM_USAGE_MAINTENANCE, CASHITEM_GROUP_BULLET_PROOF_VEST_PLUS, CASHITEM_DAY_NO),
							MAKE_ITEM_FLAG(ITEM_CLASS_MAINTENANCE, CASHITEM_USAGE_MAINTENANCE, CASHITEM_GROUP_BULLET_PROOF_VEST_GM, CASHITEM_DAY_NO),
							MAKE_ITEM_FLAG(ITEM_CLASS_MAINTENANCE, CASHITEM_USAGE_MAINTENANCE, CASHITEM_GROUP_BULLET_PROOF_VEST_METAL, CASHITEM_DAY_NO),
					};
					const INT32 checkItemCount = GET_ARRAY_COUNT(checkItemID);
...
void UIPopupUseItem::_UseItem(INT32 ItemID)
{
	...
	case USE_METHOD_CHANGE_NICK			:
		{
			...
			INT32 itemDisguiseNick = MAKE_ITEM_FLAG(ITEM_CLASS_MAINTENANCE, CASHITEM_USAGE_MAINTENANCE, CASHITEM_GROUP_DISGUISE_NICK, CASHITEM_DAY_NO);
	...
	case USE_METHOD_CHANGE_CLAN_NAME	:
		{
			...
			pFrame->TogglePopup(UPW_CLAN_NAMECHANGE,&ItemID);
		}
		break;
	case USE_METHOD_CHANGE_CLAN_MARK	:
		{
			...
			UIMainFrame *pFrame = g_pFramework->GetUIMainframe();
			//pFrame->TogglePopup(UPW_USEITEM);
			pFrame->TogglePopup(UPW_CLAN_MARKCHANGE_OLD, &m_AuthWareDBIndex, &nOldMark);
...
void UIPopupUseItem::_UseItemOk(void)
{
	switch( m_Type)
	{
	case USE_ITEM_DISGUISE_NICK:
...
BOOL UIPopupUseItem::_SetCurrentUseItemName(i3GuiEditBox * pControl)
{
...
void UIPopupUseItem::_SetEnablePopup(USE_ITEM_POPUP_TYPE type)
{
	switch( type)
	{
	case USE_ITEM_QUERY:
...
void UIPopupUseItem::_InitChangeNickPopup(void)
{
	// 아이템
...
void UIPopupUseItem::_ChangeNickOk(void)
{
	_SetInputLock(TRUE);
...
BOOL UIPopupUseItem::OnChangeNameOk( void)
{
	//	현재 닉네임과 클랜명만 가능
	if( m_Type == USE_ITEM_CHANGE_NICK || m_Type == USE_ITEM_CHANGE_CLAN_NAME || m_Type == USE_ITEM_DISGUISE_NICK)
	{
		...
		else
		{
			...
			i3String::Copy( szInputName, m_pChangeNameUserEdit->getIMETextBuf());
...
BOOL UIPopupUseItem::OnChangeNameDuplicate(void)
{
	BOOL ret = TRUE;
	...
	i3String::Copy( m_szChangeName, m_pChangeNameUserEdit->getIMETextBuf());

	if( i3String::Length(m_szChangeName) <= 0)
	{
		switch( m_Type)
		{
		case USE_ITEM_CHANGE_NICK:			i3String::Copy(szError, GAME_STRING("STR_POPUP_MESSAGE_PLEASE_NAME"));/*"닉네임을 적어주세요."*/	break;
		case USE_ITEM_CHANGE_CLAN_NAME:		i3String::Copy(szError, GAME_STRING("STR_POPUP_MESSAGE_INPUT_CLAN_NAME"));/*클랜명을 적어주세요.*/	break;
		case USE_ITEM_DISGUISE_NICK:		i3String::Copy(szError, GAME_STRING("STR_POPUP_MESSAGE_INPUT_DISGUISED_NICK_1"));/*임시 닉네임을 적어주세요.*/	break;
		}
	...
	if( m_Type == USE_ITEM_CHANGE_NICK || m_Type == USE_ITEM_DISGUISE_NICK)
	{
		if (FALSE == CStringFilter::IsValidNickname(m_szChangeName))
		{
			...
			i3String::Copy(szError, GAME_STRING("STBL_IDX_EP_LOBBY_NICK_LENGTH"));/*2자에서 16자 사이로 설정하여 주시기 바랍니다.*/
...
void UIPopupUseItem::OnOpenChangeNick(void)
{
	if( !_Open(USE_ITEM_CHANGE_NICK))
		OnExitKey();
}

void UIPopupUseItem::CbChangeNickWarnning(void* pThis, INT32 nParam)
{
	if( nParam == MBR_OK)
	{
		((UIPopupUseItem*)pThis)->OnOpenChangeNick();
	}
	else
	{
		((UIPopupUseItem*)pThis)->OnCancel();
...
void UIPopupUseItem::_InitChangeClanNamePopup(void)
{
	// 아이템
	_SetCurrentUseItemName( m_pChangeNameItemValue);
...
void UIPopupUseItem::_ChangeClanNameOk(void)
{
	_SetInputLock(TRUE);
...
void UIPopupUseItem::_InitDisguiseNickPopup(void)
{
	// 아이템
	_SetCurrentUseItemName( m_pChangeNameItemValue);
...
void UIPopupUseItem::_ChangeDisguiseNickOk(void)
{
	_SetInputLock(TRUE);
...
void UIPopupUseItem::_InitDisguiseRankPopup(void)
{
	// 아이템
...
void UIPopupUseItem::_ChangeDisguiseRankOk(void)
{
...
void UIPopupUseItem::OnChangeRankPopupButton(void)
{
	CGuiNotifyReceiver::ExclusionPopup(m_pChangeRankSelectList);
}

void UIPopupUseItem::OnChangeRankPopupList(UINT32 idxItem)
{
...
void UIPopupUseItem::_InitColorNickPopup(void)
{
...
void UIPopupUseItem::_ChangeColorNickOk(void)
{
	if( m_nSelected == -1)
	{
...
void UIPopupUseItem::OnChangeColorSelect(INT32 idx)
{
...
void UIPopupUseItem::_InitColorClanNamePopup(void)
{
	m_pChangeColorCurrentStatic->SetText(GAME_STRING("STR_POPUP_CURRENT_COLOR"));/*현재 색상:*/
	m_pChangeColorSelectStatic->SetText(GAME_STRING("STR_POPUP_MODIFIED_COLOR"));/*변경 색상:*/
...
void UIPopupUseItem::_ChangeColorClanNameOk(void)
{
	if( m_nSelected == -1)
	{
		STRING_POPUP( GAME_STRING("STR_POPUP_MESSAGE_CHOOSE_MODIFY_COLOR"));/*변경할 색상을 선택해 주십시오.*/
...
void UIPopupUseItem::_InitColorCrosshairPopup(void)
{
...
void UIPopupUseItem::_ChangeColorCrosshairOk(void)
{
	if( m_nSelected == -1)
	{
		STRING_POPUP( GAME_STRING("STR_POPUP_MESSAGE_CHOOSE_MODIFY_COLOR"));/*변경할 색상을 선택해 주십시오.*/
...
void UIPopupUseItem::OnChangeCrosshairColorSelect(INT32 idx)
{
