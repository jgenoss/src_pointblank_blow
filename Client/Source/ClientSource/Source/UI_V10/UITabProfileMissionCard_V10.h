...
class UITabProfileMissionCard_V10 : public UITabProfileBase_V10
{
	I3_CLASS_DEFINE( UITabProfileMissionCard_V10, UITabProfileBase_V10);
	typedef UITabProfileBase_V10		base;
private:
	...
	i3UIButtonImageSet*	m_CardBG[MAX_CARD_PER_CARDSET];
	i3UIButtonImageSet*	m_pCardInfo;

	void			UpdateCardSlotsAndInfoUI();
	void			UpdateCardSetListSelection();

public:
	UITabProfileMissionCard_V10();
	virtual ~UITabProfileMissionCard_V10();
...
private:
	...
	void			_SetMissionBoard();

	//카드 상세정보를 갱신
	void			SetCCardNotice(INT32, const i3::rc_wstring& wstrText);
	void			SetCCardComplete(INT32, const i3::rc_wstring& wstrText);
	void			SetCCardTItle( const i3::rc_wstring& wstrText);
};
