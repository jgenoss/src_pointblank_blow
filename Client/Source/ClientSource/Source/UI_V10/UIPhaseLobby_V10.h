/**
 * \file	UI\UIPhaseLobby_V10.h
 *
 * Declares the user interface phase lobby class.
 */


#if !defined( __UI_PHASE_LOBBY_V10_H__)
#define __UI_PHASE_LOBBY_V10_H__

#include "UI/UIPhaseBase.h"
#include "UI/UIDefine.h"

enum UI_LOBBY_BUTTON
{
	ULB_NONE = -1,
	ULB_V10_QUICKJOIN,		// 빠른 입장 버튼
	ULB_V10_CREATEROOM,		// 방생성 버튼
	ULB_V10_ENTERROOM,		// 입장가능 버튼
	ULB_V10_TUTORIAL,		// 튜토리얼 버튼
	ULB_V10_CLANMATH,		// 클랜 매치 버튼
	ULB_V10_REFRESH,		// 새로고침

	ULB_V10_MAX
};

class UIPopupNickNameSetting;
class UIFloatLobbyMapInfo_V10;
class UILoadingIcon;

class UIPhaseLobby_V10 : public UIPhaseBase
{
	I3_CLASS_DEFINE( UIPhaseLobby_V10, UIPhaseBase);
...
private:
	...
	// 로비 방 정보 툴팁입니다.
	bool						m_bTTMapUpdate;
	UIFloatLobbyMapInfo_V10 *	m_pTTMapInfo;
...
private:
	typedef void	(UIPhaseLobby_V10::*call_func)(INT32, const i3::user_data&);
