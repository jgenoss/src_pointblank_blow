...
#include "UI_V10/UIFloatLobbyMapInfo_V10.h"
...
//------------------------------------------------------------------------------//
extern "C" {
	int UIPhaseLobby_V10_ClickButton( LuaState * L)
	{
		INT32 btn = i3Lua::GetIntegerArg( L, 1);
		UIPhaseLobby_V10 * pThis = static_cast<UIPhaseLobby_V10*>(GameUI::GetPhase(UIPHASE_LOBBY));
		if( pThis != nullptr)
			pThis->ClickButton( btn);
		return 0;
	}

	int UIPhaseLobby_V10_JoinRoom( LuaState * L)
	{
...
/*
	int UIPhaseLobby_V10_CreateNickname( LuaState * L)
	{
		...
		if( pThis != nullptr)
		{
			...
			{	
				...
				if (FALSE == IsValidNicknameLetterSize(wstrNick))
				{
					...
					GameUI::MsgBox( MSG_TYPE_ERROR, GAME_RCSTRING("STBL_IDX_EP_LOBBY_NICK_LENGTH"), NULL, MAKE_CALLBACK(UIPhaseLobby_V10::CbSameNick), pThis);
					return 0;
				}

				if ( ! g_pFramework->CheckValidNickname(wstrNick) )
				{
					GameUI::MsgBox( MSG_TYPE_ERROR, GAME_RCSTRING("STBL_IDX_EP_LOBBY_WRONG_NICK"), NULL, MAKE_CALLBACK(UIPhaseLobby_V10::CbSameNick), pThis);
					return 0;
				}
			...
			{
				GameUI::MsgBox( MSG_TYPE_ERROR, GAME_RCSTRING("STBL_IDX_EP_LOBBY_NICK_EMPTY"), NULL, MAKE_CALLBACK(UIPhaseLobby_V10::CbSameNick), pThis);
			}
		}
...
*/
	...
	int UIPhaseLobby_V10_SelectRoom( LuaState * L)
	{
	...
	int UIPhaseLobby_V10_SortRoomList( LuaState * L)
	{
	...
	int UIPhaseLobby_V10_ViewAdmissionRoom( LuaState * L)
	{
	...
	int UIPhaseLobby_V10_CouponInput(LuaState* L)
	{
		g_pFramework->GetUIMainframe()->TogglePopup(UPW_COUPON_INPUT);
		return 0;
	}

	int UIPhaseLobby_V10_ScrollRoomList(LuaState * L)
	{
		UIPhaseLobby_V10 * pThis = static_cast<UIPhaseLobby_V10*>(GameUI::GetPhase(UIPHASE_LOBBY));
		if( pThis != nullptr)
	...
	int UIPhaseLobby_V10_WheelRoomList(LuaState * L)
	{
	...
	int UIPhaseLobby_V10_LeaveLobby(LuaState* L)
	{
		UIPhaseLobby_V10 * pThis = static_cast<UIPhaseLobby_V10*>(GameUI::GetPhase(UIPHASE_LOBBY));
		if( pThis != nullptr)
			pThis->ChannelSelect();
	...
	int UIPhaseLobby_V10_AttendanceCheck(LuaState * L)
	{
		INT32 EventType = i3Lua::GetIntegerArg(L, 1);
		UIPhaseLobby_V10 * pThis = static_cast<UIPhaseLobby_V10*>(GameUI::GetPhase(UIPHASE_LOBBY));
		if( pThis != nullptr)
			pThis->AttendanceOpenPopup(EventType);

		return 0;
	}
}

LuaFuncReg	PhaseLobby_V10_Glue[] =
{
	{"ClickButton",				UIPhaseLobby_V10_ClickButton	},
	{"ClickJoinRoom",			UIPhaseLobby_V10_JoinRoom		},
	{"SelectRoom",				UIPhaseLobby_V10_SelectRoom		},
	{"SortRoomList",			UIPhaseLobby_V10_SortRoomList	},
	{"ViewAdmissionRoom",		UIPhaseLobby_V10_ViewAdmissionRoom},

	{"CouponInput",				UIPhaseLobby_V10_CouponInput	},
	{"ScrollRoomList",			UIPhaseLobby_V10_ScrollRoomList },
	{"WheelRoomList",			UIPhaseLobby_V10_WheelRoomList },
	{"LeaveLobby",				UIPhaseLobby_V10_LeaveLobby },
	{"AttendanceClick",			UIPhaseLobby_V10_AttendanceCheck },
	{ nullptr,		nullptr}
};
...
static void JoinRoomPasswordCallback( bool OK, const i3::string & Password, void * pUserContext)
{
	if ( OK )
	{
		...
		UIPhaseLobby_V10 *	pLobby = (UIPhaseLobby_V10 *)pUserContext;
...
bool UIPhaseLobby_V10::sm_bAwayInput = false;
SORT_ORDER_INFO UIPhaseLobby_V10::sm_SortOrder[MAX_SORT_TYPE] = 
{
...
INT32 UIPhaseLobby_V10::sm_nCurSortType = 0;

bool UIPhaseLobby_V10::m_bPcCafeNotice = false;				// PcCafe 공지 창을 보여주는지에 대한 여부
bool UIPhaseLobby_V10::m_bEventCardSetNotice = false;
bool UIPhaseLobby_V10::m_bEvnetNotice = false;


I3_CLASS_INSTANCE( UIPhaseLobby_V10);//, UIPhaseBase);

UIPhaseLobby_V10::UIPhaseLobby_V10() : m_bNickNameCreate(false), m_bAllowRefresh(false), m_bRefreshButtonActive(false)
	...
	, m_pTTMapInfo(nullptr), m_bTTMapUpdate(false)
{
	...
	RegisterGameEventFunc( EVENT_READY_BATTLE,				&UIPhaseLobby_V10::ReceiveGameEvent_Ready_Battle);
	RegisterGameEventFunc( EVENT_JOIN_QUICK,				&UIPhaseLobby_V10::ReceiveGameEvent_Join_Quick);
	RegisterGameEventFunc( EVENT_CREATE_ROOM,				&UIPhaseLobby_V10::ReceiveGameEvent_Create_Room);
	RegisterGameEventFunc( EVENT_JOIN_ROOM,					&UIPhaseLobby_V10::ReceiveGameEvent_Join_Room);
	RegisterGameEventFunc( EVENT_LEAVE_LOBBY,				&UIPhaseLobby_V10::ReceiveGameEvent_Leave_Lobby);
	RegisterGameEventFunc( EVENT_GET_ROOM_ADDINFO_N,		&UIPhaseLobby_V10::ReceiveGameEvent_Get_RoomAddInfo);
	RegisterGameEventFunc( EVENT_M_DESTROYED_NETWORK,		&UIPhaseLobby_V10::ReceiveGameEvent_DestroyedNetwork);
	RegisterGameEventFunc( EVENT_UPDATE_USER_LIST,			&UIPhaseLobby_V10::ReceiveGameEvent_Update_UserList);
	RegisterGameEventFunc( EVENT_READYBOX_MESSAGE,			&UIPhaseLobby_V10::ReceiveGameEvent_Ready_Box_Message);
	RegisterGameEventFunc( EVENT_ATTENDANCE_CLEAR,			&UIPhaseLobby_V10::ReceiveGameEvent_Attendance_Clear);
	RegisterGameEventFunc( EVENT_PLAYTIME_ALARM,			&UIPhaseLobby_V10::ReceiveGameEvent_PlayTimeAlarm);
	RegisterGameEventFunc( EVENT_SHUTDOWNTIME_ALARM,		&UIPhaseLobby_V10::ReceiveGameEvent_ShutDownTimeAlarm);
	RegisterGameEventFunc( EVENT_CREATE_NICK_NAME,			&UIPhaseLobby_V10::ReceiveGameEvent_Create_NickName);
	RegisterGameEventFunc( EVENT_CREATE_CHARA,				&UIPhaseLobby_V10::ReceiveGameEvent_Create_Chara);

	
	m_bEnableUsingClanChannel = true;
}

UIPhaseLobby_V10::~UIPhaseLobby_V10()
{
...
void UIPhaseLobby_V10::__AutoRefreshRoomList( REAL32 rDeltaSeconds)
{
...
void UIPhaseLobby_V10::__ProcessAbusing( REAL32 rDeltaSeconds)
{
...
void UIPhaseLobby_V10::_SetButtonState( void)
{
	...
	{	
		...
		if (ChannelContext::i()->GetBattleChannel()) // 클랜채널이라면 튜토리얼 버튼 대신 클랜매치 버튼을 출력함
			bEnableTutorial = false;
		i3Lua::PushBoolean(L , bEnableTutorial == true ? true : false);
		i3Lua::PushBoolean(L, m_bViewAdmissionRoom == true ? true : false);
		i3Lua::PushBoolean(L, false);
		_EndLuaFunction(L , 3);
	}
}

void UIPhaseLobby_V10::_SetCurSelectItem( void)
{
...
void UIPhaseLobby_V10::_InitRoomList(void)
{
...
void UIPhaseLobby_V10::_SetRoomList( void)
{
	...
	for( size_t idx = 0 ; idx < UI_LOBBY_ROOMLIST_VIEWCOUNT ; ++idx)
	{
		...
		if( bEnterRoom)
			...
			_SetRoomColor( idx, GameGUI::GetColor( GCT_VALUE));
		else
...
void UIPhaseLobby_V10::_BuildRoomListDummy(void)
{
...
void UIPhaseLobby_V10::_UpdateRoomList(void)
{
...
void UIPhaseLobby_V10::_ProcessFirstLogin(void)
{	
...
void UIPhaseLobby_V10::_ProcessCheckInvite(REAL32 rDeltaSeconds)
{
...
void UIPhaseLobby_V10::_ProcessCommon( REAL32 rDeltaSeconds)
{
...
void UIPhaseLobby_V10::_SetRoomInfo( INT32 idx, INT32 RoomIdx, ROOM_INFO_BASIC* pInfo)
{
	//LuaState * L;
	...
	{
		{
			...
			{
				if ( CStageInfoUtil::GetStageData(pInfo->_StageID) == nullptr)
				{
					wstr_mapNameText = L"";
				}
				else
	...
	m_pItemListBox->SetItemCell(idx, 0, wstrNumber.c_str());			// 방번호
	m_pItemListBox->SetItemCell(idx, 1, (INT32)mode);					// 모드이름
	m_pItemListBox->SetItemCell(idx, 2, wstr_mapNameText.c_str());		// 맵이름
	m_pItemListBox->SetItemCell(idx, 3, pInfo->_Title);					// 방이름
	
	m_pItemListBox->SetItemCell(idx, 4, mode_option);					// 공수 교대 & 무기 제한 여부
	m_pItemListBox->SetItemCell(idx, 5, secret);						// 방잠금

	m_pItemListBox->SetItemCell(idx, 6, wstrState.c_str());				// 상태
	m_pItemListBox->SetItemCell(idx, 7, wstrPerson.c_str());			// 사용자 수
	m_pItemListBox->SetItemCell(idx, 8, pInfo->_Ping-1);					// 핑

}

void UIPhaseLobby_V10::_SetEmptyRoomInfo( INT32 idx)
{
	for(INT32 i = 0; i < 9; i++)
	{
		i3UIControl* cell = m_pItemListBox->getCellChild(idx, i);
		if(i3::same_of<i3UIButtonImageSet*>(cell))
		{
			//cell->HideShapes(true);
			m_pItemListBox->SetItemCell(idx, i, -1);
		}
		else if(i3::same_of<i3UIListView_Cell*>(cell))
		{
			//cell->HideShapes(true);
			m_pItemListBox->SetItemCell(idx, i, "");
		}
		else if( i3UIControl_Dummy* pDummy = i3::same_cast<i3UIControl_Dummy*>(cell))
		{
			pDummy->SetChildAlignStyle(1,0);
			pDummy->SetChildPos(1, 0, 7);

			i3::vector<i3::string> List2;
			List2.push_back("        ");
			List2.push_back("-1");
			m_pItemListBox->SetItemCell(idx, 2, List2);		// 맵이름
		}
	}
}

void UIPhaseLobby_V10::_SetRoomColor( INT32 RoomIdx, I3COLOR * pColor)
{
...
void	UIPhaseLobby_V10::_RestoreRoomColor(INT32 RoomIdx)
{
	for(INT32 j=0; j< m_pItemListBox->getCellCount(RoomIdx); j++)
	{
		m_pItemListBox->RestoreItemCellTextColor(RoomIdx, j, false);
	}
}

void UIPhaseLobby_V10::_FirstLoginPopup()
{
...
void UIPhaseLobby_V10::_CloseAllSubPhaseAndPopup( void)
{
...
void UIPhaseLobby_V10::_ClearRoomInfo( void)
{
...
void UIPhaseLobby_V10::_AddMsgBoxToTopMenuScreen(void)
{
...
bool UIPhaseLobby_V10::CompareRoom( const LOBBY_ROOM_INFO* pItem1, const LOBBY_ROOM_INFO* pItem2)
{
...
bool UIPhaseLobby_V10::CompareAdmissionRoom( const LOBBY_ROOM_INFO* pItem1, const LOBBY_ROOM_INFO* pItem2)
{
	const ROOM_INFO_BASIC* pRoom[2];
...
bool UIPhaseLobby_V10::OnInviteMove( void)
{
...
void UIPhaseLobby_V10::SelectRoom( INT32 idx)
{
...
void UIPhaseLobby_V10::RefreshSelectedRoom( void)
{
...
void	UIPhaseLobby_V10::OnRefreshButton()
{
...
void  UIPhaseLobby_V10::OnScrollRoomList(void)
{
...
void  UIPhaseLobby_V10::OnWheelRoomList(INT32 zDelta)
{
	if(m_pScrollRoomList == nullptr)
		return;
...
void  UIPhaseLobby_V10::SortRoomList(INT32 nType)
{
...
void	UIPhaseLobby_V10::SortAdmissionRoom(bool bViewAdmissionRoom)
{
...
void UIPhaseLobby_V10::ConfirmAbuseOK( void)
{
...
void UIPhaseLobby_V10::DisableRoomTooltip(void)
{
...
void UIPhaseLobby_V10::_SortRoomList(void)
{
#if defined( TEST_LOBBY_ROOM_LIST)
	_BuildRoomListDummy();
#else
...
void UIPhaseLobby_V10::_EmptyRoomList(void)
{
	INT32 j;
...
/*virtual*/ void UIPhaseLobby_V10::OnCreate( i3GameNode * pParent)
{
	UIPhaseBase::OnCreate( pParent);

	// Load Scene
	AddScene( "Scene/Main/PointBlankRe_Lobby_RoomList.i3UIs",  PhaseLobby_V10_Glue);

	m_pTTMapInfo = UIFloatLobbyMapInfo_V10::new_object();
	m_pTTMapInfo->OnCreate(this);
}

void	UIPhaseLobby_V10::OnUpdate_NoneRsc(REAL32 rDeltaSeconds)
{
...
/*virtual*/ void UIPhaseLobby_V10::OnUpdate( REAL32 rDeltaSeconds)
{
	...
	if( GameLoginContext::i()->IsFirstLogin())
	{
		...
		GameUI::MsgBox( MSG_TYPE_ERROR, GAME_STRING("STBL_IDX_EP_SERVER_FAIL_REFRESH_C"), NULL, EXIT_CALLBACK(CbExitNormal,true), this);*/
	}
	...
	if( m_SelectedElement > -1 && m_SelectedElement < (INT32)m_RoomList.size() )
	{
		...
		{
			if(m_pTTMapInfo != nullptr && m_bTTMapUpdate)
			{
		...
		if( m_bRequestRoomInfoAdd)
		{
			...
			if ( g_pFramework->GetAccTime() - m_LastRefreshTime > g_pConfigEx->GetNetwork().RefreshInterval_RoomInfo)
			{
				...
				I3TRACE("EVENT_GET_ROOM_ADDINFO_N [UIPhaseLobby_V10::SelectRoom]\n");
				GameEventSender::i()->SetEvent( EVENT_GET_ROOM_ADDINFO_N );
...
/*virtual*/ void	UIPhaseLobby_V10::ProcessGameEvent( GAME_EVENT evt, INT32 arg, const i3::user_data& UserData)
{
...
bool UIPhaseLobby_V10::OnEvent(UINT32 event,i3ElementBase* pObj,UINT32 param2, I3_EVT_CODE code)
{
...
bool UIPhaseLobby_V10::OnEntranceStart( void * pArg1 /*= NULL*/, void * pArg2 /*= NULL*/)
{
...
// 입장중
/*virtual*/ bool UIPhaseLobby_V10::OnEntrancing( REAL32 rDeltaSeconds)
{
	if( g_pFramework->GetClanContext()->GetVisibleLobbyPhase())
	{
...
// 입장 완료
/*virtual*/ void UIPhaseLobby_V10::OnEntranceEnd( void)
{
	...
	if( L != NULL)
	{
		...
		i3Lua::PushBoolean(L , bClanChannel && bClanMatchEnable ? TRUE : FALSE );
		_EndLuaFunction(L , 1);
	}*/
	...
	if( TutorialContext::i()->WasTutorial())
	{
		...
		{
			//GameUI::MsgBox( MSG_TYPE_GAME_QUERY, GAME_RCSTRING("튜토리얼 모드 완료를 축하드립니다. AI전을 하시겠습니까?"), NULL, MAKE_CALLBACK(CbAIMode), this);
		...
		if (UserInfoContext::i()->IsContentsEndabled(CONTENTS_CLAN_MATCH_NEW))
	{
		...
		else if (GetMainFrame()->getDeprotationClanChannel() == false)
		{
			GameEventSender::i()->SetEventOnce(EVENT_LEAVE_LOBBY);
			m_bMyPhase = false;
		}
	...
	UIBase::_AttachToTopScreen();
}

/*virtual*/ bool UIPhaseLobby_V10::OnExitStart( void)
{
	...
	// Exit하는 순간부터는 Update에서 체크를 안합니다.
	m_bTTMapUpdate = false;
...
/*virtual*/ void UIPhaseLobby_V10::OnGotoBackPhase( void)
{
	m_bAllowRefresh = false;
	m_bRefreshButtonActive = false;
...
/*virtual*/ bool UIPhaseLobby_V10::OnDblClick( i3UIControl * pControl)
{
...
void UIPhaseLobby_V10::ClickButton( INT32 BtnIdx)
{
	if( BtnIdx > -1 && BtnIdx < ULB_V10_MAX)
	{
		switch(BtnIdx)
		{
		case ULB_V10_QUICKJOIN :	SendEventQuickJoin();			break;
		case ULB_V10_CREATEROOM :
			{
				if (
			...
			break;
		case ULB_V10_ENTERROOM :	EnterRoom( -1);			break;
		case ULB_V10_CLANMATH :		_EnterClanMatch();		break;
		case ULB_V10_REFRESH:		OnRefreshButton();		break;
		case ULB_V10_TUTORIAL:		EnterTutorial();		break;
		}
	}
}

void UIPhaseLobby_V10::EnterAIMode(void)
{
...
void UIPhaseLobby_V10::EnterTutorial( void)
{
	ROOM_INFO_BASIC		MyRoomInfo;
	ROOM_INFO_ADD		MyRoomInfoAdd;
...
// 방에 입장
void UIPhaseLobby_V10::EnterRoom( INT32 idxRoom)
{
...
void UIPhaseLobby_V10::SendEventQuickJoin( void)
{
...
void UIPhaseLobby_V10::EventItemPopup()
{
...
// PcCafe용 Popup을 띄운다.
void UIPhaseLobby_V10::ShowPcCafePopup(void)
{
...
// 공지 사항 창을 띄운다.
void UIPhaseLobby_V10::ShowNoticePopup(void)
{
	if( true )
	{
...
// 출석 체크 추가
void UIPhaseLobby_V10::ShowAttendancePopup(void)
{
...
void UIPhaseLobby_V10::EhWrongQuickJoin(void)
{
	// 방 생성 Popup을 띄운다.
	bool isLobby = true;
#if 1
...
void UIPhaseLobby_V10::EhSlotFull(void)
{
...
void UIPhaseLobby_V10::EhKickedByVote()
{
...
void UIPhaseLobby_V10::EhWrongPW(void)
{
	QPParam	Param;
...
void UIPhaseLobby_V10::EhNotExist(void)
{
...
void UIPhaseLobby_V10::CbSameNick(void* pThis,INT32 nParam)
{
	g_pFramework->GetUIMainframe()->OpenPopup( UPW_CREATENICKNAME);
}

void UIPhaseLobby_V10::CbBeInvited(void* pThis,INT32 nParam)
{
	INT32 i;

	UIPhaseLobby_V10* pLobby = (UIPhaseLobby_V10*)pThis;
...
void UIPhaseLobby_V10::CbSuppliedConfirm(void* pThis,INT32 nParam)
{
	// 프로필화면 갱신
	((UIPhaseLobby_V10*)pThis)->GetMainFrame()->EnableFloatWin( UFW_MINIPROFILE);
}

void UIPhaseLobby_V10::CbEventCardSetConfirm(void* pThis,INT32 nParam)
{
	// 이벤트 카드셋 팝업 출력 후, PC방 팝업 출력
	((UIPhaseLobby_V10*)pThis)->ShowPcCafePopup();
}
// 닉네임 생성 후 튜토리얼 모드로 진행할 것인지에 대한 팝업창
void UIPhaseLobby_V10::TutorialPopup( void)
{
...
void UIPhaseLobby_V10::CbTutorialMode(void* pThis,INT32 nParam)
{
	if( nParam == MBR_OK)
		((UIPhaseLobby_V10*)pThis)->EnterTutorial();
}

void UIPhaseLobby_V10::CbAIMode(void* pThis, INT32 nParam)
{
	if( nParam == MBR_OK)
		((UIPhaseLobby_V10*)pThis)->EnterAIMode();
	//if( nParam == MBR_OK)
	//	EnterAIMode();
}

void UIPhaseLobby_V10::CbKickedByVote( void * pThis, INT32 nParam)
{
	USER_INFO_BASIC basic;
	UserInfoContext::i()->GetMyUserInfoBasic(&basic);
	...
	((UIPhaseLobby_V10*)pThis)->m_pKickedMassageBox = nullptr;
}


// Key : 출석 체크 추가
void UIPhaseLobby_V10::CbShowAttendancePopup(void *pThis, INT32 nParam)
{
	if (nParam == MB_OK)
	{
		AttendanceContext::i()->SetNextType();
	}
	((UIPhaseLobby_V10*)pThis)->ShowAttendancePopup();
}

void UIPhaseLobby_V10::CbOnGotoBackPhase(void *pThis, INT32 nParam)
{
	((UIPhaseLobby_V10*)pThis)->OnGotoBackPhase();
}

void UIPhaseLobby_V10::CbLeaveTeam(void* pThis, INT32 nParam)
{
	// 용병 재참가
	if(nParam == MB_OK)
	{
...
void UIPhaseLobby_V10::CbRemoveMercenary(void* pThis, INT32 nParam)
{
	// 클랜채널 이탈
	if (nParam == MB_OK)
	{
		...
		// 클랜에 소속된 경우, 용병 등록 해제만으로 클랜채널에서 나갈 수 없기 때문에
		// 처리를 한번 더 해줍니다.
...
void UIPhaseLobby_V10::_InitializeAtLoad( i3UIScene * pScene )
{
	...
	i3UIButtonComposed3* pBtnClanMatch = static_cast<i3UIButtonComposed3*>(pScene->FindChildByName("Btn_ClanMatch"));
	if(pBtnClanMatch != nullptr)
	{
		if(ChannelContext::i()->GetBattleChannel() && 
			UserInfoContext::i()->GetMyClanState() != CLAN_MEMBER_LEVEL_UNKNOWN)
			pBtnClanMatch->EnableCtrl(true);
		else
			pBtnClanMatch->EnableCtrl(false);
	}
...
void UIPhaseLobby_V10::RegisterGameEventFunc( GAME_EVENT evt, call_func f)
{
	m_EventFuncMap.insert(i3::unordered_map< GAME_EVENT, call_func >::value_type( evt, f));
}

void UIPhaseLobby_V10::ReceiveGameEvent_Ready_Battle( INT32 arg, const i3::user_data&)
{
	OnExitEnd();
...
void UIPhaseLobby_V10::ReceiveGameEvent_Join_Quick( INT32 arg, const i3::user_data&)
{
	if( EV_SUCCESSED(arg))
	{	
...
void UIPhaseLobby_V10::ReceiveGameEvent_Create_Room( INT32 arg, const i3::user_data&)
{
	if( EV_SUCCESSED(arg))
	{
...
void UIPhaseLobby_V10::ReceiveGameEvent_Join_Room( INT32 arg, const i3::user_data&)
{
	DisableRoomTooltip();
...
void UIPhaseLobby_V10::ReceiveGameEvent_Leave_Lobby( INT32 arg, const i3::user_data&)
{
	if( EV_SUCCESSED(arg))
	{
...
void UIPhaseLobby_V10::ReceiveGameEvent_Get_RoomAddInfo( INT32 arg, const i3::user_data&)
{
	m_bQueRoomInfo = true;
}

void UIPhaseLobby_V10::ReceiveGameEvent_DestroyedNetwork( INT32 arg, const i3::user_data&)
{
...
void UIPhaseLobby_V10::ReceiveGameEvent_Update_UserList( INT32 arg, const i3::user_data&)
{
...
void UIPhaseLobby_V10::ReceiveGameEvent_Ready_Box_Message( INT32 arg, const i3::user_data&)
{
	//보급 상자 채팅 메세지를 띈운다.
	_ApplyMsgReadyBox();
}

void UIPhaseLobby_V10::ReceiveGameEvent_PlayTimeAlarm( INT32 arg, const i3::user_data&)
{
	GetMainFrame()->EnableFloatWin( UFW_PLAYTIMEALARM);
}

void UIPhaseLobby_V10::ReceiveGameEvent_ShutDownTimeAlarm( INT32 arg, const i3::user_data&)
{
	GetMainFrame()->EnableFloatWin( UFW_PLAYTIMEALARM);
}

void UIPhaseLobby_V10::ReceiveGameEvent_Create_NickName( INT32 arg, const i3::user_data&)
{
	// 닉네임 생성 성공
	if( EV_SUCCESSED(arg))
	{
		...
		UserInfoContext::i()->SetCreateNickName(true);
...
void UIPhaseLobby_V10::ReceiveGameEvent_Create_Chara( INT32 arg, const i3::user_data&)
{
	if (EV_SUCCESSED(arg))
	{
		if (CCharaInfoContext::i()->GetHasCharacterCount() >= TEAM_COUNT)
		{
			for (INT32 idx = 0; idx < TEAM_COUNT; ++idx)
			{
				...
				// 다른 방향보단 내꺼를 그냥 한 번 더 돌리는 셈
				if (true == UserInfoContext::i()->GetCreateNickName())
				{
					UIFloatCharacterView * pCharaView = static_cast<UIFloatCharacterView *>(GameUI::GetFloating(UFW_CHARACTER));
					if (pCharaView != nullptr)
						pCharaView->ChangeOtherUserChara(BattleSlotContext::i()->getMySlotIdx());
			}
		}
	}
}
...
void UIPhaseLobby_V10::ReceiveGameEvent_Attendance_Clear( INT32 arg, const i3::user_data&)
{
...
void UIPhaseLobby_V10::AttendanceOpenPopup(INT32 EvtType)
{
...
void UIPhaseLobby_V10::ChannelSelect()
{
	if (ChannelContext::i()->GetBattleChannel())
	{
		if (UserInfoContext::i()->IsContentsEndabled(CONTENTS_CLAN_MATCH_NEW))
