...
#if TTV_PLATFORM_IOS
/**
 * Callback signature for CVPixelBuffer unlock
 */
typedef void (*TTV_CVPixelBufferUnlockCallback) (const CVPixelBufferRef buffer, void* userData);
#endif
...
const uint kMaxLiveGameStreamsPerRequest = 100;		/* Maximum number of live streams for a game returned in each call to TTV_GetGameLiveStreams */
