...
* @notes - Twitch is moving away from granting access to this function. Please look into using 
*			a browser based login solution.
*/					   
...
/**
* TTV_GetAuthTokenWithBrowser - Get an auth token using the implicit grant method
* @param[in] clientID - The Twitch client ID assigned to your application
* @param[in] port - Local pc port to listen to for the redirect
* @param[in] successRedirect - URL to redirect to on a successful login.
* @param[in] failureRedirect - URL to redirect to on a failed login.
* @param[in] flags - A bitfield of feature options.  See twitchsdktypes.h for valid options.
* @param[in] callback - The callback function to be called when the request is completed
* @param[in] userData - Optional pointer to be passed through to the callback function
* @param[out] authToken - The authentication token to be written to
* @notes
*			- When you setup your client information for your application you are required to set a Redirect URI. To use
*			 this function set the Redirect URI to http://localhost:<port> (for example http://localhost:55542). Then pass
*			 the port you chose to this function. When the function is called a web browser will be launched and the user
*			 will be prompted to login or authorize the app as needed. Once the user accepts the auth the users' browser will
*			 be redirected to the localhost port, the twitch sdk will grab the auth token from the redirect and send the user
*			 to your successRedirect. We recommend that you setup a page on your own web server that is the redirect target
*			 and explain to the user that they need to alt-tab back to the game now. If the user fails to login or does not 
*			 authorize the token they will be redirected to the failureRedirect
*			 
*			- There is no need to embed your client secret in your app if you are using GetAuthTokenWithBrowser
*			- See more info at https://github.com/justintv/Twitch-API/blob/master/authentication.md#implicit-grant-flow
*/
TTVSDK_API TTV_ErrorCode TTV_ImplicitGrantAuthToken(const char* clientId,
													const char* port,
													const char* successRedirect,
													const char* failureRedirect,
													uint32_t flags,
													TTV_TaskCallback callback,
													void* userData,
													TTV_AuthToken* authToken);
...
* to call TTV_RequestAuthToken or TTV_RequestAuthToken to obtain a new one.
...
#if TTV_PLATFORM_IOS
#define TTV_SubmitVideoFrame_Attributes  __attribute__((deprecated("Please use the much faster TTV_SubmitCVPixelBuffer")))
#else
#define TTV_SubmitVideoFrame_Attributes
#endif
...
* @return - TTV_EC_SUCCESS if function succeeds; error code otherwise
*
* @notes - On platforms where there is a platform specific implementation of this function
*		   (e.g IOS has TTV_SubmitCVPixelBuffer) the platform specific implementation is
*          preferable and most likely a lot faster.
*/
TTVSDK_API TTV_ErrorCode TTV_SubmitVideoFrame(const uint8_t* frameBuffer, 
											  TTV_BufferUnlockCallback callback,
											  void* userData) TTV_SubmitVideoFrame_Attributes;

/**
* TTV_SubmitVideoFrameWithTimeStamp - Submit a video frame to be added to the video stream and set the timestamp
* @param[in] frameBuffer - Pointer to the frame buffer. This buffer will be considered "locked"
*                          until the callback is called.
* @param[in] callback - The callback function to be called when buffer is no longer needed
* @param[in] timeStamp - timeStamp in platform specific system time. Ideally use TTV_GetCurrentTimeStamp
* @param[in] userData - Optional pointer to be passed through to the callback function
* @return - TTV_EC_SUCCESS if function succeeds; error code otherwise
*
* @notes - On platforms where there is a platform specific implementation of this function
...
*          preferable and most likely a lot faster.
*/
TTVSDK_API TTV_ErrorCode TTV_SubmitVideoFrameWithTimeStamp(const uint8_t* frameBuffer, 
														  TTV_BufferUnlockCallback callback,
														  uint64_t timeStamp,
														  void* userData) TTV_SubmitVideoFrame_Attributes;

/**
* TTV_GetCurrentTimeStamp - Retrieves the current internal timestamp.
*
* @notes - The scale of the timestamp is system dependent and really should only be used with
*          twitch sdk functions that take a timestamp param
*/
TTVSDK_API uint64_t TTV_GetCurrentTimeStamp();

#if TTV_PLATFORM_IOS
/**
 * TTV_SubmitCVPixelBufferWithTimeStamp - Submit a video frame in the form of a CVPixelBufferRef to be added 
 * to the video stream and set the timestamp
 * @param[in] frameBuffer - CVPixelBufferRef to use. TwitchSDK will not retain or release it.
 * @param[in] callback - The callback function to be called when buffer is no longer needed
 * @param[in] timeStamp - timeStamp in platform specific system time. Ideally use TTV_GetCurrentTimeStamp
 * @param[in] userData - Optional pointer to be passed through to the callback function
 * @return - TTV_EC_SUCCESS if function succeeds; error code otherwise
 */
TTVSDK_API TTV_ErrorCode TTV_SubmitCVPixelBufferWithTimeStamp(CVPixelBufferRef frameBuffer,
									  TTV_CVPixelBufferUnlockCallback callback,
									  uint64_t timeStamp,
									  void* userData);

/**
 * TTV_SubmitCVPixelBuffer - Submit a video frame in the form of a CVPixelBufferRef to be added to the video stream
 * @param[in] frameBuffer - CVPixelBufferRef to use. TwitchSDK will not retain or release it.
 * @param[in] callback - The callback function to be called when buffer is no longer needed
 * @param[in] userData - Optional pointer to be passed through to the callback function
 * @return - TTV_EC_SUCCESS if function succeeds; error code otherwise
 */
TTVSDK_API TTV_ErrorCode TTV_SubmitCVPixelBuffer(CVPixelBufferRef frameBuffer,
									  TTV_CVPixelBufferUnlockCallback callback,
									  void* userData);
#endif
...
/**
* TTV_GetGameLiveStreams - Gets the current live streams of the given game
* @param[in] gameName - Name of the game to get the streams for
* @param[in] limit - Maximum number of streams to be returned - Maximum is kMaxLiveGameStreamsPerRequest
* @param[in] offset - Offset for pagination (you use this in conjunction with limit to get one of page of results at a time)
* @param[in] callback - The callback function to be called when the web API has been called
* @param[in] userData - Optional pointer to be passed through to the callback function
...
/**
 * TTV_GetSDKManager - Obtains a reference to the C++ SDK Manager instance.
 * @return - A reference to the SDKManager object, or nullptr on failure.
 *			The singleton returned must be initialized by calling its Init method
 *			prior to use. Call TTV_DiscardSDKManager when you no longer need the
 *			access to Twitch SDK functionality.
 */
TTVSDK_API ttv::ISDKManager* TTV_GetSDKManager();

/**
 * TTV_DiscardSDKManager - Obtains a reference to the C++ SDK Manager instance.
 * @return - true if the singleton instance of the SDK Manager was deallocated.
 *				false if the singleton hasn't been instantiated.
 */
TTVSDK_API bool TTV_DiscardSDKManager();
