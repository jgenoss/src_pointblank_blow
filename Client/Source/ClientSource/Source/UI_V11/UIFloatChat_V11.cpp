...
//								Lua Glue Functions								//
//------------------------------------------------------------------------------//
static UIFloatChat_V11 * GetThis(void)
{
	UIMainFrame * pFrame = g_pFramework->GetUIMainframe();

	if (pFrame != nullptr)
		return (UIFloatChat_V11*)pFrame->GetFloating(UFW_CHAT);
...
extern "C" {
	// 펼치기
	int UIFloatChat_V11_Expand(LuaState * L)
	{
		GetThis()->OnMaximizeStart();
		return 0;
	}

	// 접기
	int UIFloatChat_V11_Collapse(LuaState * L)
	{
		GetThis()->OnMinimizeStart();
		return 0;
	}

	int UIFloatChat_V11_SetChatType(LuaState * L)
	{
	...
	int UIFloatChat_V11_ChangeChatChannel(LuaState * L)
	{
		INT32 tab = i3Lua::GetIntegerArg(L, 1);
	...
	int UIFloatChat_V11_ChangeLanguage(LuaState * L)
	{
		GetThis()->ToggleLang();
		return 0;
	}
	...
	//일단 확성기에만 사용 합니다.
	int UIFloatChat_V11_ClickMegaPhoneOpen(LuaState * L)
	{
		UIFloatChat_V11 * pThis = static_cast<UIFloatChat_V11 *>(GameUI::GetFloating(UFW_CHAT));
		if (pThis != nullptr)
...
LuaFuncReg FloatChat_Glue_V11[] = {
	{ "Expand",					UIFloatChat_V11_Expand },
	{ "Collapse",				UIFloatChat_V11_Collapse },
	{ "SetChatType",				UIFloatChat_V11_SetChatType },	// Chatting type을 설정
	{ "ChangeChatChannel",		UIFloatChat_V11_ChangeChatChannel },
	{ "ChangeLanguage",			UIFloatChat_V11_ChangeLanguage },
	{ "ClickMegaPhoneOpen",		UIFloatChat_V11_ClickMegaPhoneOpen },
	{ nullptr,						nullptr }
};

I3_CLASS_INSTANCE(UIFloatChat_V11);

bool UIFloatChat_V11::m_bUseHistory = false;

UIFloatChat_V11::UIFloatChat_V11()
{
...
UIFloatChat_V11::~UIFloatChat_V11()
{
	...
	if (m_pUserList)
	{
		m_pUserList->SetEnable(false);
		RemoveChild(m_pUserList);
		m_pUserList->Release();
	}
}
...
/*virtual*/ void UIFloatChat_V11::_InitializeAtLoad(i3UIScene * pScene)
{
	UIFloatingBase::_InitializeAtLoad(pScene);

	if (i3::generic_is_iequal(pScene->GetName(), "PointBlankRe_ChattingWindow"))
	{

		m_slide.AddFrameWnd(m_pFrameWnd, UISlide::B_TO_T);

		m_pFrameWnd = (i3UIFrameWnd*)pScene->FindChildByName("main");
		I3ASSERT(m_pFrameWnd != nullptr);

		i3UITab * pTab = (i3UITab*)pScene->FindChildByName("i3UITab_CW_01");
		I3ASSERT(pTab);
		...
		m_pChatMainBox = (i3UIFrameWnd*)pScene->FindChildByName("i3UIFrameWnd_CW_01");
		I3ASSERT(m_pChatMainBox);

		m_pSubChatMain = (i3UIFrameWnd*)m_pChatMainBox->FindChildByName("i3UIFrameWnd_CW_01Sub");
		I3ASSERT(m_pSubChatMain);

		m_pTextBox = (i3UIEditBox*)pScene->FindChildByName("i3UIEditBox0");
		...
		m_pChatBox = (i3UIEditBox*)pScene->FindChildByName("edChat");
		I3ASSERT(m_pChatBox);

		m_pChatCombo = (i3UIComboBox*)pScene->FindChildByName("i3UIComboBox_CW_01");
		I3ASSERT(m_pChatCombo);

		m_pExpandBtn = (i3UIButton*)pScene->FindChildByName("i3UIButton0_Expand_CW");
		I3ASSERT(m_pExpandBtn);
		m_pExpandBtn->EnableCtrl(false);

		m_pCollapseBtn = (i3UIButton*)pScene->FindChildByName("i3UIButton_Collapse_CW");
		I3ASSERT(m_pCollapseBtn);

		// UserList Box
		I3ASSERT(m_pUserList != nullptr);
		m_pUserList->BindScene(pScene);
		...
		m_slide.AddFrameWnd(m_pParentMegPhoneMain, UISlide::B_TO_T);

		InitChat(CHAT_AREA_LOBBY, 6);

		GetAllCtlInfo(pScene, &m_pLargeCtl);
		m_pMainScene = pScene;
	}
	else
	{
		GetAllCtlInfo(pScene, &m_pSmallCtl);
...
void UIFloatChat_V11::GetAllCtlInfo(i3GameNode *pScene, i3::vector<ALLCONTROLINFO>* AllInfo)
{
	i3GameNode * pNode = pScene;
	...
	while (pNode != nullptr)
	{
		if (pNode->hasName())
		{
			...
			{
				ALLCONTROLINFO info;
				info.strName = pNode->GetNameString();
				info.pos = *((i3UIControl*)pNode)->getPos();
				info.size = *((i3UIControl*)pNode)->getSize();
				AllInfo->push_back(info);
			}

		}

		if (i3::kind_of<i3UIListView_Item*>(pNode)== false)
			GetAllCtlInfo(pNode->getFirstChild(), AllInfo);

		pNode = pNode->getNext();
	}
}

void UIFloatChat_V11::SetAllCtlInfo(i3::vector<ALLCONTROLINFO>* AllInfo)
{
	i3UIControl* pCtl = nullptr;
	for (size_t i = 0; i < AllInfo->size(); i++)
	{
		ALLCONTROLINFO* info = &AllInfo->at(i);
		pCtl = (i3UIControl*)m_pMainScene->FindChildByName(info->strName.c_str());
		if (pCtl != nullptr)
		{
			VEC3D pos = *pCtl->getPos();
			pos.x = info->pos.x;
			pCtl->setPos(&pos);
			pCtl->setSize(&info->size);
		}
...
ALLCONTROLINFO* UIFloatChat_V11::GetOrgCtlInfo(const char * pszName, i3::vector<ALLCONTROLINFO>* AllInfo)
{
	for (size_t i = 0; i < AllInfo->size(); i++)
	{
		ALLCONTROLINFO* info = &AllInfo->at(i);

		if (i3::generic_is_iequal(info->strName.c_str(), pszName))
		{
			return info;
		}
	}
...
/*virtual*/ void UIFloatChat_V11::_ClearAtUnload(i3UIScene * pScene)
{
	...
	m_pExpandBtn = nullptr;
	m_pCollapseBtn = nullptr;
	m_pMegaPhoneMain_EditBox = nullptr;
	m_pParentMegPhoneMain = nullptr;

	// UserList Box
	I3ASSERT(m_pUserList != nullptr);
	m_pUserList->UnBindScene();
}

void UIFloatChat_V11::_SearchLanguage(void)
{
	INT32 i, j, k;
...
void UIFloatChat_V11::_ResetLanguage(void)
{
...
//------------------------------------------------------------------------------//
/*virtual*/ void UIFloatChat_V11::OnCreate(i3GameNode * pParent)
{
	UIFloatingBase::OnCreate(pParent);

	// Load Scene
	AddScene("Scene/Main/PointBlankRe_ChattingWindow.i3UIs", FloatChat_Glue_V11);
	AddScene("Scene/Main/PointBlankRe_ShopChattingWindow.i3UIs", FloatChat_Glue_V11);

	m_pUserList = UIFloatUserList_V11::new_object();
	AddChild(m_pUserList);
...
/*virtual*/ bool UIFloatChat_V11::OnEvent(UINT32 event, i3ElementBase * pObj, UINT32 param2, I3_EVT_CODE code /*= I3_EVT_CODE_ACTIVATE*/)
{
	...
	if (event == I3_EVT_IME)
	{
		...
		{
			if (g_pFramework->getFocus() == m_pUserList->GetSerchEditBox())
			{
				m_pUserList->InputSearchEditBox(param2);
			}
			else
			{
	...
	// IME 이벤트만 체크 UserList Event를 업데이트 해야 합니다.
	else
	{
		m_pUserList->OnEvent(event, pObj, param2, code);
	}

	return bRv;
}

/*virtual*/ bool UIFloatChat_V11::OnKeyInput(i3InputDeviceManager * pInputMng)
{
...
/*virtual*/ bool UIFloatChat_V11::OnKey_Enter(void)
{
	if (this->IsMaximizeDone())
	{
		...
		if (g_pFramework->IsMessageBoxEnabled() == false)
		{
			...
			{
				UIPopupBase* pPopup = GetMainFrame()->GetCurrentPopup();
				if (i3::same_of<UIPhaseNonClanMember*>(pPopup) || i3::same_of<UIPhaseClanMember*>(pPopup))
				{
					return false;
				}
				else
...
/*virtual*/ void UIFloatChat_V11::OnCompleteLoad(void)
{
	/*if( m_pLastCtrl)
	{
...
/*virtual*/ void UIFloatChat_V11::ProcessGameEvent(GAME_EVENT evt, INT32 arg, const i3::user_data& UserData)
{
	if (m_pUserList != nullptr)
		m_pUserList->ProcessGameEvent(evt, arg, UserData);
}

/*virtual*/ bool UIFloatChat_V11::OnEntranceStart(void * pArg1 /*= nullptr*/, void * pArg2 /*= nullptr*/)
{
	if (UIFloatingBase::OnEntranceStart(pArg1, pArg2) == false)
		return false;

	m_pUserList->OnMaximizeEnd();
...
void	UIFloatChat_V11::OnEntranceEnd()
{
	UIFloatingBase::OnEntranceEnd();

	UIFloatChat_V11::SetFocus_ChatEditBox();
}

void	UIFloatChat_V11::SetFocus_ChatEditBox()
{
...
/*virtual*/ bool UIFloatChat_V11::OnExitStart(void)
{
...
/*virtual*/ bool UIFloatChat_V11::OnEnterMouse(i3UIControl * pControl)
{
...
/*virtual*/ void UIFloatChat_V11::OnMovePhase(UIPHASE phase)
{
	switch (phase)
	{
	...
	if (m_pUserList)
	{
		m_pUserList->OnMovePhase(phase);
	}

	_SetMegaPhoneMainFrame(false);
}

/*virtual*/ void UIFloatChat_V11::OnMoveSubPhase(UISUBPHASE phase)
{
	ALLCONTROLINFO* mainframe = nullptr;
	ALLCONTROLINFO* tab = nullptr;
	switch (phase)
	{
	case USP_SHOP:
	case USP_GIFTSHOP:
	case USP_INVENTORY:
		SetAllCtlInfo(&m_pSmallCtl);
		mainframe = GetOrgCtlInfo("main", &m_pSmallCtl);
		tab = GetOrgCtlInfo("i3UITab_CW_01", &m_pSmallCtl);
		break;
	default:
		SetAllCtlInfo(&m_pLargeCtl);
		mainframe = GetOrgCtlInfo("main", &m_pLargeCtl);
		tab = GetOrgCtlInfo("i3UITab_CW_01", &m_pLargeCtl);
		//	case UIPHASE_CLAN :	m_ChatArea = CHAT_AREA_CLAN_MEMBER;	break;
	}

	_SetMegaPhoneMainFrame(false);

	//m_pFrameResizer->BindFrameWnd(m_pFrameWnd, getY(m_pFrameWnd->getSize()) - (m_pTab->getHeight() + getY(m_pTab->getPos())), nullptr, sMaximizeDone, UI_SIZESTYLE_MOVEBOTTOM);

	m_pFrameResizer->ResetBindFrameWnd(&mainframe->pos, &mainframe->size, mainframe->size.y - (tab->size.y + tab->pos.y), nullptr, sMaximizeDone, UI_SIZESTYLE_MOVEBOTTOM);

	GameEventReceiver::i()->PushReceivedGameEvent(EVENT_UPDATE_USER_LIST);
}

bool UIFloatChat_V11::OnRevive(i3RenderContext * pCtx)
{
...
/*virtual*/ void UIFloatChat_V11::OnUpdate(REAL32 rDeltaSeconds)
{
	UIFloatingBase::OnUpdate(rDeltaSeconds);
}

void UIFloatChat_V11::ClickMegaPhoneOpenButton()
{
	{
		// 최상위로 그리도록 한다. 
...
bool UIFloatChat_V11::IsMinimizeDone(void)
{
	I3ASSERT(m_pFrameResizer);
	return m_pFrameResizer->IsMinimizeDone();
}

bool UIFloatChat_V11::IsMaximizeDone(void)
{
	I3ASSERT(m_pFrameResizer);
	return m_pFrameResizer->IsMaximizeDone();
}

UI_SIZESTATE	UIFloatChat_V11::GetSizeState(void) const
{
	I3ASSERT(m_pFrameResizer);
	return m_pFrameResizer->GetSizeState();
}

void UIFloatChat_V11::OnMinimizeStart(void)
{
	// 창 내부는 안보이게..
	m_pSubChatMain->EnableCtrl(false, true);
	// 버튼을 바꿔준다.
	m_pExpandBtn->EnableCtrl(true);
	m_pCollapseBtn->EnableCtrl(false);
	// Minimize Sequence를 실행
	m_pFrameWnd->addState(I3UI_STATE_WAITLOAD);
	m_pFrameResizer->OnMinimizeStart();

	// UserList
	m_pUserList->OnMinimizeStart();

	//메가폰은 최소화 했을 경우 무조건 닫습니다.
	_SetMegaPhoneMainFrame(false);

}

void UIFloatChat_V11::OnMaximizeStart(void)
{
	m_pExpandBtn->EnableCtrl(false);
	m_pCollapseBtn->EnableCtrl(true);
	m_pFrameResizer->OnMaximizeStart();
	m_pUserList->OnMaximizeStart();
}

void UIFloatChat_V11::OnMaximizeEnd(void)
{
	m_pSubChatMain->EnableCtrl(true, true);
	// 컨트롤들을 framewnd 단위가 아니라 개별적으로 enable/disable한 경우에는 다시 enable시킬때 정상적으로 enable동작이 실행되지 않을 수 있다.
	// 가급적 framewnd단위로 enable시키거나, 내부 컨트롤들을 개별 컨트롤 하여야 하는경우, framewnd에 state flag를 추가하여 refresh시킨다.
	m_pFrameWnd->addState(I3UI_STATE_WAITLOAD);
	// UserList
	m_pUserList->OnMaximizeEnd();

	//UIFloatChat_V11::SetFocus_ChatEditBox();		// 창활성화되면, 채팅창에 포커스를 자동으로 둡니다.(11.12.16.수빈)
...
void UIFloatChat_V11::SelectChatChannel(INT32 idx)
{
	if (m_bWhisperMode)
	{
		m_pChatBox->SetText(L"");
...
void UIFloatChat_V11::InitChat(CHAT_AREA area, INT32 nMaxLineCount)
{
	LuaState * L;
	...
	m_Tab[0] = CHAT_TYPE_ALL;	m_Tab[1] = CHAT_TYPE_TEAM;	m_Tab[2] = CHAT_TYPE_CLAN;	m_Tab[3] = CHAT_TYPE_WHISPER;
...
void UIFloatChat_V11::_UpdateMacroChat(void)
{
...
void UIFloatChat_V11::_SendMacroChat(INT32 nIdx)
{
...
void UIFloatChat_V11::_UpdateLang(void)
{
...
void UIFloatChat_V11::_ChatHistory(EDIT_STATE state)
{
...
void UIFloatChat_V11::_ToWhisperChatMessage(i3::wstring& out, const i3::rc_wstring& target, const i3::wliteral_range& msg)
{
	i3::sprintf(out, L"[To:%s] %s", target, msg);
}

void UIFloatChat_V11::_SetMegaPhoneMainFrame(bool bEnable)
{
	...
	if (bEnable)
	{
		...
		//VEC3D* frame = m_pSubChatMain->getPos();
		//VEC3D phone = *m_pParentMegPhoneMain->getPos();
		//phone.x = frame->x;
		//m_pParentMegPhoneMain->setPos(&phone);
	}
}

void UIFloatChat_V11::_SetChatColorFromType(void)
{
	COLOR col;
...
void UIFloatChat_V11::_UpdateWhisperChat(const i3::rc_wstring& wstrTarget)
{
...
CHAT_SYS_TYPE UIFloatChat_V11::_UpdateChatBox(const i3::wliteral_range& wImeBufRng)
{
...
bool UIFloatChat_V11::_SetWhisperChatToSend(i3::stack_wstring& out_only_chat, i3::stack_wstring& out_colored_chat, const i3::wliteral_range& wImeBufRng)
{
...
void UIFloatChat_V11::_SendWhisperChatting(const i3::rc_wstring& wstrTarget, const i3::wliteral_range& wMsgRng, const i3::wliteral_range& wImeBufRng, bool bReply)
{
...
void UIFloatChat_V11::_SendMegaPhoneChatting(const i3::wliteral_range& wMsgRng)
{
	const i3::wliteral_range& strMsg = wMsgRng;
...
void UIFloatChat_V11::_SendChattingMsg(i3::wstring& inout_wstrImeBuf, CHAT_SYS_TYPE eChatType)
{
	bool bWhisper = false;
...
void UIFloatChat_V11::_UpdateParsingType(EDIT_STATE state)
{
...
void UIFloatChat_V11::UpdateChatBuffer(void)
{
	INT32 i;
...
void UIFloatChat_V11::UpdateMegaPhoneBuffer(void)
{
	INT32 i;
...
void UIFloatChat_V11::ToggleLang(void)
{
	i3Ime * pIME = i3UI::getGlobalIme();
...
void UIFloatChat_V11::_UpdateWhisperBackupBuffer(void)
{
...
void UIFloatChat_V11::_UpdateChatType(void)
{
	...
	else if (g_pFramework->getKeyStroke() & GAME_KEY_FUNC_F4)
	{
			...
			m_bOnceChatTypeAll == true)
		{
			...
			_EndLuaFunction(L, 1);
			SelectChatChannel(CHAT_TYPE_CLAN);
		}
		g_pFramework->SetFocus(m_pChatBox);
	}
}

void UIFloatChat_V11::_SetChannel(CHAT_TYPE type)
{
	switch (type)
	{
	case CHAT_TYPE_ALL:
...
void UIFloatChat_V11::_SetChattingCombo(INT32 cur)
{
	LuaState * L;
...
void UIFloatChat_V11::_CopyChatBufferToBox(i3::wstring& inout_wstr, INT32 bufferIdx)
{
...
void UIFloatChat_V11::_CopyMegaPhoneBufferToBox(i3::wstring& inout_wstr, INT32 bufferIdx)
{
	CGameChatBox * pChatBox = g_pFramework->getChatBox();
...
void UIFloatChat_V11::SetWhisperCommand(void)
{
...
void UIFloatChat_V11::SetLastWhsiperNick(const i3::rc_wstring& wstrNick)
{
...
void UIFloatChat_V11::OnWhisper(INT32 slotIdx)
{
...
void UIFloatChat_V11::OnWhisper(const i3::rc_wstring& wstrNick)
{
	g_pFramework->SetFocus(nullptr);
...
void UIFloatChat_V11::_GetOnlyChatMessage(i3::stack_wstring& out, const i3::wliteral_range& wfullMsgRng)
{
...
void UIFloatChat_V11::SetChatTypeFromCombo(INT32 idxItem)
{
	if (idxItem == -1)
		return;
...
void UIFloatChat_V11::SetChatType(INT32 type)
{
	if (m_nChatType != type)
	{
		if (m_nChatType == CHATTING_TYPE_WHISPER)
