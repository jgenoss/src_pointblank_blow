...
#include "UI/UINewAndHotBanner.h"
#include "UI/UIToolTipLobbyMapInfo.h"
...
static UIPhaseLobby_V11 * GetThis( void)
{
	...
	I3ASSERT( i3::same_of<UIPhaseLobby_V11*>(pPhase));

	return (UIPhaseLobby_V11*) pPhase;
}
...
//------------------------------------------------------------------------------//
extern "C" {
	int UIPhaseLobby_V11_ClickButton( LuaState * L)
	{
		INT32 btn = i3Lua::GetIntegerArg( L, 1);
		UIPhaseLobby_V11 * pThis = static_cast<UIPhaseLobby_V11*>(GameUI::GetPhase(UIPHASE_LOBBY));
		if( pThis != nullptr)
			pThis->ClickButton( btn);
		return 0;
	}

	int UIPhaseLobby_V11_JoinRoom( LuaState * L)
	{
...
/*
	int UIPhaseLobby_V11_CreateNickname( LuaState * L)
	{
		...
		if( pThis != nullptr)
		{
			...
			{	
				...
				if (false == IsValidNicknameLetterSize(wstrNick))
				{
					...
					GameUI::MsgBox( MSG_TYPE_ERROR, GAME_RCSTRING("STBL_IDX_EP_LOBBY_NICK_LENGTH"), nullptr, MAKE_CALLBACK(UIPhaseLobby_V11::CbSameNick), pThis);
					return 0;
				}

				if ( ! g_pFramework->CheckValidNickname(wstrNick) )
				{
					GameUI::MsgBox( MSG_TYPE_ERROR, GAME_RCSTRING("STBL_IDX_EP_LOBBY_WRONG_NICK"), nullptr, MAKE_CALLBACK(UIPhaseLobby_V11::CbSameNick), pThis);
					return 0;
				}
			...
			{
				GameUI::MsgBox( MSG_TYPE_ERROR, GAME_RCSTRING("STBL_IDX_EP_LOBBY_NICK_EMPTY"), nullptr, MAKE_CALLBACK(UIPhaseLobby_V11::CbSameNick), pThis);
			}
		}
...
*/
	...
	int UIPhaseLobby_V11_SelectRoom( LuaState * L)
	{
	...
	int UIPhaseLobby_V11_SortRoomList( LuaState * L)
	{
		INT32 nBtnIdx = i3Lua::GetIntegerArg(L, 1);
		INT32 bAscend = i3Lua::GetIntegerArg(L, 2);
		GetThis()->SortRoomList(nBtnIdx , bAscend);
		return 0;
	}

	int UIPhaseLobby_V11_ViewAdmissionRoom( LuaState * L)
	{
	...
	int UIPhaseLobby_V11_CouponInput(LuaState* L)
	{
		g_pFramework->GetUIMainframe()->TogglePopup(UPW_COUPON_INPUT);
		return 0;
	}

	//int UIPhaseLobby_V11_ScrollRoomList(LuaState * L)
	//{
	//	UIPhaseLobby_V11 * pThis = static_cast<UIPhaseLobby_V11*>(GameUI::GetPhase(UIPHASE_LOBBY));
	//	if( pThis != nullptr)
	...
	//int UIPhaseLobby_V11_WheelRoomList(LuaState * L)
	//{
...
LuaFuncReg	PhaseLobby_V11_Glue[] =
{
	{"ClickButton",				UIPhaseLobby_V11_ClickButton	},
	{"ClickJoinRoom",			UIPhaseLobby_V11_JoinRoom		},
	{"SelectRoom",				UIPhaseLobby_V11_SelectRoom		},
	{"SortRoomList",			UIPhaseLobby_V11_SortRoomList	},
	{ nullptr,		nullptr}
};
...
static void JoinRoomPasswordCallback( bool OK, const i3::string & Password, void * pUserContext)
{
	if ( OK )
	{
		...
		UIPhaseLobby_V11 *	pLobby = (UIPhaseLobby_V11 *)pUserContext;
...
bool UIPhaseLobby_V11::sm_bAwayInput = false;
SORT_ORDER_INFO UIPhaseLobby_V11::sm_SortOrder[MAX_SORT_TYPE] = 
{
...
INT32 UIPhaseLobby_V11::sm_nCurSortType = 0;

bool UIPhaseLobby_V11::m_bPcCafeNotice = false;				// PcCafe 공지 창을 보여주는지에 대한 여부
bool UIPhaseLobby_V11::m_bEventCardSetNotice = false;
bool UIPhaseLobby_V11::m_bEvnetNotice = false;


I3_CLASS_INSTANCE( UIPhaseLobby_V11);//, UIPhaseBase);

UIPhaseLobby_V11::UIPhaseLobby_V11() : m_bNickNameCreate(false), m_bAllowRefresh(false), m_bRefreshButtonActive(false)
	...
	, m_bTutorialEnable(false)
{
	...
	RegisterGameEventFunc( EVENT_READY_BATTLE,				&UIPhaseLobby_V11::ReceiveGameEvent_Ready_Battle);
	RegisterGameEventFunc( EVENT_JOIN_QUICK,				&UIPhaseLobby_V11::ReceiveGameEvent_Join_Quick);
	RegisterGameEventFunc( EVENT_CREATE_ROOM,				&UIPhaseLobby_V11::ReceiveGameEvent_Create_Room);
	RegisterGameEventFunc( EVENT_JOIN_ROOM,					&UIPhaseLobby_V11::ReceiveGameEvent_Join_Room);
	RegisterGameEventFunc( EVENT_LEAVE_LOBBY,				&UIPhaseLobby_V11::ReceiveGameEvent_Leave_Lobby);
	RegisterGameEventFunc( EVENT_GET_ROOM_ADDINFO_N,		&UIPhaseLobby_V11::ReceiveGameEvent_Get_RoomAddInfo);
	RegisterGameEventFunc( EVENT_M_DESTROYED_NETWORK,		&UIPhaseLobby_V11::ReceiveGameEvent_DestroyedNetwork);
	RegisterGameEventFunc( EVENT_UPDATE_USER_LIST,			&UIPhaseLobby_V11::ReceiveGameEvent_Update_UserList);
	RegisterGameEventFunc( EVENT_READYBOX_MESSAGE,			&UIPhaseLobby_V11::ReceiveGameEvent_Ready_Box_Message);
	RegisterGameEventFunc( EVENT_ATTENDANCE_CLEAR,			&UIPhaseLobby_V11::ReceiveGameEvent_Attendance_Clear);
	RegisterGameEventFunc( EVENT_PLAYTIME_ALARM,			&UIPhaseLobby_V11::ReceiveGameEvent_PlayTimeAlarm);
	RegisterGameEventFunc( EVENT_SHUTDOWNTIME_ALARM,		&UIPhaseLobby_V11::ReceiveGameEvent_ShutDownTimeAlarm);
	RegisterGameEventFunc( EVENT_CREATE_NICK_NAME,			&UIPhaseLobby_V11::ReceiveGameEvent_Create_NickName);
	RegisterGameEventFunc( EVENT_CREATE_CHARA,				&UIPhaseLobby_V11::ReceiveGameEvent_Create_Chara);
	RegisterGameEventFunc( EVENT_ENTER_TEMP,				&UIPhaseLobby_V11::ReceiveGameEvent_Enter_Temp);
	RegisterGameEventFunc( EVENT_LEAVE_TEMP,				&UIPhaseLobby_V11::ReceiveGameEvent_Leave_Temp);

	m_bEnableUsingClanChannel = true;
}

UIPhaseLobby_V11::~UIPhaseLobby_V11()
{
...
void UIPhaseLobby_V11::__AutoRefreshRoomList( REAL32 rDeltaSeconds)
{
...
void UIPhaseLobby_V11::__ProcessAbusing( REAL32 rDeltaSeconds)
{
...
void UIPhaseLobby_V11::_SetButtonState( void)
{
	bool enterRoom = true;

	LuaState * L = _CallLuaFunction( "SetBtnMode");
	if (L != nullptr)
	{
		i3Lua::PushBoolean(L, true);
		i3Lua::PushBoolean(L, ChannelContext::i()->GetBattleChannel());
		// GD팀 요청으로 매치메이킹을 사용할 경우만 버튼을 On 합니다.
		i3Lua::PushBoolean(L, UserInfoContext::i()->IsContentsEndabled( CONTENTS_CLAN_MATCH_NEW));
		_EndLuaFunction(L, 3);
	}
...
void UIPhaseLobby_V11::_SetCurSelectItem( void)
{

}

void UIPhaseLobby_V11::_InitRoomList(void)
{

}

void UIPhaseLobby_V11::_ClearRoomList( INT32 count)
{
	LuaState * L = _CallLuaFunction( "ClearRoomList");
	if( L != nullptr)
	{
...
void UIPhaseLobby_V11::_SetRoomList( void)
{
	...
	for(size_t i = 0; i < m_RoomList.size(); i++)
	{
		...
		if (ROOM_STATE_EMPTY == pRoomInfo->_State)
		{
			_SetEmptyRoomInfo( i);
		}
		else
		{
			...
			_SetRoomInfo(i, pRoomInfo->_Idx, pRoomInfo);

			//	입장 할 수 여부에 따라 방 리스트의 색상을 결정
			if( bEnterRoom)
				_SetRoomColor( i, GameGUI::GetColor( GCT_VALID_C));
			else
				_SetRoomColor( i, GameGUI::GetColor( GCT_INVALID_C));
		}
	}

	// 나머지 공간을 채운다.
	for(size_t i = m_RoomList.size(); i < UI_LOBBY_ROOMLIST_VIEWCOUNT-1; i++)
		_SetEmptyRoomInfo( i);
}

void UIPhaseLobby_V11::_BuildRoomListDummy(void)
{
	...
	for( i=0; i<g_nRoomInfoDummyCount; i++)
	{
		...
		if( (INT32) (i3Math::Randf() * 2.0))		pDummy->_RoomInfo._StageMask |= STAGE_MASK_TYPE_ESCAPE;
...
void UIPhaseLobby_V11::_UpdateRoomList(void)
{
	...
	// UI RoomList를 제거
	INT32 roomCount = MAX( 9, (INT32)m_RoomList.size());
	_ClearRoomList( roomCount);

	_SetRoomList();
}

void UIPhaseLobby_V11::_ProcessFirstLogin(void)
{	
...
void UIPhaseLobby_V11::_ProcessCheckInvite(REAL32 rDeltaSeconds)
{
...
void UIPhaseLobby_V11::_ProcessCommon( REAL32 rDeltaSeconds)
{
...
void UIPhaseLobby_V11::_SetRoomInfo( INT32 idx, INT32 RoomIdx, ROOM_INFO_BASIC* pInfo)
{
	LuaState * L;
	...
	{
		{
			if (GET_ROOM_INFO_RANDOM_MODE( pInfo->_InfoFlag ))
			{
				...
				// 랜덤맵 부스트 이벤트 중이라면 EVENT 스트링을 붙인다.
				if (true == ClientStageInfoContext::i()->IsRandomMapEventOpened())
					i3::generic_string_cat(wstr_mapNameText, GAME_RCSTRING("STR_EVENT_RANDOMMAP_ROOM_LIST"));
			}
			else
			{
				...
				i3::wstring wstrStageName = mapData != nullptr ? mapData->getDisplayName() : L"NotHaveData";
	...
	if( L != nullptr)
	{
		...
		i3Lua::PushInteger( L, pInfo->_Ping-1);				// Ping
		i3Lua::PushInteger( L, state);				// 상태
		_EndLuaFunction( L, 10);
	}
}

void UIPhaseLobby_V11::_SetEmptyRoomInfo( INT32 idx)
{
	LuaState * L;
	...
	if( L != nullptr)
	{
		...
		i3Lua::PushInteger( L, -1);					// Ping
		i3Lua::PushInteger( L, -1);					// 상태
		_EndLuaFunction( L, 10);
	}
}

void UIPhaseLobby_V11::_SetRoomColor( INT32 RoomIdx, I3COLOR * pColor)
{
	for(INT32 j=0; j< m_pItemListBox->getCellCount(RoomIdx); j++)
	{
		m_pItemListBox->SetItemCellTextColor(RoomIdx, j, pColor, false);
	}
}

void	UIPhaseLobby_V11::_RestoreRoomColor(INT32 RoomIdx)
{

}

void UIPhaseLobby_V11::_FirstLoginPopup()
{
...
void UIPhaseLobby_V11::_CloseAllSubPhaseAndPopup( void)
{
...
void UIPhaseLobby_V11::_ClearRoomInfo( void)
{
	GetMainFrame()->DisableFloatWin( UFW_LOBBYMAPINFO);

	if(ChannelContext::i()->GetBattleChannel())
	{
		GetMainFrame()->EnableFloatWin( UFW_MERCENARY );
	}
	else
	{
...
void UIPhaseLobby_V11::_SetUserFilterCallback( bool bEnter)
{
	LuaState * L = _CallLuaFunction("SetFilterCallBack");

	if(L != nullptr)
	{
		i3Lua::PushUserData(L , bEnter == true? UserFilterCallback : 0);
		_EndLuaFunction(L , 1);
	}
}

void UIPhaseLobby_V11::UserFilterCallback(i3UIControl * pThis)
{
	GetThis()->ChangeUserFilterPosition((i3UIListView_Box*)pThis);
}

void UIPhaseLobby_V11::ChangeUserFilterPosition(i3UIListView_Box *pThis)
{
	for(INT32 i = 0 ; i < pThis->getCellCount() ; i++)
	{
		LuaState* L = _CallLuaFunction("ChangeFilterPos");
		INT32 width = (INT32)pThis->getHeaderButton(i)->getWidth();
		INT32 PosX = (INT32)pThis->getHeaderButton(i)->getAccumulatedPos()->x;

		if(L != nullptr)
		{
			i3Lua::PushInteger(L, i);
			i3Lua::PushInteger(L, width);
			i3Lua::PushInteger(L, PosX);
			_EndLuaFunction(L , 3);
		}
	}
}

void UIPhaseLobby_V11::_AddMsgBoxToTopMenuScreen(void)
{
...
bool UIPhaseLobby_V11::CompareRoom( const LOBBY_ROOM_INFO* pItem1, const LOBBY_ROOM_INFO* pItem2)
{
...
bool UIPhaseLobby_V11::CompareAdmissionRoom( const LOBBY_ROOM_INFO* pItem1, const LOBBY_ROOM_INFO* pItem2)
{
	const ROOM_INFO_BASIC* pRoom[2];
...
bool UIPhaseLobby_V11::OnInviteMove( void)
{
	...
	else
	{
		...
		if( ui32TargetMatch > 0 && MATCH_TEAM_INVALID_IDX != ui32TargetMatch)
		{
...
void UIPhaseLobby_V11::SelectRoom( INT32 idx)
{
...
void UIPhaseLobby_V11::RefreshSelectedRoom( void)
{
...
void	UIPhaseLobby_V11::OnRefreshButton()
{
...
void  UIPhaseLobby_V11::OnScrollRoomList(void)
{
...
void  UIPhaseLobby_V11::OnWheelRoomList(INT32 zDelta)
{
	//if(m_pScrollRoomList == nullptr)
	//	return;
...
void  UIPhaseLobby_V11::SortRoomList(INT32 nType , INT32 bAscend)
{
	// Listbox에서 직접적으로 얻어오는 idx가 RoomSortType 와 맞지 않는 인덱스를 매칭시킴
	switch (nType)
	{
	case 0:		sm_nCurSortType = ROOM_SORT_NUMBER;		break;
	case 1:		sm_nCurSortType = ROOM_SORT_ROOMNAME;	break;
	case 3:		sm_nCurSortType = ROOM_SORT_MODE;		break;
	case 5:		sm_nCurSortType = ROOM_SORT_MAP;		break;
	case 6:		sm_nCurSortType = ROOM_SORT_PERSON;		break;
	case 7:		sm_nCurSortType = ROOM_SORT_STATE;		break; // column자체는 ping
	default:	sm_nCurSortType = ROOM_SORT_NUMBER;		break;
	}

	if(sm_SortOrder[sm_nCurSortType]._bAscending == true)
		sm_SortOrder[sm_nCurSortType]._bAscending = false;
	else
		sm_SortOrder[sm_nCurSortType]._bAscending =  true;

	_ClearRoomInfo();

	_SortRoomList();

	LuaState* L = _CallLuaFunction("setFilterState");
	i3Lua::PushInteger(L , nType);
	i3Lua::PushInteger(L , bAscend);
	_EndLuaFunction(L , 2);
...
void	UIPhaseLobby_V11::SortAdmissionRoom(bool bViewAdmissionRoom)
{
...
void UIPhaseLobby_V11::ConfirmAbuseOK( void)
{
...
void UIPhaseLobby_V11::DisableRoomTooltip(void)
{
...
void UIPhaseLobby_V11::_SortRoomList(void)
{
#if defined( TEST_LOBBY_ROOM_LIST)
	_BuildRoomListDummy();
#else
...
void UIPhaseLobby_V11::_EmptyRoomList(void)
{
	INT32 j;
...
/*virtual*/ void UIPhaseLobby_V11::OnCreate( i3GameNode * pParent)
{
	UIPhaseBase::OnCreate( pParent);

	// Load Scene
	AddScene( "Scene/Main/PointBlankRe_Lobby_RoomList.i3UIs",  PhaseLobby_V11_Glue);

	//m_pTTMapInfo = UIToolTipLobbyMapInfo::new_object();
	//m_pTTMapInfo->OnCreate(this);
}

void	UIPhaseLobby_V11::OnUpdate_NoneRsc(REAL32 rDeltaSeconds)
{
...
/*virtual*/ void UIPhaseLobby_V11::OnUpdate( REAL32 rDeltaSeconds)
{
	...
	//if( pCharaView != nullptr && (pCharaView->isLoadWaiting() == false) )	// 아바타 로딩중이 아닌 경우
	//	ready = true;

	//if( ready && pCharaView->isLoadedChara())
	//{
	...
	if( m_SelectedElement > -1 && m_SelectedElement < (INT32)m_RoomList.size() )
	{
		...
		if( m_bQueRoomInfo )
		{
			if( GetMainFrame()->GetFloating( UFW_LOBBYMAPINFO)->isSliding() == false)
			{
				GetMainFrame()->EnableFloatWin( UFW_LOBBYMAPINFO, &pInfo->_RoomInfo, (void*)LobbyContext::i()->getSelectedRoomInfoAdd());
				I3TRACE( "Enable RoomInfo\n");
		...
		if( m_bRequestRoomInfoAdd)
		{
			...
			if ( g_pFramework->GetAccTime() - m_LastRefreshTime > g_pConfigEx->GetNetwork().RefreshInterval_RoomInfo)
			{
				...
				I3TRACE("EVENT_GET_ROOM_ADDINFO_N [UIPhaseLobby_V11::SelectRoom]\n");
				GameEventSender::i()->SetEvent( EVENT_GET_ROOM_ADDINFO_N );
...
/*virtual*/ void	UIPhaseLobby_V11::ProcessGameEvent( GAME_EVENT evt, INT32 arg, const i3::user_data& UserData)
{
...
bool UIPhaseLobby_V11::OnEvent(UINT32 event,i3ElementBase* pObj,UINT32 param2, I3_EVT_CODE code)
{
...
bool UIPhaseLobby_V11::OnEntranceStart( void * pArg1 /*= nullptr*/, void * pArg2 /*= nullptr*/)
{
...
// 입장중
/*virtual*/ bool UIPhaseLobby_V11::OnEntrancing( REAL32 rDeltaSeconds)
{
	if( g_pFramework->GetClanContext()->GetVisibleLobbyPhase())
	{
...
// 입장 완료
/*virtual*/ void UIPhaseLobby_V11::OnEntranceEnd( void)
{
	...
	// 신규 계정일 경우, 1.1/1.0 은 생성할 닉네임을 먼저 받아줍니다.
	...
	/* @eunil. 클랜 매치메이킹 구조 개선 아래 주석은 이제 사용하지 않지만 일단 나중을 위해...여기로 들어오면 애초에 버그
	...
	//{    
	//	// 14092 : 일반 로비에서 강제 퇴장은 클랜 미가입자만 처리
	//	// @eunil. PBBUG-16295 팝업은 따로 띄우지 않음
	//	// 첫 패널티
	//	// 클랜이 있는 용병 - 로비 입장은 유지하되 용병리스트 갱신/취소
	//	// 클랜이 없는 용병 - 채널 리스트로 바로 이동
	//	if (ClanGameContext::i()->IsMyMercDepartured() &&
	//		UserInfoContext::i()->GetMyClanState() == CLAN_MEMBER_LEVEL_UNKNOWN)
	//	{
	//		GameEventSender::i()->SetEventOnce(EVENT_LEAVE_LOBBY);
	//		m_bMyPhase = false;
	//	}
	//	else if(ClanGameContext::i()->IsMyMercDepartured())
	//	{
	...
	*/
	...
	//매치메이킹이 꺼져있는 클랜채널에서 탈퇴 및 제명되었을때 로비에서 나가지도록 함
	if (ChannelContext::i()->GetBattleChannel() &&
		UserInfoContext::i()->GetMyClanState() == CLAN_MEMBER_LEVEL_UNKNOWN &&
		!UserInfoContext::i()->CheckAuthLevel())							//MOD,GM은 클랜 유무와 상관없이 나가지지 않는다))
	{
	...
	// 퀵조인 스타트시 이미 자신의 슬롯번호가 셋팅되어 있어 슬롯번호 0번으로 고정을 건너 뜁니다.
	if (QuickJoinContext::i()->wasPassingthroughLobby() == true)
		return;
...
/*virtual*/ bool UIPhaseLobby_V11::OnExitStart( void)
{
	if( UIPhaseBase::OnExitStart() == false)
		return false;

	_SetUserFilterCallback(false);
...
/*virtual*/ void UIPhaseLobby_V11::OnGotoBackPhase( void)
{
	m_bAllowRefresh = false;
	m_bRefreshButtonActive = false;
...
/*virtual*/ bool UIPhaseLobby_V11::OnDblClick( i3UIControl * pControl)
{
...
void UIPhaseLobby_V11::ClickButton( INT32 BtnIdx)
{
	//닉네임 생성시.. 중복체크동안 모든 로비 이벤트를 막는다.
	if (!UserInfoContext::i()->HaveNickName())
	{
		I3TRACE("UIPhaseLobby_V11::ClickButton HaveNickName = nullptr\n");
		return;
	}

	switch( BtnIdx )
	{
	case ULB__V11_TUTORIAL :		EnterTutorial();		break;
	case ULB__V11_CREATEROOM :
		{
			bool isLobby = true;
			// 용병은 클랜채널에서 방 생성 못함
			if( ChannelContext::i()->GetBattleChannel() &&
		...
		break;
	case ULB__V11_ENTERROOM :	EnterRoom( -1);			break;
	case ULB__V11_QUICK :		SendEventQuickJoin();	break;
	case ULB__V11_REFRESH :		OnRefreshButton(); break;
	case ULB__V11_CLANMATCH:	
		{
...
void UIPhaseLobby_V11::EnterAIMode(void)
{
...
void UIPhaseLobby_V11::EnterTutorial( void)
{
	ROOM_INFO_BASIC		MyRoomInfo;
	ROOM_INFO_ADD		MyRoomInfoAdd;
...
void UIPhaseLobby_V11::OnCreateRoomCancel( void)
{
...
// 방에 입장
void UIPhaseLobby_V11::EnterRoom( INT32 idxRoom)
{
...
void UIPhaseLobby_V11::SendEventQuickJoin( void)
{
...
void UIPhaseLobby_V11::EventItemPopup()
{
...
// PcCafe용 Popup을 띄운다.
void UIPhaseLobby_V11::ShowPcCafePopup(void)
{
...
// 공지 사항 창을 띄운다.
void UIPhaseLobby_V11::ShowNoticePopup(void)
{
	if(true)
	{
...
// 출석 체크 추가
void UIPhaseLobby_V11::ShowAttendancePopup(void)
{
...
void UIPhaseLobby_V11::EhWrongQuickJoin(void)
{
	// 방 생성 Popup을 띄운다.
	bool isLobby = true;
#if 1
...
void UIPhaseLobby_V11::EhSlotFull(void)
{
...
void UIPhaseLobby_V11::EhKickedByVote()
{
...
void UIPhaseLobby_V11::EhWrongPW(void)
{
	QPParam	Param;
...
void UIPhaseLobby_V11::EhNotExist(void)
{
...
void UIPhaseLobby_V11::CbSameNick(void* pThis,INT32 nParam)
{
	g_pFramework->GetUIMainframe()->OpenPopup( UPW_CREATENICKNAME);
}

void UIPhaseLobby_V11::CbBeInvited(void* pThis,INT32 nParam)
{
	INT32 i;

	UIPhaseLobby_V11* pLobby = (UIPhaseLobby_V11*)pThis;
...
void UIPhaseLobby_V11::CbSuppliedConfirm(void* pThis,INT32 nParam)
{
	// 프로필화면 갱신
	((UIPhaseLobby_V11*)pThis)->GetMainFrame()->EnableFloatWin( UFW_MINIPROFILE);
}

void UIPhaseLobby_V11::CbEventCardSetConfirm(void* pThis,INT32 nParam)
{
	// 이벤트 카드셋 팝업 출력 후, PC방 팝업 출력
	((UIPhaseLobby_V11*)pThis)->ShowPcCafePopup();
}
// 닉네임 생성 후 튜토리얼 모드로 진행할 것인지에 대한 팝업창
void UIPhaseLobby_V11::TutorialPopup( void)
{
...
void UIPhaseLobby_V11::CbTutorialMode(void* pThis,INT32 nParam)
{
	if( nParam == MBR_OK)
		((UIPhaseLobby_V11*)pThis)->EnterTutorial();
}

void UIPhaseLobby_V11::CbAIMode(void* pThis, INT32 nParam)
{
	if( nParam == MBR_OK)
		((UIPhaseLobby_V11*)pThis)->EnterAIMode();
	//if( nParam == MBR_OK)
	//	EnterAIMode();
}

void UIPhaseLobby_V11::CbKickedByVote( void * pThis, INT32 nParam)
{
	USER_INFO_BASIC basic;
	UserInfoContext::i()->GetMyUserInfoBasic(&basic);
	...
	((UIPhaseLobby_V11*)pThis)->m_pKickedMassageBox = nullptr;
}


// Key : 출석 체크 추가
void UIPhaseLobby_V11::CbShowAttendancePopup(void *pThis, INT32 nParam)
{
	if (nParam == MB_OK)
	{
	...
	((UIPhaseLobby_V11*)pThis)->ShowAttendancePopup();
}

void UIPhaseLobby_V11::CbOnGotoBackPhase(void *pThis, INT32 nParam)
{
	((UIPhaseLobby_V11*)pThis)->OnGotoBackPhase();
}

void UIPhaseLobby_V11::_InitializeAtLoad( i3UIScene * pScene )
{
	...
	m_pBtnTutorial = (i3UIButton*) pScene->FindChildByName( "btn01");
	I3ASSERT( m_pBtnTutorial != nullptr);

	m_pScrollRoomList = m_pItemListBox->getScrollBar();
	I3ASSERT( m_pScrollRoomList != nullptr );

	m_pBtnTeamLIst = i3::same_cast<i3UIButton*>(pScene->FindChildByName("btn06"));
...
void UIPhaseLobby_V11::RegisterGameEventFunc( GAME_EVENT evt, call_func f)
{
	m_EventFuncMap.insert(i3::unordered_map< GAME_EVENT, call_func >::value_type( evt, f));
}

void UIPhaseLobby_V11::ReceiveGameEvent_Ready_Battle( INT32 arg, const i3::user_data&)
{
	// Key : User Guide System
	{
...
void UIPhaseLobby_V11::ReceiveGameEvent_Join_Quick( INT32 arg, const i3::user_data&)
{
	if( EV_SUCCESSED(arg))
	{	
...
void UIPhaseLobby_V11::ReceiveGameEvent_Create_Room( INT32 arg, const i3::user_data&)
{
	if( EV_SUCCESSED(arg))
	{
...
void UIPhaseLobby_V11::ReceiveGameEvent_Join_Room( INT32 arg, const i3::user_data&)
{
	DisableRoomTooltip();
...
void UIPhaseLobby_V11::ReceiveGameEvent_Leave_Lobby( INT32 arg, const i3::user_data&)
{
	if( EV_SUCCESSED(arg))
	{
...
void UIPhaseLobby_V11::ReceiveGameEvent_Get_RoomAddInfo( INT32 arg, const i3::user_data&)
{
	m_bQueRoomInfo = true;
}

void UIPhaseLobby_V11::ReceiveGameEvent_DestroyedNetwork( INT32 arg, const i3::user_data&)
{
...
void UIPhaseLobby_V11::ReceiveGameEvent_Update_UserList( INT32 arg, const i3::user_data&)
{
...
void UIPhaseLobby_V11::ReceiveGameEvent_Ready_Box_Message( INT32 arg, const i3::user_data&)
{
	//보급 상자 채팅 메세지를 띈운다.
	_ApplyMsgReadyBox();
}

void UIPhaseLobby_V11::ReceiveGameEvent_PlayTimeAlarm( INT32 arg, const i3::user_data&)
{
	GetMainFrame()->EnableFloatWin( UFW_PLAYTIMEALARM);
}

void UIPhaseLobby_V11::ReceiveGameEvent_ShutDownTimeAlarm( INT32 arg, const i3::user_data&)
{
	GetMainFrame()->EnableFloatWin( UFW_PLAYTIMEALARM);
}

void UIPhaseLobby_V11::ReceiveGameEvent_Create_NickName( INT32 arg, const i3::user_data&)
{
	if( EV_SUCCESSED(arg))
	{
		...
		UIFloatMiniProfile_V11* pProfile = static_cast<UIFloatMiniProfile_V11*>(GameUI::GetFloating(UFW_MINIPROFILE));
		pProfile->UpdateUserInfo();
	}
	else
	{
...
void UIPhaseLobby_V11::ReceiveGameEvent_Create_Chara( INT32 arg, const i3::user_data&)
{
	if( EV_SUCCESSED(arg))
	{	
		if (CCharaInfoContext::i()->GetHasCharacterCount() >= TEAM_COUNT)
		{
			for (INT32 idx = 0; idx < TEAM_COUNT; ++idx)
			{
				UINT8 slot_idx = CCharaInfoContext::i()->GetMainCharaSlotIdx(static_cast<TEAM_TYPE>(idx));
				CCharaInfoContext::i()->SetMainCharaBySlotIdx(slot_idx);
			}
		}
	}
}

void UIPhaseLobby_V11::ReceiveGameEvent_Attendance_Clear( INT32 arg, const i3::user_data&)
{
...
void UIPhaseLobby_V11::ReceiveGameEvent_Enter_Temp( INT32 arg, const i3::user_data&)
{
	GetMainFrame()->ChangeSubPhase( USP_TEMP);
}
void UIPhaseLobby_V11::ReceiveGameEvent_Leave_Temp( INT32 arg, const i3::user_data&)
{
