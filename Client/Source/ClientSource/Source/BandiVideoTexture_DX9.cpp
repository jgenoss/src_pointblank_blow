...
CBandiVideoDevice_DX9::CBandiVideoDevice_DX9()
{
	...
	m_d3d9 = g_pD3D;
	m_d3d9_device = g_pD3DDevice;
	m_hwnd_parent = g_hWnd;
}
...
HRESULT	CBandiVideoDevice_DX9::Resize(INT width, INT height)
{
	...
	// Create the viewport
	D3DVIEWPORT9 vp = { 0, 0, DWORD(width), DWORD(height), 0.0f, 1.0f };
...
HRESULT	CBandiVideoTexture_DX9::Open(INT vid_width, INT vid_height)
{
	...
	hr = D3DXCreateTexture(d3d9_device, tex_width, tex_height, 1, D3DUSAGE_DYNAMIC, m_fmt_d3d, D3DPOOL_DEFAULT, &texture);
	if(FAILED(hr))
	{
		/*hr = d3d9_device->CreateTexture(tex_width, tex_height, 1, 0, 
									m_fmt_d3d, D3DPOOL_MANAGED, &texture, 0);*/
		hr = D3DXCreateTexture(d3d9_device, tex_width, tex_height, 1, 0, m_fmt_d3d, D3DPOOL_MANAGED, &texture);
		if(FAILED(hr))
		{
			Beep(1000, 1000);
			ASSERT(0);
