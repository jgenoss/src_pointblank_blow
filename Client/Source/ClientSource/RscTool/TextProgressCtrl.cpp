...
CTextProgressCtrl::CTextProgressCtrl() : m_bShowPercentFirst(TRUE)
{
	// default range
	m_nMin = 0;
	m_nMax = 100;
...
void CTextProgressCtrl::OnPaint()
{
	...
	CRect rcPaintDCRect;
	PaintDC.GetClipBox(rcPaintDCRect);

	CMemDC dc(PaintDC, rcPaintDCRect);						// memory device context
	...
	if (GetFont())
		dc.GetDC().SelectObject(GetFont());
	else
		dc.GetDC().SelectObject(GetParent()->GetFont());
	...
	DrawEdge(dc.GetDC(), ClientRect, EDGE_SUNKEN, BF_ADJUST | BF_RECT | BF_FLAT);
	...
	if (bLeft)
		dc.GetDC().FillSolidRect(LeftRect, crBarBkColor);
	if (bMiddle)
		dc.GetDC().FillSolidRect(MiddleRect, crBarColor);
	if (bRight)
		dc.GetDC().FillSolidRect(RightRect, crBarBkColor);
	...
	{
		CString strPercent;
		strPercent.Format(_T("%d%%"), (int)((dFraction * 100.0) + 0.5));

		if (!m_bShowPercentFirst)
		{
			str.AppendFormat(strPercent);
		}
		else
		{
			strPercent.Append(str);				
			str = strPercent;
		}
	}
	...
	if (str.GetLength())
	{
		...
		if (dwStyle & PBS_VERTICAL)
		{
			pOldFont = (CFont*)dc.GetDC().SelectObject(&m_VerticalFont);

			szText = dc.GetDC().GetOutputTextExtent(str);
		...
		if (bLeft)
		{
			...
			dc.GetDC().SetTextColor(crTextColor);
			dc.GetDC().ExtTextOut(ptStart.x, ptStart.y, ETO_CLIPPED, &ClientRect, str, NULL);
			rgn.DeleteObject();
		}
		...
		if (bMiddle)
		{
			...
			dc.GetDC().SelectClipRgn(&rgn);
			dc.GetDC().SetTextColor(crTextBkColor);
			dc.GetDC().ExtTextOut(ptStart.x, ptStart.y, ETO_CLIPPED, &ClientRect, str, NULL);
			rgn.DeleteObject();
		}
