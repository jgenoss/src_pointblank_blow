...
int InjectDll()
{
	...
	void	*pLibRemote	= NULL;		// Address (in the remote process) where szLibPath will be copied to
	char*	szLibName	= NULL;
	int		nLibNameSize	= 0;
	...
	szLibName	= strrchr(szLibPath, '\\') + 1;
	if( szLibName != NULL )
	{
		nLibNameSize	= MAX_PATH - ( (szLibPath - szLibName) + 1 );
	}

	strcpy_s( szLibName, nLibNameSize, szDllName);
...
int InjectCode ()
{
	...
	BYTE		*pCodeRemote		= NULL;	// Address of InjectFunc() in the remote process.
	BYTE		*pGetSASWndRemote	= NULL;	// Address of GetSASWnd() in the remote process.
	...
	__try
	{
		...
		strcpy_s( DataLocal.szClassName, 50, "SAS Window class");
		strcpy_s( DataLocal.szWindowName, 50, "SAS window");
