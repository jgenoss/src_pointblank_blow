...
public:
	...
	// EarlyZ
	inline
	bool				isTexCoordTransform(void)				{ return (m_Code & I3G_SHADER_FLAG_TEX_TRANSFORM) != 0; }

	inline
	void				setTexCoordTransform( bool bFlag)		{ bFlag ? (m_Code |= I3G_SHADER_FLAG_TEX_TRANSFORM) : (m_Code &= ~I3G_SHADER_FLAG_TEX_TRANSFORM); }

	// Instancing
	inline
	bool				isInstancingEnable(void)				{ return (m_Code & I3G_SHADER_FLAG_INSTANCING) != 0; }

	inline
	void				setInstancingEnable( bool bFlag)		{ bFlag ? (m_Code |= I3G_SHADER_FLAG_INSTANCING) : (m_Code &= ~I3G_SHADER_FLAG_INSTANCING); }

	////////////////////////
	// Xray ±â´É
	inline
		bool				getXrayColorEnable(void)			{ return (m_Code & I3G_SHADER_FLAG_XRAYCOLOR) != 0; }

	inline
		void				setXrayColorEnable( bool bFlag)	{ bFlag ? (m_Code |= I3G_SHADER_FLAG_XRAYCOLOR) : (m_Code &= ~I3G_SHADER_FLAG_XRAYCOLOR); }

	////////////////////////
	// Light Calc. Model
	inline
	I3G_SHADER_FLAG_LIGHT_MODEL		getLightModel(void)
	{
		return (I3G_SHADER_FLAG_LIGHT_MODEL) ((m_Code >> I3G_SHADER_SHIFT_LIGHT_MODEL) & I3G_SHADER_MASK_LIGHT_MODEL);
	}

	inline
	void							setLightModel( I3G_SHADER_FLAG_LIGHT_MODEL model)
	{
		UINT64 mask = ~((UINT64)I3G_SHADER_MASK_LIGHT_MODEL << I3G_SHADER_SHIFT_LIGHT_MODEL);

		m_Code = (m_Code & mask) | (model << I3G_SHADER_SHIFT_LIGHT_MODEL);
	}
	...
	UINT64							getCode(void) const			{ return m_Code; }
	void							setCode( UINT64 code)		{ m_Code = code; }

	void							setLights( i3Light * pLight1 = nullptr, i3Light * pLight2 = nullptr);

	void							GenDefName( char * pszName, UINT16 nBufSize);
	void							SetName( const char * pszName);

	void							SaveToXML( i3XMLFile * pFile, i3XMLElement * pXML);
	void							LoadFromXML( i3XMLFile * pFile, i3XMLElement * pXML);
};
