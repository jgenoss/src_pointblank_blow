...
struct I3SND_PLAYSTATE_DATA
{
	INT32	nState = 0;					//Loop
	bool	bUseWorldCoordinate = false;	// 듣는사람의 위치에 상관없이 나는 소리인지 여부

	VEC3D*	vecPosition = nullptr;			//if(speakertype> mono) position( worldbase? listenerbase?)
...
{
	I3_EXPORT_ABSTRACT_CLASS_DEFINE( i3SoundPlayState, i3ElementBase );
...
protected:
	//Volatile Member
	i3SoundPlayInfo*		m_pInfo = nullptr;

	UINT32					m_nState = I3SND_RT_PLAYSTATE_FREE;
	UINT32					m_nIndex = 0;
	...
	//Priority
	INT32					m_nPriorityLevel = 0;	//다른 소리들과의 Priority상하관계
	INT32					m_nPriorityType = 0;	//같은 소리들과의 Priority상하 형태
	...
	INT32					m_nFinalVolume = 100;

	//Streamming
protected:
	UINT32					m_nPlayFlag = 0;
	WCHAR32					m_dwPriority = 0;
	WCHAR32					m_dwPlayPointer = 0;
...
public:
	...
	UINT32	GetVoiceNum()					{ return m_nIndex;}
...
public:
	...
	virtual bool Play( i3SoundPlayInfo * pSoundInfo, MATRIX * pMat, INT32 GrpIdx = -1, FMOD_CHANNEL_CALLBACK callback = nullptr)					{ return false;}
	virtual bool Replay( i3SoundPlayInfo * pSoundInfo, MATRIX * pMat, INT32 GrpIdx = -1, FMOD_CHANNEL_CALLBACK callback = nullptr)					{ return false;}
