#if !defined( __I3_PERSPECTIVE_SHADOW_MAP_H)
#define __I3_PERSPECTIVE_SHADOW_MAP_H
...
class I3_EXPORT_SCENE i3PerspectiveShadowMap : public i3Node
{
	I3SG_NODE_CLASS_DEFINE( i3PerspectiveShadowMap);
protected:
	UINT32						m_Style;
	i3Node *					m_pReceiver;
	i3Node *					m_pStaticCaster;
	i3Node *					m_pCaster;
	...
	UINT32						m_State;
	i3RenderTargetAttr *		m_pStaticRTAttr;
	...
	i3ShaderAttr *				m_pCopyShaderAttr;
	...
	MATRIX						m_matViewPrime;
	MATRIX						m_matProjPrime;
	MATRIX						m_matViewProjPrime;
	MATRIX						m_matShadowBuffer;
	bool						m_shadowDepthTestInverted;
...
protected:
	...
	void		TransformWorldToScreen( VEC3D * pOut, VEC3D * pVec);

public:
	i3PerspectiveShadowMap(void);
	virtual ~i3PerspectiveShadowMap(void);
	...
	i3Node *	getStaticCaster(void)				{ return m_pStaticCaster; }
	void		setStaticCaster( i3Node * pNode);
	...
	i3Texture *	getStaticShadowMap(void)			{ return m_pStaticRTAttr->GetColorRenderTarget(); }
