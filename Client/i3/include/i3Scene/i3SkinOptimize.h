...
protected:
	...
	// 가공할 데이터
	List		m_OptGroupList;		// 본 매트릭스에 맞춰진 버택스 그룹리스트
	List		m_VertexBlendList;	// i3VertexBlendBuffer 새로 생성할 버택스 데이터

	// 임시 데이터
	BinList	m_PrimitiveOrder;	// 인덱스로부터 뽑은 버택스 어레이
	BinList	m_IndexOrder;
	...
	// 인덱스에 대한 버택스 나열
	void		_BuildPrimOrder( i3VertexArray * pVA, i3IndexArray * pIA, INT32 nPrimCount );
	...
	BOOL		_CheckBoneMatrix( List * pMatrixMap, UINT32 vIdx );
...
public:
	...
	BOOL		BuildOptimizedData( I3G_PRIMITIVE type, i3VertexArray * pVA, i3IndexArray * pIA,
		INT32 BlendIndexCount, INT32 BoneMatrixCount, INT32 nPrimCount );
	...
	void		GetOptimizedData( List * pList );
