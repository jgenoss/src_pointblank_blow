...
struct ALIGN4 I3_WAYPOINTSLERP
{
	INT32		m_nCount = 0;
	REAL32		m_fHeuristicCost = 0.0f;
	VEC3D *		m_pPosition = nullptr;
};

class I3_EXPORT_SCENE i3NavigationPointSlerp : public i3ElementBase
{
	I3_EXPORT_CLASS_DEFINE( i3NavigationPointSlerp, i3ElementBase );
protected:
	UINT32		m_nType = I3_NAVPOINT_FINDTYPE_SHORTCUT;

	VEC3D		m_vecStartPos;
	VEC3D		m_vecTargetPos;

	i3::vector<VEC3D*>		m_WayPointList;		// (VEC3D)
	
	VEC3D		m_vecDirection;
	VEC3D		m_vecCurrentPos;
	REAL32		m_fSlerp = 0.0f;
	REAL32		m_fLength = 0.0f;
	INT32		m_nCurrentWayPoint = -0;
	REAL32		m_fBoundSphereRadius = 0.0f;

	REAL32		m_fHeuristicCost = 0.0f;

	i3CollisionContext *	m_pCollisionContext = nullptr;
	UINT32		m_nCollisionWorldGroupIndex = 0;
...
public:
	...
	void	setWayPoint( INT32 idx, VEC3D * pVec )		{ i3::vu::set_value_force(m_WayPointList, idx, pVec ); }
