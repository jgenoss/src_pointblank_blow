...
{
	I3_EXPORT_CLASS_DEFINE(i3OptAnimation, i3SceneOptimizer);
public:
	i3OptAnimation(void);
	...
	void	OptimizeAnim( i3Animation * pAnim, bool bDeleteTranslation = true, bool b16Bits = false);
...
private:
	...
	i3TransformSource *	OptEraseSameKeyFrame(i3TransformSource * pAnim ,REAL32	ApproximateValue, bool bDeleteTranslation = true);

	// 이전프레임과의 보간식이 일치하면 삭제하는 함수.. (등속도, 등각속도에 의한 최적화와 동일한 효과를 낼것임)
	i3TransformSource * OptEraseSameKeyFrame_Lerp(i3TransformSource * pAnim, REAL32	ApproximateValue, bool bDeleteTranslation);

	//모든 키프레임이 동일한 값을 가질경우 FIXED 플래그를 주어 첫번째 데이터만을 사용하게되고 
	//첫번째 데이터 이후의 키프레임을 삭제한다.
	//i3TransformSource *	OptEraseSameKeyFrameAll(i3TransformSource * pAnim ,REAL32	ApproximateValue);		// 없는 함수로 보여 일단 주석처리. 나중에 삭제합니다.
	...
	i3TransformSource * _OptTransSource( i3TransformSource * pSrc, bool bDeleteTranslation = true);	
};
