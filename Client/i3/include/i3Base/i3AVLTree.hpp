...
#if defined( I3_DEBUG)
#define		CALL_DUMP( pNode)		if( m_bDump) { I3TRACE( "[%p] : ", (pNode)); if( m_pDumpProc != nullptr) m_pDumpProc( &(pNode)->m_Data); I3TRACE( "\n"); }
...
{
	I3ASSERT( pNode->m_pLeft == nullptr);

	_AVLT( "AddLeft\n");

	i3AVLNode<T> * pNew = new i3AVLNode<T>;
	NetworkDump_Malloc( pNew, sizeof( i3AVLNode<T>), __FILE__, __LINE__);
...
{
	I3ASSERT( pNode->m_pRight == nullptr);
...
i3AVLNode<T> *		i3AVLTree<T>::Add( const T & data)
{
	...
	if( m_pRoot == nullptr)
	{
		...
		NetworkDump_Malloc( pNewNode, sizeof( i3AVLNode<T>), __FILE__, __LINE__);
...
T *		i3AVLTree<T>::FindLinear( const T & data)
{
	...
	info.m_pData = (T *) &data;
	info.m_pFindNode = nullptr;

	Traversal( _FindLinearProc, &info);

	if( info.m_pFindNode == nullptr)
		return nullptr;
...
{
	i3AVLNode<T> * pParent, * pSubtree = nullptr;
	i3AVLNode<T> * pRetraceStart = nullptr;
	...
	NetworkDump_Free( pNode, __FILE__, __LINE__);
	delete pNode;

	if( pRetraceStart != nullptr)
	{
...
{
	i3::vector<T*>& List = *reinterpret_cast<i3::vector<T*>*>(pUserData);
...
template <class T>
void i3AVLTree<T>::GetAll( i3::vector<T*>& List)
{
	Traversal( _GetAllProc, &List);
}
...
void i3AVLTree<T>::DumpNode( i3AVLNode<T> * pNode, TRAVERSAL pDumpProc)
{
	...
	{
		pNode = list[i];
		pDumpProc( pNode, list_size - i - 1, nullptr);
	}
}
