...
protected:
	...
	bool	m_bNoFreeMem = false;

protected:
	void	AdjustBufferSize( UINT32 newsz);

public:
	i3TBandBuffer(UINT32 allocUnit = 32) : m_AllocUnit(allocUnit) {}
...
void i3TBandBuffer<T>::AdjustBufferSize( UINT32 newsz)
{
	...
	/*
	INT32 hMem;
	if( m_pBuffer == nullptr)	hMem = i3MemAlloc( m_MaxCount * sizeof(T));
	else					hMem = i3MemReAlloc( m_pBuffer, m_MaxCount * sizeof(T));
	m_pBuffer = (T *) i3MemLock( hMem);
	*/
	if( m_pBuffer == nullptr)	m_pBuffer = (T *)i3MemAlloc( m_MaxCount * sizeof(T));
