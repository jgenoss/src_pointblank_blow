...
//#include <xutility>
#include "type_traits/has_iterator_category.h"
...
namespace i3 
{
	...
	namespace detail
	{
		// is_iterator 구현 문제로 iterator_traits 구현 변경..(2012.12.05.수빈)
		struct iterator_traits_valid_tag;
		struct iterator_traits_invalid_tag;
		template<class Iterator, class Tag>
		struct iterator_traits_impl;

		template<class Iterator>
		struct iterator_traits_impl<Iterator, iterator_traits_valid_tag>
		{	
		...
		template<class Iterator>
		struct iterator_traits_impl<Iterator, iterator_traits_invalid_tag>
		{	
			typedef mpl::na					value_type;
			typedef mpl::na					difference_type;
			typedef mpl::na					pointer;
			typedef mpl::na					reference;
			typedef mpl::na					iterator_category;
		};

	}

	template<class Iterator>
	struct iterator_traits : detail::iterator_traits_impl<Iterator,	
		typename mpl::if_< i3::has_iterator_category<Iterator>, 
		detail::iterator_traits_valid_tag, detail::iterator_traits_invalid_tag>::type > {};
	...
	struct iterator_traits<T[N]> : iterator_traits<T*> {};

	template<class T, std::size_t N>
	struct iterator_traits<const T[N]> : iterator_traits<const T*> {};

	template<class T>
	struct iterator_traits<T[]> : iterator_traits<T*> {};

	template<class T>
	struct iterator_traits<const T[]> : iterator_traits<const T*> {};
