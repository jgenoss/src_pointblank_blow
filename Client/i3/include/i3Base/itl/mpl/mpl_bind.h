...
namespace i3
{
	namespace mpl
	{
		namespace aux
		{
			...
			template<class Arg>
			struct replace_unnamed_arg<const mpl::arg<-1>, Arg> 
			{  typedef typename Arg::next next;   typedef const Arg type; };
			...
			template<int N, pp_enum_params_1(mpl_arity_limit, class U) >
			struct resolve_bind_arg< const arg<N>, pp_enum_params_1(mpl_arity_limit, U) >
			{
				typedef typename const pp_cat(apply_wrap, mpl_arity_limit)<mpl::arg<N>, pp_enum_params_1(mpl_arity_limit, U) >::type type;
			};
