...
#ifndef mpl_reverse_fold_impl_h
#define mpl_reverse_fold_impl_h
...
#define aux_iter_fold_forward_step(unused, i, unused2) \
	typedef typename apply2< ForwardOp, pp_cat(fwd_state, i), typename deref< pp_cat(iter, i)>::type>::type pp_cat(fwd_state, pp_inc(i)); \
	typedef typename mpl::next<pp_cat(iter, i)>::type	pp_cat(iter, pp_inc(i));

#define aux_iter_fold_backward_step_func(i) \
	typedef typename apply2<BackwardOp, pp_cat(bkwd_state, i), typename deref<pp_cat(iter, pp_dec(i))>::type>::type pp_cat(bkwd_state, pp_dec(i));

#define aux_iter_fold_backward_step(unused, i, j) aux_iter_fold_backward_step_func(pp_sub_d(1, j, i))

// 파일반복
#define		pp_iteration_params_1	(3, (0, mpl_unrolling_limit, "mpl_reverse_fold_impl.h"))		// 0 ~ 10까지 특화 / 자체재귀..(양개구간임에 유의할것)
#include	pp_iterate()
//
	...
	namespace mpl
	{
		namespace aux
		{
			...
			struct reverse_fold_impl
			{
				...
				typedef reverse_fold_impl<	( ( N - mpl_unrolling_limit) < 0 ? 0 : N - mpl_unrolling_limit )
			...
			{
				typedef reverse_fold_impl<-1, typename mpl::next<First>::type, Last
					, typename apply2<ForwardOp,State, typename deref<First>::type >::type
					, BackwardOp, ForwardOp	> nested_step;
...
namespace i3
{
	namespace mpl
	{
		namespace aux
		{
			...
			struct reverse_fold_impl<i_,First,Last,State,BackwardOp,ForwardOp>
			{
				typedef First iter0;
				typedef State fwd_state0;
