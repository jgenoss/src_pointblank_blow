...
namespace i3
{
	...
	public:
		...
		explicit vector(const allocator_type& al) : allocator_base(al), m_first(nullptr), m_last(nullptr), m_end(nullptr) {}
		explicit vector(size_type n) : m_first(nullptr), m_last(nullptr), m_end(nullptr) {	construct_n( n, T() );	}
		explicit vector(size_type n, const T& val) : m_first(nullptr), m_last(nullptr), m_end(nullptr) {	construct_n(n, val);  }
		vector(size_type n, const T& val, const allocator_type& al) : allocator_base(al), m_first(nullptr), m_last(nullptr), m_end(nullptr) { construct_n( n, val ); }
		vector(const my_type& rhs ) : allocator_base( rhs ), m_first(nullptr), m_last(nullptr), m_end(nullptr)
		{
		...
		template< class Iter>
		vector( Iter first, Iter last ) : m_first(nullptr), m_last(nullptr), m_end(nullptr)
		{  construct( first, last, typename iterator_traits<Iter>::iterator_category()  );      }
		
		template< class Iter>
		vector( Iter first, Iter last, const allocator_type& al ) : m_first(nullptr), m_last(nullptr), m_end(nullptr), allocator_base(al)
		{  construct( first, last, typename iterator_traits<Iter>::iterator_category()  );      }
...
#ifdef _DEBUG
			if (size() <= Pos) 
				throw std::out_of_range("i3::vector subscript out of range");
#endif
			return (*(m_first + Pos));	
		}
...
#ifdef _DEBUG
			if (size() <= Pos)
				I3TRACE("i3::vector subscript out of range\n");
#endif
			return (*(m_first + Pos));	
		}
...
////		
		...
		{	// insert _Count * _Val at _Where
			...
			{	// new stuff spills off end
				...
				i3::uninitialized_fill_1(m_last, val, allocator_base::get());			// i3::uninitialized_fill_1( m_last, Tmp, m_alloc);	// insert new stuff off end
				++m_last;
			}
			else
			{	// new stuff can all be assigned
				...
				pointer Oldend = m_last;
				m_last = i3::uninitialized_copy_1(Oldend - 1, m_last, allocator_base::get()); // i3::uninitialized_copy( Oldend - 1, Oldend, m_last, m_alloc);// copy suffix
				i3::copy_backward(Where, Oldend - 1, Oldend );	// copy hole
				i3::fill_1(Where, Tmp);			// insert into hole
			}
		}
