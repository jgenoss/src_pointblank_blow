...
namespace i3
{
	...
	namespace detail
	{
		...
		template<class T, bool is_fun> struct ct_impl3 { typedef T param_type; };
		template<class T> struct ct_impl3<T, false>    { typedef const T& param_type; };

		template<class T> struct ct_impl< T, false, false> 
		{ 
			typedef typename ct_impl3<T, is_function<T>::value >::param_type param_type;
		};
	...
	struct call_traits
	{
		...
		typedef typename detail::ct_impl< T, i3::is_pointer<T>::value, 
											 i3::is_arithmetic<T>::value || i3::is_enum<T>::value >::param_type	param_type;
	};
	...
	namespace detail
	{
		...
		{
			typedef typename detail::ct_impl< T, i3::is_pointer<T>::value, i3::is_arithmetic<T>::value 
																			|| i3::is_enum<T>::value >::param_type	type;
		};
