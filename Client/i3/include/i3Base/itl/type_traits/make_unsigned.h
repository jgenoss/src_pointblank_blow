...
namespace i3
{
	namespace detail
	{
		...
		struct make_unsigned_imp
		{
			...
			compile_assert((mpl::or_<is_integral<T>, is_enum<T> >::value));
			compile_assert(( !is_same< t_no_cv, bool>::value ));
	...
	template<class T> struct make_unsigned { typedef typename detail::make_unsigned_imp<T>::type type; };
