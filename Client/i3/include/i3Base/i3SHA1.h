...
struct SHA1Context
{
	UINT32 Intermediate_Hash[SHA1HashSize / 4] = { 0 }; /* Message Digest  */

	UINT32 Length_Low = 0;            /* Message length in bits      */
	UINT32 Length_High = 0;           /* Message length in bits      */

								/* Index into message block array   */
	INT32 Message_Block_Index = 0;
	UINT8 Message_Block[64] = { 0 };      /* 512-bit message blocks      */

	INT32 Computed = 0;               /* Is the digest computed?         */
	I3_SHA_RESULT Corrupted = shaSuccess;             /* Is the message digest corrupted? */
} ;
...
namespace i3SHA1
{
	...
	I3_EXPORT_BASE	I3_SHA_RESULT Reset(  SHA1Context *);
	I3_EXPORT_BASE	I3_SHA_RESULT Input(  SHA1Context *, const UINT8 * pMsg, UINT32 length);
	I3_EXPORT_BASE	I3_SHA_RESULT Result( SHA1Context *, UINT8 Message_Digest[SHA1HashSize]);
};
