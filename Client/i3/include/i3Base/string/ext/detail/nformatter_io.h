...
namespace i3
{
	namespace detail_sprintf
	{
		...
		public:
			...
			nformatter_io( const Formatter& fmter, size_t out_count) : m_obj(fmter.m_obj, out_count)		// 이 부분 변경해야한다..이렇게 처리하면..
			{																								// sprint_object가 복사가 된다..
			}																								//

			template<class T> inline typename i3::disable_if<is_scalar<T>,
			snprint_io<Ch, Tag>&>::type	operator%(const T& val) 
			{ 
				return m_obj % val;
			}

			template<class T> inline typename i3::enable_if< is_scalar<T>,
			snprint_io<Ch, Tag>&>::type	operator%( T val )
			{
				return m_obj % val;
			}
			
			template<class T> inline
			snprint_io<Ch, Tag>&	operator%(const T* val )
			{
				return m_obj % val;
			}
