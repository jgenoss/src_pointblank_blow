...
namespace i3
{
	struct generic_nless_filepath
	{
		generic_nless_filepath(std::size_t n) : m_size(n) {}	std::size_t m_size;
		...
		bool operator()(const Rng1& lhs, const Rng2& rhs) const
		{
			return i3::range::lexicographical_compare(
				i3::as_n_literal(lhs, m_size), i3::as_n_literal(rhs, m_size), is_less_filechar());
		}
