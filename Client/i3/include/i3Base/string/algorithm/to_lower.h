...
namespace i3
{
	...
	{
		typename iterator_return<OutIt>::type it = range::transform( i3::as_literal(Input), Output, to_lower_functor_fast<typename range_value<Range>::type>());
		*it = 0;			// 널값처리 필요..
		return it;
	}
	...
	{
		return range::transform( i3::as_literal(Input), Output, to_lower_functor_fast<typename range_value<Range>::type>());
	}

	template<class Seq> inline
	Seq to_lower_copy( const Seq& Input)
	{
		to_lower_functor_fast<typename range_value<Seq>::type> fn();
	...
	{
		range::inplace_transform(i3::as_literal(InOut), to_lower_functor_fast<typename range_value<InOutRange>::type>() );
	}
