...
public:
	...
	void	setAddress( char * address )		{ i3String::Copy( m_szAddress, address ); }

	UINT16	getPort( void )						{ return m_nPort; }
	void	setPort( UINT16 nPort )				{ m_nPort = nPort; }

	char *	getUserID( void )					{ return m_szUserID; }
	void	setUserID( char * id )				{ i3String::Copy( m_szUserID, id ); }

	char *	getUserPass( void )					{ return m_szUserPass; }
	void	setUserPass( char * pass )			{ i3String::Copy( m_szUserPass, pass ); }
