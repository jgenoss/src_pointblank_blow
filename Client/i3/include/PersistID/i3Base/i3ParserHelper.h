...
inline INT32 GetIndexFromString( const char *szBuf, const char *szString[], UINT32 nNum )
{
	...
	for( i = 0; i < nNum; i++ )
	{
		...
		if( i3String::Compare( szBuf, szString[i] ) == 0 )			
		{
			return i;
		}
	}
...
#define _GET_VALUE(_LABEL_,_DEFAULT_,_VAR_,_IMPORTANT_)	if(!pParser->GetValue(_LABEL_,_DEFAULT_,_VAR_))\
														{\
															...
															if( bTemp)\
															{\
																I3FATAL( "Failed to find label '%s'", _LABEL_);\
																goto ExitLabel;\
															}\
														}
...
#define _GET_STRING(_LABEL_,_DEFAULT_,_VAR_,_BUFSZ_,_IMPORTANT_)	if(!pParser->GetValue(_LABEL_,_DEFAULT_,_VAR_,_BUFSZ_))\
														{\
															BOOL bTemp = _IMPORTANT_;\
															if( bTemp)\
															{\
...
#define _GET_INDEX(_LABEL_,_STRLIST_,_DEFAULT_,_VAR_,_IMPORTANT_)	{\
																		...
																		{\
																			if(_VAR_<0)\
																			{\
																				I3FATAL( "Unknown '%s', '%s'", _LABEL_, pParser->GetTempBuffer() );\
																				goto ExitLabel;\
																			}\
																			else\
																			{\
