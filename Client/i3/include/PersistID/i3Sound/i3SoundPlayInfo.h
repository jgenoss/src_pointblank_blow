...
{
	I3_CLASS_DEFINE( i3SoundPlayInfo);
...
protected:
	i3Sound *			m_pSound;					// 플레이될 사운드 소스

	UINT32				m_nStyle;					// 플레이될 사운드 기본 스타일

	INT32				m_nDefaultVolume;			// 0 ~ 100 : default 볼륨이면서 3dPositional volume의 볼륨과 같이 작용
	INT32				m_nListenerVolume;			// 0 ~ 100 : positional volume을 사용하지 않는것의 볼륨

	INT32				m_nPriorityLevel;			// 우선순위 0 : Highest, 255 : Lowest : 미적용

	REAL32				m_rMinDistance;				// 사운드의 소리가 작아지기 시작하는거리, 이거리의 1/2n 만큼 소리는 증가한다.
	REAL32				m_rMaxDistance;				// 사운드의 소리가 더이상 작아지지 않는거리, bMuteAtMaxDistance가 켜져잇다면, 이거리 이후에는 소리는 나지않는다.
	INT32				m_nVolType;					// 현재 적용되지 않음.

	bool				m_bMuteAtMaxDistance;		// max거리에서 소리가 묵음이 될지 여부

	VEC3D				m_vPosition;				// 발음 위치( attached 라면 부모의 영향을 받는다.)
	VEC3D				m_vDirection;				// 발음 방향(				"					 )
	...
	REAL32				m_rUpdateInterval;			// Update 간격

	UINT32				m_nFrequency;				// Frequency( pitch)

	INT32				m_nI3DL2RvbPreset;			// 환경 리버브 프리셋

	//volatile
	bool				m_bForceStopAtStopAllSound;	// StopAllSound( false)가 요청되는경우에 멈추어야할사운드인지 여부
