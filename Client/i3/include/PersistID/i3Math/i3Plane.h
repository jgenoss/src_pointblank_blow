...
//		i3Vector::Scale( pOut, pIn, 1.0f / mag);
	...
	inline
	I3_FACE_SIDE	getSide( VEC4D * pPlane, VEC3D * p)
	{
		REAL32 dist = Distance( pPlane, p);

		if( i3Math::abs(dist) < 0.0001f)
	...
	inline
	I3_FACE_SIDE	getSide( VEC4D * pPlane, VEC3D * p1, VEC3D * p2, VEC3D * p3)
	{
		INT32 front, back;
		I3_FACE_SIDE side;
		...
		side = getSide( pPlane, p1);
		if( side == I3_FACE_FRONT)			front++;
		else if( side == I3_FACE_BACK)		back++;

		side = getSide( pPlane, p2);
		if( side == I3_FACE_FRONT)			front++;
		else if( side == I3_FACE_BACK)		back++;

		side = getSide( pPlane, p3);
		if( side == I3_FACE_FRONT)			front++;
		else if( side == I3_FACE_BACK)		back++;
