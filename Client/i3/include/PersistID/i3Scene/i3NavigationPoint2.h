...
// 포인터 구조
typedef struct ALIGN4 _tagi3NavPoint
{
	VEC3D		m_vecPosition;				// 좌표
	REAL32		m_fRadius;					// 영역
	
	INT32		m_nNeighborCount;			// 이웃한 포인터 갯수
	UINT16 *	m_nNeighborArray;
	VEC2D *		m_pNeighborBox;				// 이웃한 포인터에 대한 박스(2개점만 가지고 있으면 된다)
} I3_NAVPOINT_WITHSPHERE;

class I3_EXPORT_SCENE i3NavigationPoint2 : public i3Node
{
	I3_CLASS_DEFINE( i3NavigationPoint2 );
protected:
	// Persistent member
	INT32		m_nPointCount;				// 전체 포인트 갯수
	I3_NAVPOINT_WITHSPHERE *	m_pPoint;	// 전체 포인트

	UINT16		m_nIndex;					// 현재 노드의 인덱스

	// Volatile member
	UINT8 *		m_pNavInfo;					// 현재 포인트에서 이웃한 포인트에 대한 방향
	INT32		m_NavInfoSize;
