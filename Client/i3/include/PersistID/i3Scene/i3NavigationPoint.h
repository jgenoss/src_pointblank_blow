...
typedef struct PACKED _tag_I3_NAVPOINT
{
	VEC2D	vecPos;
	UINT8	nNeighbor[I3_MAX_NVPOINT_NEIGHBOR];
} I3_NAVPOINT_INFO;
...
{
	I3_CLASS_DEFINE( i3NavigationPoint );
protected:
	I3_NAVPOINT_INFO	*m_pPointInfo;
//	INT32				m_hPointMemId;
...
public:
	...
	VEC2D *				GetPosition( UINT8 idx)			{ return &m_pPointInfo[idx].vecPos; }
	void				SetPosition( UINT8 idx, VEC2D *pPos );

	I3_NAVPOINT_INFO *	GetPointArray(void)				{ return m_pPointInfo; }
	I3_NAVPOINT_INFO *	GetPoint( UINT8 idx )			{ return m_pPointInfo + idx; }
	...
	{
		//I3ASSERT( m_pPointInfo != NULL && nIndex < m_nCount );
		//I3ASSERT( 0 <= nCount && nCount < I3_MAX_NVPOINT_NEIGHBOR );
		return m_pPointInfo[ nIndex ].nNeighbor[ nCount ];
	}

	I3_NAVPOINT_INFO*	GetNeighbor( UINT8 nIndex, UINT8 nCount )
	{
		//I3ASSERT( m_pPointInfo != NULL && nIndex < m_nCount );
		//I3ASSERT( 0 <= nCount && nCount < I3_MAX_NVPOINT_NEIGHBOR );
