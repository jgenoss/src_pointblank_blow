...
{
	I3_EXPORT_CLASS_DEFINE( i3GameObjBase, i3GameNode);
protected:
	UINT32							m_Type = 0;
	...
	// Persist Members
	i3GameResSceneGraph	 *			m_pGameRes = nullptr;

	i3::vector<i3ResourceObject*>	m_RefClassList;
protected:
...
public:
	...
	virtual bool	Create( i3Node *pParentNode = nullptr, bool bEnable = true );
	...
	void				InitColliderSet( INT32 GrpIdx, UINT32 mask, i3Node * pRoot = nullptr );
	...
	INT32				getRefObjectCount(void) const
	{ 
		return m_RefClassList.size(); 
	}
	i3ResourceObject *	getRefObject( INT32 idx) const		{ return m_RefClassList[idx]; }

	virtual void	AttachScene(void);
	virtual void	DetachScene(void);
	bool			IsAttachedScene(void)						{ return (m_GameNodeState & I3_GAMENODE_STATE_ATTACHED) != 0; }
	...
	virtual void		OnBuildObjectList(i3::vector
		<i3PersistantElement*>& List);
