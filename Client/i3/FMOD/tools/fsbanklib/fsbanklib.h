...
{                                   
    ...
    unsigned int mode;                   // FSOUND_MODES bits. Bits allowed are FSOUND_LOOP_NORMAL, FSOUND_LOOP_BIDI, FSOUND_2D, FSOUND_HW2D, FSOUND_HW3D, FSOUND_CHANNELMODE_ALLMONO and FSOUND_CHANNELMODE_ALLSTEREO
    int          quality;                // Compression quality. 0 = lowest, 100 = highest, -1 = use "quality" parameter passed to FSBank_Build function
    bool         optimisesamplerate;     // Ouput sample rate optimisation flag. true=attempt to optimise
    float        optimisedratereduction; // If 'optimisesamplerate' is true, try to multiply the optimised rate by this fraction
