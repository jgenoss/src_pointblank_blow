...
LONGLONG WINAPI llMulDiv(LONGLONG a, LONGLONG b, LONGLONG c, LONGLONG d)
{
    ...
    ULARGE_INTEGER p[2];
	p[0].LowPart = p[0].HighPart = 0;
	p[1].LowPart = p[1].HighPart = 0;
    ...
    */
    ...
    if (d != 0) {
		...
		ud[0].LowPart = ud[0].HighPart = 0;
		ud[1].LowPart = ud[1].HighPart = 0;

        if (bSign) {
            ud[0].QuadPart = (DWORDLONG)(-d);
            if (d > 0) {
                /*  -d < 0 */
                ud[1].QuadPart = (DWORDLONG)(LONGLONG)-1L;
            } else {
                ud[1].QuadPart = (DWORDLONG)0L;
            }
        } else {
            ud[0].QuadPart = (DWORDLONG)d;
            if (d < 0) {
