...
bool		operator<(const i3ClassMeta& lhs, const i3ClassMeta& rhs)
{
	return lhs.getHashCode() < rhs.getHashCode();
}


i3::vector_set<i3ClassMeta*>		s_ClassMetaList;
...
{
	if (s_ClassMetaList.empty()) 
		return TRUE;

	g_iNumPool = 0;

	for(size_t i = 0; i < s_ClassMetaList.size(); i++)
	{
		i3ClassMeta * pMeta = s_ClassMetaList.get_vector()[i];
...
void i3ClassMeta::UnRegister(void)
{
	...
	const size_t cnt = s_ClassMetaList.size();
...
void i3ClassMeta::PrintClassMeta()
{
	...
	for(size_t i=0; i< s_ClassMetaList.size(); i++)
	{
		pMeta = s_ClassMetaList.get_vector()[i];
...
i3ClassMeta * i3ClassMeta::FindClassMetaByHash( UINT32 hash)
{
	...
	start = 0;
	end = (INT32)s_ClassMetaList.size() - 1;

	while( start < end)
	{
		idx = (start + end) >> 1;
		
		pMeta = s_ClassMetaList.get_vector()[idx];
...
void i3ClassMeta::GetAllDerivedClasses( i3::vector<i3ClassMeta*>& List, bool bRecursive)
{
	i3ClassMeta * pMeta;
	...
	for(size_t i = 0; i < s_ClassMetaList.size(); i++)
	{
		...
		{
			List.push_back( pMeta);

			if( bRecursive)
			{
				pMeta->GetAllDerivedClasses( List, bRecursive);
			}
		}
	}
}
...
void i3ClassMeta::CheckCollision(void)
{
	...
	for(size_t i = 0; i < s_ClassMetaList.size() - 1; i++)
	{
		pMeta1 = s_ClassMetaList.get_vector()[i];

		for(size_t j = i + 1; j < s_ClassMetaList.size(); j++)
		{
			pMeta2 = s_ClassMetaList.get_vector()[j];
