...
void	CSelectTextureDialog::_SetTextureList(const i3::shared_ptr<i3::vector<i3GuiToolImageElement*> >& pList)
{
	if( pList == NULL )	return ;

	const size_t	nCount = pList->size();
	for( size_t	i = 0;i < nCount; ++i)
	{
		i3GuiToolImageElement * pElm = (*pList)[i];
