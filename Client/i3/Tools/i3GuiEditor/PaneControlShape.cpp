...
CPaneControlShape::~CPaneControlShape()
{
	...
	i3::cu::for_each_safe_release(m_ShapeRectList, &i3GuiToolRectElement::Release);
}
...
/*
//
void	CPaneControlShape::_CreateBitmap(HBITMAP * hBitmap,INT32	nStartX, INT32	nStartY, INT32	nWidth, INT32	nHeight, i3Texture * pTex)
{
	/////////////////////////////////////////////////////////////////////////////////////////////////
	//Create Bitmap
	BITMAPINFOHEADER fi;
	...
	//Create DC
	//For Image
	*hBitmap = CreateDIBSection(NULL,&ih,DIB_RGB_COLORS,&pRaster,NULL,0);

	if( pTex == NULL )	return ;
	if( pRaster == NULL )	return ;

	//get rect in image
	INT32	nSrcStartPixelPos	= i3Gfx::CalcRasterBufferSize( 1,1,1,pTex->GetFormat()) * nStartX; //원본이미지의 x좌표의 초기위치
	INT32	nSrcLineSize		= i3Gfx::CalcRasterBufferSize( pTex->GetWidth(), 1, 1, pTex->GetFormat()); //원본이미지의 한라인 길이
	INT32	nLineSize			= i3Gfx::CalcRasterBufferSize( nWidth, 1, 1, pTex->GetFormat());//원본이미지중 복사할 길이
	INT32	nBufferSize			= nLineSize * nHeight;
...

	{
		INT32	i;
		char *  pTemp = pSrc;
		char *	pLocked	= (char*)pTex->Lock(0);

		pLocked += nSrcStartPixelPos + nStartY * nSrcLineSize;
		for( i = nStartY; i < (nStartY + nHeight); ++i)
		{
			memcpy( pTemp, pLocked, nLineSize );
	...
	i3Image_Convert( pTex->GetFormat(), pSrc, nWidth, nHeight, 1, pTex->GetClut(0),
...
BOOL CPaneControlShape::OnInitDialog()
{
	...
	i3TDK::RegisterUpdate( m_hWnd, i3GameNode::static_meta(), I3_TDK_UPDATE_SET);
	i3TDK::RegisterUpdate( m_hWnd, i3GuiTemplateElement::static_meta(), I3_TDK_UPDATE_ALL );
	i3TDK::RegisterUpdate( m_hWnd, i3Texture::static_meta(), I3_TDK_UPDATE_SET );
	i3TDK::RegisterUpdate( m_hWnd, i3GuiImage::static_meta(), I3_TDK_UPDATE_EDIT);
	i3TDK::RegisterUpdate( m_hWnd, i3GuiControl::static_meta(), I3_TDK_UPDATE_SELECT);

	return TRUE;  // return TRUE unless you set the focus to a control
	// EXCEPTION: OCX Property Pages should return FALSE
}

void CPaneControlShape::_SetShapeRect( i3GuiImage * pImage, INT32	nIndex)
{
	if( pImage == NULL )	return ;

	if( nIndex > (INT32)m_ShapeRectList.size())	return ;

	i3GuiToolRectElement * pElm = i3GuiToolRectElement::new_object();
	...
	i3GuiToolRectElement * pOldElm = m_ShapeRectList[nIndex];
	I3_SAFE_RELEASE( pOldElm);

	m_ShapeRectList[nIndex] = pElm;
}

bool CPaneControlShape::InitShapeRect(i3GuiTemplateElement* pElement)
{
	if ( !m_bNeedCalculateShapeRect)
		return false;

	i3::cu::for_each_safe_release_clear(m_ShapeRectList, &i3GuiToolRectElement::Release);
...
//			I3ASSERT( pElmImage != NULL);
		...
		m_ShapeRectList.push_back( pElm );
	...
	{
		m_pParentCtrl->setSize( m_CurCtrlRect.right, m_CurCtrlRect.bottom); 
	}

	//i3TDK::Update( m_hWnd, I3_TDK_UPDATE_EDIT | I3_TDK_USRWND_CTRLSHAPE, pElement);
...
void CPaneControlShape::InitTemplate(i3GuiTemplateElement * pElement)
{
...
void CPaneControlShape::OnUpdate( I3_TDK_UPDATE_INFO * pInfo)
{
	...
	if( pInfo->m_Event & I3_TDK_UPDATE_SELECT )
	{
		...
		{
			if( i3::kind_of<i3GuiControl*>(pInfo->m_pMeta)) //->IsTypeOf(i3GuiControl::static_meta()))
			{
				m_pParentCtrl = (i3GuiControl*)pInfo->m_pObject;
				i3GuiControlData * pData = m_pParentCtrl->GetControlData();
				m_bFromControl = TRUE;
		...
		{
			if( i3::kind_of<i3GuiTemplateElement*>(pInfo->m_pMeta)) //->IsTypeOf( i3GuiTemplateElement::static_meta()))
			{
				m_bFromControl	= FALSE;
				m_pParentCtrl	= NULL;
				InitTemplate((i3GuiTemplateElement*)pInfo->m_pObject);
			}
		}
		else
		{
	...
	else if( pInfo->m_Event & I3_TDK_UPDATE_SET)
	{
		...
		{
			if( i3::same_of<i3GuiRoot*>(pInfo->m_pMeta)) //->IsExactTypeOf( i3GuiRoot::static_meta()))
			{
				m_pTemplate = NULL;
				Invalidate();
			}
	...
	else if( pInfo->m_Event & I3_TDK_UPDATE_EDIT )
	{
		if( pInfo->m_Event & I3_TDK_USRWND_UVEDIT || pInfo->m_Event & I3_TDK_USRWND_PROPERTY)
		{
			...
			if( pInfo->m_pMeta == i3GuiImage::static_meta())
			{
				if( m_pTemplate != NULL )
				{
					m_bNeedCalculateShapeRect = TRUE;
		...
		{
			if( i3::kind_of<i3GuiImage*>(pInfo->m_pMeta)) //->IsTypeOf(i3GuiImage::static_meta()))
			{
				_InitUV( m_nCurShape, (i3GuiImage*)pInfo->m_pObject);
			}
		}
	}
}
...
void CPaneControlShape::OnPaint()
{
	...
	{
		i3GuiToolRectElement * pElm = m_ShapeRectList[i];
		...
		{
			i3GuiImage * pImage = m_pTemplate->getImage( i);
			INT32 idx = i3::vu::int_index_of(m_listSelected, pImage);
			if( idx != -1)
			{
...
void CPaneControlShape::_UpdateBitmap( INT32	nIndex)
{
	...
	i3GuiImage			 * pImage	= m_pTemplate->getImage(nIndex);
	i3Texture			 * pTex		= pImage->m_pTexture;

	i3GuiToolRectElement * pElm		= m_ShapeRectList[nIndex];
...
void CPaneControlShape::_OnDrawTemplate(HDC hdc)
{
	...
	for(i = 0; i < nShapeCount; ++i)
	{
		...
		I3ASSERT( i3::generic_is_iequal( pResTexture->GetName(), pTex->GetName()) );
...
void CPaneControlShape::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if( m_nCurShape < nShapeCount )
	{
		i3GuiImage * pImage = m_pTemplate->getImage( m_nCurShape );
		if( pImage != NULL )
		{
		...
		if( GetKeyState( VK_LCONTROL) & 0x80)
		{
			...
			if( idx == -1)
			{
				m_listSelected.push_back( pImage);
			}
			else
			{
				i3::vu::remove(m_listSelected, pImage);
			}
		}
		else
		{
...
{
	i3GuiToolRectElement * pElm = m_ShapeRectList[m_nCurShape];
	RECT* pRt = pElm->getRect();
...
{
	if( nShapeIndex >= (INT32)m_ShapeRectList.size())
		return;
...
void CPaneControlShape::_MoveShape(CPoint delta, INT32 nShapeIndex, bool bUpdate)
{
	...
	i3GuiToolRectElement * pElm = m_ShapeRectList[nShapeIndex];
	RECT* pRt = pElm->getRect();

	REAL32 moveX = ((REAL32)delta.x / m_nScale);
	REAL32 moveY = ((REAL32)delta.y / m_nScale);

	i3GuiImage * pImage = m_pTemplate->getImage( nShapeIndex );
	if( pImage != NULL )
	{
		pImage->m_nX += moveX;
		pImage->m_nY += moveY;
...
void CPaneControlShape::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if( m_bLBDragging )
	{
		if(!( GetKeyState( VK_LCONTROL) & 0x80))
		{
			...
			//1개만 선택인경우
			if(( m_nCurShape >= 0) && (m_nCurShape < (INT32)m_ShapeRectList.size()) && (m_listSelected.size() <= 1))
			{
				i3GuiToolRectElement * pElm = m_ShapeRectList[m_nCurShape];
				RECT* pRt = pElm->getRect();
				rt = *pRt;
...
void CPaneControlShape::OnRButtonDown(UINT nFlags, CPoint point)
{
	...
	if( m_listSelected.size() <= 1 && ((m_nCurShape < nCount) && (m_nCurShape >= 0)))
	{
...
void CPaneControlShape::_CopyTextureUVtoAllOthers( INT32 OriginShape)
{
	...
	i3GuiImage * pOrigin = (i3GuiImage*)m_pTemplate->getImage( m_nCurShape);
	if( pOrigin == NULL)
		return;
...
void CPaneControlShape::_InitUV(INT32	nShapeIndex, i3GuiImage * pImage)
{
	if( pImage == NULL )
		return ;
	...
	if( pTex != NULL )
	{
		for( INT32 i = 0; i < m_pTemplate->getShapeCount(); ++i)
		{
			i3GuiImage * pOtherImage = m_pTemplate->getImage( i);
...
void CPaneControlShape::_AdjustSizebyUV( INT32	nShapeIndex)
{
	...
	if( nShapeIndex == -1)
	{
		if( m_listSelected.size() >= 2)
		{
			...
			{
				i3GuiImage * pImage = m_pTemplate->getImage( i);
				if( i3::vu::int_index_of(m_listSelected, pImage) != -1)
				{
					if( pImage->m_pTexture != NULL )
					{
...
void CPaneControlShape::_CopySizetoOthers( INT32 nSrcIndex, INT32 nShapeIndex)
{
	...
	i3GuiImage * pSrc = m_pTemplate->getImage( nSrcIndex);
	if( pSrc == NULL)
		return;
		...
		case VK_TAB:
			{
				...
				i3GuiImage * pImage = m_pTemplate->getImage( curshape);
				if( pImage == NULL)
					break;
...
void	CPaneControlShape::_SpreadShapes( INT32 nCol, REAL32 rSeperateX, REAL32 rSeperateY, INT32 nOrigin, bool bAll, bool byValue)
{
	...
	for( INT32 i = 0;i  < m_pTemplate->getShapeCount(); ++i)
	{
		i3GuiImage * pDstImage = m_pTemplate->getImage( i);

		if( !bAll)
		{
			INT32 idx = i3::vu::int_index_of(m_listSelected, pDstImage);
			if( idx == -1)
				continue;
		}
