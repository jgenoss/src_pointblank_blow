...
void CSelectTemplateDialog::_OnDrawTemplate(HDC hdc)
{
	...
	{
		i3GuiImage * pImage = m_pSelectedTemplate->getImage(i);
...
void CSelectTemplateDialog::_SetTemplateList()
{
	...
	i3::vector<i3GuiTemplateElement*>& TempList = m_pTemplate->getTemplateList();
	if ( !TempList.empty() )
	{
		const size_t	nCount = TempList.size();
		
		for( size_t i = 0;i < nCount; ++i)
		{
			i3GuiTemplateElement * pElement = TempList[i];
			if( pElement != NULL )
			{
				_AddItem( pElement);
			}
...
void	CSelectTemplateDialog::_AddItem( i3GuiTemplateElement * pElement )
{
	char szName[1024];
	INT32	ItemIndex;
...
BOOL CSelectTemplateDialog::_IsValidTemplateElement( i3GuiTemplateElement * pElement)
{
	if( pElement == NULL )
	{
...
void CSelectTemplateDialog::OnNMClickListSeltemplate(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	if( pos != NULL )
	{
		...
		i3GuiTemplateElement * pElement = (i3GuiTemplateElement*)m_CtrlTemplateListView.GetItemData( idx );
