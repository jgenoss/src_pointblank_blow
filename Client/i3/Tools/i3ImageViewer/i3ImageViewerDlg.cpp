...
void Ci3ImageViewerDlg::OnFileSave32823()
{
	if( m_szFileName[0] == 0)
	{
		OnFileSaveas();
...
void Ci3ImageViewerDlg::OnFileSaveas()
{
	...
	CFileDialog	Dlg( FALSE, NULL, m_szFileName, OFN_ENABLESIZING | OFN_EXPLORER | OFN_FILEMUSTEXIST | OFN_LONGNAMES | OFN_PATHMUSTEXIST,  
		"All Supported Texture file format(*.I3I;*.TGA)|*.I3I;*.TGA||", this);
	...
	//chunjong.song 13.9.11
	//Dlg에서 선택된 경로에 저장되게 수정
	//FileName = Dlg.GetFileName();
	FileName = Dlg.GetPathName();
	SaveImageFile( LPCTSTR( FileName));
	OpenImageFile(m_szFileName);
}
