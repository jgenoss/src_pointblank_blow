...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWndEx)
	...
	ON_UPDATE_COMMAND_UI(IDC_BT_LOAD_PEF, &CMainFrame::OnUpdateButton)
...
static bool g_ThreadState = true;

unsigned WINAPI ThreadSave(void* arg)
{
	while(g_ThreadState)
	{
		if( CGlobalInstance::getSingletonPtr() )
		{
			CGlobalInstance::getSingletonPtr()->Save();
		}
		else
		{
			break;
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	CMFCRibbonStatusBarPane* pStatuPane = new CMFCRibbonStatusBarPane(ID_STATUSBAR_PANE1, L"준비", TRUE);

	m_wndStatusBar.AddElement(pStatuPane, L"준비");
	m_wndStatusBar.AddExtendedElement(new CMFCRibbonProgressBar(ID_STATUSBAR_PANE2), L"prograsbar");
...
// 	DockPane(&m_wndProperties);
	...
	lstBasicCommands.AddTail(ID_MENU_PEFPATH);
	...
	// thread를 생성.
	m_hThread = (HANDLE)_beginthreadex(NULL, 0, ThreadSave, NULL, 0, (unsigned*)&m_dwThreadID);
	if(m_hThread == 0)
	{
...
// 	}
	...
	CGlobalInstance::getSingleton().Create(this);
	//DockPane( &m_Weapon);
