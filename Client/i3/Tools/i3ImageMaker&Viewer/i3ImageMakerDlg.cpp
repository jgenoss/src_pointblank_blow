...
void NormalizeCmdArg_( char *szOut, const char *szIn )
{
	while( *szIn )
	{
		if( *szIn != '"' )
...
class CAboutDlg_ : public CDialog
{
public:
	CAboutDlg_();
...
CAboutDlg_::CAboutDlg_() : CDialog(CAboutDlg_::IDD)
{
}

void CAboutDlg_::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
}

BEGIN_MESSAGE_MAP(CAboutDlg_, CDialog)
END_MESSAGE_MAP()
...
BEGIN_MESSAGE_MAP(Ci3ImageMakerDlg, CDialog)
	...
	ON_BN_CLICKED(IDOK, &Ci3ImageMakerDlg::OnBnClickedOk)
END_MESSAGE_MAP()
...
// 		m_pCtx->Create( &fmt, m_hWnd);
	...
	if( m_szCmdLineArg != NULL )
	{
		if( m_szCmdLineArg[0] != 0 )
		{
			...
			NormalizeCmdArg_( szFileName, m_szCmdLineArg );
...
//	szOutFileName = i3String::InsertStr( szOutPath, ".i3i", strlen( szOutPath)  , 4);
	...
	i3::generic_string_ncat( szOutPath, ".i3i", 4);
...
void Ci3ImageMakerDlg::OnBnClickedOk()
{
	UpdateControls();
	...
	Ci3ImageViewerDlg *pDlg = (Ci3ImageViewerDlg *)GetParent();

	int count = DragQueryFile(pDlg->m_hDropInfo, 0xFFFFFFFF, NULL, 0);
	int nSuccess = 0;

	if ( i3::generic_empty(pDlg->m_strFileName) == false)
	{
		strcpy( szTemp, pDlg->m_strFileName);
		count = 1;
	}

	if (count == 0)
	{
		MessageBox("Can not find image file, Please load image file");
		OnOK();
		return;
	}
	...
	for(int i=0; i<count; i++)
	{
		DragQueryFile( pDlg->m_hDropInfo, i, szTemp, sizeof(szTemp) - 1);
	...
	msg.Format( "The Conversion Was Successful / Total : %d/%d", nSuccess, count );
