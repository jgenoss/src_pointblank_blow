...
#include "i3Base/itl/range/generic_nreplace_range.h"
...
{
	SendEditor( SCI_SETLEXER, SCLEX_LUA );						// Lua lexer
	...
	/*SendEditor( SCI_SETMARGINTYPEN, 50, SC_MARGIN_NUMBER);
	SendEditor( SCI_STYLESETVISIBLE, STYLE_LINENUMBER, true);
	SetAStyle( STYLE_LINENUMBER, black, white, 10, "Courier New");*/
...
void CScriptWnd::Find( void)
{
	...
	if( dlg.DoModal() == IDOK)
	{
		// Settings
		if( dlg.IsMatchCase())
			m_SearchFlags |= SCFIND_MATCHCASE;
		else
			m_SearchFlags &= ~SCFIND_MATCHCASE;
		
		if( dlg.IsMatchWholeWord())
			m_SearchFlags |= SCFIND_WHOLEWORD;
		else
			m_SearchFlags &= ~SCFIND_WHOLEWORD;

		i3::string_ncopy_nullpad( m_SearchText, dlg.GetFindWhat(), sizeof( m_SearchText));

		// Search
		if( dlg.IsSearchUp())
		{
			m_SearchDirection = SCI_SEARCHPREV;
			m_bFindForward = true;
		}
		else
		{
...
void CScriptWnd::OnSetFocus(CWnd* pOldWnd)
{
	...
	//FileWatcher를 통해 ScreenScript 또는 LibraryScript의 modification이 감지되었다면
	//열려 있는 다른 ScriptWnd들에도 알린다. 
	CMainFrame* pMainFrm = (CMainFrame*)AfxGetMainWnd();
	
	bool bScreen = true;
	for( INT32 i=0; i< 2; i++)
	{
		if( pMainFrm->IsScriptChanged( bScreen))
		{
			pMainFrm->SetScriptChanged( bScreen, false);
			pMainFrm->OnFileChanged( bScreen);
		}

		bScreen = false;
	}
		
	//파일을 reload해야하는 경우 MessageBox를 출력하여 물어보지 않고 무조건 reload한다.
	if( m_bReload)
	{
		m_bReload = false;

		//INT32 nRes = AfxMessageBox( "This script has been modified outside of this pane.\nDo you want to reload it?", MB_YESNO);
		//I3TRACE("This script has been modified outside of this pane.\nDo you want to reload it?");
		...
		//if( nRes == IDYES)
		{
			...
			//::SetFocus( this->m_hWnd);
			//SendEditor( SCI_CLEARSELECTIONS);
			//SendEditor( SCI_SETSELECTIONSTART, 10);
			//SendEditor( SCI_SETSELECTIONEND, 20);
		}
	}
}
