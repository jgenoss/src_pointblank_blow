...
void CUIGripCtrl::OnDraw(HDC hdc)
{

}

void CUIGripCtrl::setPos( POINT pos)
{
	I3ASSERT( m_pHandler != NULL);

	m_pHandler->setPos( (REAL32) pos.x, (REAL32) pos.y);
...
INT32 CUIGripCtrl::getPosX(void)
{
	I3ASSERT( m_pHandler != NULL);

	return (INT32) m_pHandler->getLeft();
}

INT32 CUIGripCtrl::getPosY(void)
{
	I3ASSERT( m_pHandler != NULL);

	return (INT32) m_pHandler->getTop();
}

INT32 CUIGripCtrl::getWidth(void)
{
	I3ASSERT( m_pHandler != NULL);

	return (INT32)(m_pHandler->getRight() - m_pHandler->getLeft());
}

INT32 CUIGripCtrl::getHeight(void)
{
	I3ASSERT( m_pHandler != NULL);

	return (INT32)(m_pHandler->getBottom() - m_pHandler->getTop());
}
