...
bool CUIShapeGripManager::isSelected( void * p)
{
	
	for( size_t i = 0;i < m_SelectedList.size(); ++i)
	{
		CUIGripCtrlRect * pRect = (CUIGripCtrlRect*)m_SelectedList[i];
		if( pRect->getObject() == p)
		{
			return true;
		}
	}
...
INT32 CUIShapeGripManager::AddSelect( void * pElement, i3ClassMeta * pMeta, bool bReverse)
{
	...
	{
		CUIGripCtrlRect* pCtrlRect = (CUIGripCtrlRect*)m_SelectedList[m_SelectedList.size()-1];
...
void CUIShapeGripManager::Draw( HDC hdc)
{
	...
	for(size_t i = 0;i < m_SelectedList.size(); ++i)
	{
		...
		CPoint ptPrev;
		ptPrev.SetPoint(pRect->getPosX(), pRect->getPosY());

		CPoint ptCurr;
		ptCurr.x = pRect->getPosX() + m_ptDelta.x;
		ptCurr.y = pRect->getPosY() + m_ptDelta.y;
