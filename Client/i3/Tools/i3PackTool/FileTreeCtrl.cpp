...
{
	if(g_pOption->m_szScriptPath[0] == 0)
		SHGetDesktopFolder(&CFileTreeCtrl::m_lpsfDesktop);
	else
		CShell::PathToShellFolder(g_pOption->m_szScriptPath, CFileTreeCtrl::m_lpsfDesktop);
...
BOOL CFileTreeCtrl::SetScriptRefreshFolder()
{
...
//	DWORD dwStyle = GetStyle();   // read the windowstyle
	...
	//child 데이터 목록 받기위해 한번 초기화 합니다.
	ExpandAllRefresh(m_hDesktop);
...
BOOL CFileTreeCtrl::DisplayTree()
{
	...
	m_imgState.Create( IDB_STATE, 16, 1, RGB(255,255,255) );
...
bool CFileTreeCtrl::ExpandAllRefresh(HTREEITEM hSelectItem)
{
	if(g_pOption->m_szScriptPath[0] == 0)
		return false;

	HTREEITEM pchild, pchild2= NULL;
	pchild = hSelectItem;
	while(pchild != NULL)
	{
		Expand(pchild,TVE_EXPAND);
		pchild2 = GetNextItem(pchild, TVGN_CHILD);
		if(pchild2 != NULL){
			ExpandAllRefresh(pchild2);
		}
		Expand(pchild,TVE_COLLAPSE);
		pchild = GetNextItem(pchild, TVGN_NEXT);
	}
...
//    LPSHELLFOLDER   lpsf2=NULL;
    ...
    LPITEMIDLIST    lpi=NULL, lpifqThisItem=NULL;			// lpiTemp=NULL,
...
{
	CString strName;
	strName = file.Right( file.GetLength() - file.ReverseFind('\\')-1);
	strName.MakeLower();
	m_strMatchFile.MakeLower();

	if(m_bSelScript == true)
	{
		CString strTemp = file.Right(13);
		if(m_bFindFile == true)
		{
			if (!strTemp.CollateNoCase(".i3PackScript") && -1 != strName.Find(m_strMatchFile.GetBuffer()) ) 
			{
				return TRUE;
			}
				return FALSE;
		...
		{
			if (!strTemp.CollateNoCase(".i3PackScript")) return TRUE;
			return FALSE;
		}
	...
	else
	{
		...
		if(m_bFindFile == true)
		{
			if (!strTemp.CollateNoCase(".bat") && -1 != strName.Find(m_strMatchFile.GetBuffer()) ) 
			{
				return TRUE;
			}
			return FALSE;
		...
		{
			if (!strTemp.CollateNoCase(".bat")) return TRUE;
			return FALSE;
		}
	}
			...
			while (hChild)
			{
				...
				if (newState == REFRESH) 
				{
					if (fChildInfo->IsFolder) 
					{
						...
						if (fInfo) 
						{
							int ChildState2 = GetItemState(hChild, &fChildInfo);
							if (ChildState2 != DISABLED)
							{
								fInfo->nNumDirs++;
								if (SelectedCompletely(ChildState2, fChildInfo)) fInfo->nSelDirs++;
								else if (ChildState2 != UNCHECKED) fInfo->nPartSelDirs++;
							}
						}
					}
...
BOOL CFileTreeCtrl::CheckItem(HTREEITEM hItem)
{
	...
	pFrame->m_PanelTreeScript.OnGetlist();
...
BOOL CFileTreeCtrl::ToggleForward (HTREEITEM hItem)
{
	...
	if (nState == FILE)
	{
		bool ischecked = UncheckItem(hItem) != FALSE;
		pFrame->m_PanelTreeScript.OnGetlist();
		return ischecked;
	}
...
//	UINT newState;
	...
	UINT nState = GetItemState(hItem, &lptvid);		nState;		// 경고제거..

	if (lptvid->IsFolder == false)
	{
		bool istest = false;
		SHELLEXECUTEINFO si;
		...
		si.lpFile = __TEXT("notepad");           //실행파일
		si.lpParameters = lptvid->FullName->GetBuffer();						//실행파일 인자값.
...
CString CFileTreeCtrl::GetInfo(HTREEITEM hItem)
{
	...
	if (hItem)
	{
		...
		if (lptvid) res.Format("Files: %d;   Dirs: %d;   SelFiles: %d;   SelDirs: %d;   PartDirs: %d",
