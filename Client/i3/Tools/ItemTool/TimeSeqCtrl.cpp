...
IMPLEMENT_DYNAMIC(CTimeSeqCtrl, cMyTDKTimeSeqCtrl)
...
BEGIN_MESSAGE_MAP(CTimeSeqCtrl, cMyTDKTimeSeqCtrl)
END_MESSAGE_MAP()
...
{
 	i3SceneObject* pSceneObject		= g_pChara->geti3Chara()->getSceneObject();
 	i3AnimationContext* pAniContext = pSceneObject->GetBody()->getAnimationContext();
 	for(int i = 0; i < pAniContext->getCombinerCount(); i++)
 	{
 		i3TransformSourceCombiner * pCombiner = pAniContext->getCombiner( i);
 
 		pCombiner->SetPlayCount( 0);
 	}
...
void CTimeSeqCtrl::OnLoadI3S( POINT * pPoint)
{
	...
	{
		I3WARN( "%s 파일을 열 수 없습니다.", LPCTSTR( Dlg.GetPathName()));
		return;
	}
...
void CTimeSeqCtrl::OnLoadSound( POINT * pPoint)
{
	...
	::SendMessage(g_HwndDlgTimeTrack, WM_TDK_UPDATE, (WPARAM) &Data, (LPARAM)NULL);
...
{
	cMyTDKTimeSeqCtrl::OnSelect( idx);
