...
namespace pack
{
	struct GIC_SHAPE_AI
	{
		UINT8			m_ID[4] = { 'G', 'A', 'I', '1' };
		OBJREF64		m_pAIState = 0;
		UINT32			m_SrcTextSize = 0;
		UINT32			m_CommentTextSize = 0;
		UINT32			pad[30] = { 0 };
	};
}
...
bool GICShapeAI::OnSaveXML(i3XMLFile * pFile, i3XMLElement * pXML)
{
	bool bRv = i3GICShapeRect::OnSaveXML(pFile, pXML);
	if (bRv == false)
		return false;

	int nSrcTextSize = GetStringListBufferSize(m_SrcText);
	int nCommentTextSize = GetStringListBufferSize(m_CommentText);

	pXML->addAttr("AIStateID", (OBJREF64)pFile->GetObjectPersistID(m_pAIState));
	pXML->addAttr("SrcTextSize", nSrcTextSize);
	pXML->addAttr("CommentTextSize", nCommentTextSize);

	// Src Text
	if (nSrcTextSize > 0)
	{
		char * pBuf = new char[nSrcTextSize + 1];
		I3ASSERT(pBuf != NULL);

		CopyFromStringListToBuffer(m_SrcText, pBuf, nSrcTextSize + 1);

		pXML->addAttr("SrcText", pBuf);

		delete pBuf;
	}

	// Comment Text
	if (nCommentTextSize > 0)
	{
		char * pBuf = new char[nCommentTextSize + 1];
		I3ASSERT(pBuf != NULL);

		CopyFromStringListToBuffer(m_CommentText, pBuf, nCommentTextSize + 1);

		pXML->addAttr("CommentText", pBuf);
...
bool GICShapeAI::OnLoadXML(i3XMLFile * pFile, i3XMLElement * pXML)
{
	bool bRv = i3GICShapeRect::OnLoadXML(pFile, pXML);
	if (bRv == false)
		return false;

	OBJREF64 temp;
	pXML->getAttr("AIStateID", &temp);
	if (temp != 0)
	{
	...
	// Src Text
	int nSrcTextSize;
	pXML->getAttr("SrcTextSize", &nSrcTextSize);
	if (nSrcTextSize > 0)
	{
		i3::rc_string str = pXML->getAttr("SrcText");

		CopyFromBufferToStringList(m_SrcText, str.c_str());
	}

	// Comment Text
	int nCommenTextSize;
	pXML->getAttr("SrcTextSize", &nCommenTextSize);
	if (nCommenTextSize > 0)
	{
		i3::rc_string str = pXML->getAttr("CommentText");

		CopyFromBufferToStringList(m_CommentText, str.c_str());
	}
