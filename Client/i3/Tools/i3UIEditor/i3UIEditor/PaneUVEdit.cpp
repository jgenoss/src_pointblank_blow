...
BOOL CPaneUVEdit::OnInitDialog()
{
	...
	i3TDK::RegisterUpdate( m_hWnd, i3Texture::GetClassMeta(), I3_TDK_UPDATE_SELECT );
	i3TDK::RegisterUpdate( m_hWnd, i3UIImage::GetClassMeta(), I3_TDK_UPDATE_SET );
...
void CPaneUVEdit::OnUpdate( I3_TDK_UPDATE_INFO * pInfo )
{
	...
	if( pInfo->m_Event & I3_TDK_UPDATE_SELECT )
	{
		if( pInfo->m_Event & I3_TDK_USRWND_RESLIST)
		{
			...
			{
				if( pInfo->m_pObject->IsTypeOf( i3Texture::GetClassMeta()))
				{
	...
	else if( pInfo->m_Event & I3_TDK_UPDATE_SET)
	{
		if( pInfo->m_Event & I3_TDK_USRWND_CTRLSHAPE)
		{
			if( pInfo->m_pObject != NULL)
			{
				if( pInfo->m_pObject->IsTypeOf( i3UIImage::GetClassMeta()))
				{
					i3UIImage * pImage = (i3UIImage*)pInfo->m_pObject;
