...
void CEditDlgAction_ScaleTo::SetScaleEventData( I3UI_EVT_SCALE_DATA * pData)
{
	m_pData = pData;
}
...
void CEditDlgAction_ScaleTo::OnOK()
{
	...
	UpdateData( TRUE);
	{
		...
		if( m_edDuration.GetModify())
		{
			...
			m_pData->m_fDestScaleX = (RT_REAL32)atof( conv);
		}
		...
		if( m_edDuration.GetModify())
		{
			...
			m_pData->m_fDestScaleY = (RT_REAL32)atof( conv);
		}
