...
bool i3Viewer2Framework::LoadI3S( const char * pszPath)
{
	...
	MakeNMByManual( m_pSgInfo->getInstanceSg() );
#endif

#ifdef CONVERT_BY_AUTO
	MakeNMByAuto( m_pSgInfo->getInstanceSg() );
	
	
	i3ResourceFile fileB;
	fileB.Load( pszPath );

	int a = fileB.GetObjectCount();
	int iMemoryBuffer = 0;

	char fileName[256];
	i3String::SplitFileName( pszPath, fileName, false );
	char buf[256];
	i3String::Format( buf, 256, "D:\\Project\\PointBlank\\Tools\\AStarAlgorism\\AStarAlgorism\\%s.nm", fileName );
	...
	for( int i = 0; i < a; i++ )
	{
		i3PersistantElement* e = fileB.GetObject( i );	
		
		if ( e->GetMeta() == i3MemoryBuffer::GetClassMeta() )
		{
			// 1번째 메모리 버퍼 데이터는 무시하고 2번째 메모리 버퍼 데이터만 사용합니다.
			if ( iMemoryBuffer == 1 )
			{
				i3MemoryBuffer* obj = (i3MemoryBuffer*)e;
				nFace = obj->GetSize() / 16;
				pvec = (UINT*)obj->getBuffer(); // 16 * nFace 만큼의 버퍼
			}

			iMemoryBuffer++;
			if ( iMemoryBuffer == 2 ) iMemoryBuffer = 0;
		}
		else if ( e->GetMeta() == i3Vector3Array::GetClassMeta() )
		{
			i3Vector3Array* obj = (i3Vector3Array*)e;
			unsigned vc = obj->GetVectorCount();
			VEC3D *db = new VEC3D[vc];
			for( unsigned k = 0; k < vc; k++ )
			{
				D3DXVec3TransformCoord( (D3DXVECTOR3*)&db[k], (D3DXVECTOR3*)obj->GetVector(k), (D3DXMATRIX*)&g_matrix[imat] );
			}

			INT32 i32FaceSize = nFace*4;
			for( int i = 0; i < i32FaceSize; i += 4 )
			{
				fwrite( &db[pvec[i+0]], sizeof(VEC3D), 1, fp );
				fwrite( &db[pvec[i+1]], sizeof(VEC3D), 1, fp );
				fwrite( &db[pvec[i+2]], sizeof(VEC3D), 1, fp );
			}
			delete[] db;
	...
	// Transform2 데이터의 갯수와 i3memorybuffer+i3vertex3list 데이터의 갯수가 다른 경우
	if ( imat != g_imatrix )
	{
		// 어떻게 처리해야 할지 미정
		int y;
...
bool i3Viewer2Framework::ApplyOptionReflectMap(void)
{
	...
	{
		i3String::Copy( szFull, g_pOption->m_szReflectMap);
	}
...
#ifdef CONVERT_BY_AUTO
static I3SG_TRAVERSAL_RESULT g_GetMarix( i3Node* pNode, void * pUserData, i3MatrixStack* pStack)
{
	if ( pNode->IsTypeOf( i3Transform2::GetClassMeta()) )
	{
		i3mem::Copy( &g_matrix[g_imatrix], pStack->GetTop(), sizeof(MATRIX) );
		g_imatrix++;
	}
...
void i3Viewer2Framework::MakeNMByAuto( i3Node* pRoot)
{
	i3MatrixStack stack;
	stack.Reset();

	g_imatrix = 0;
	i3Scene::Traverse( m_pSgInfo->getInstanceSg(), (I3_SG_TRAVERSE_PROC)g_GetMarix, NULL, &stack);
}
#endif

#ifdef CONVERT_BY_MANUAL
static I3SG_TRAVERSAL_RESULT g_WriteFile( i3Node* pNode, void * pUserData, i3MatrixStack* pStack)
{
	if( pNode->IsExactTypeOf( i3Geometry::GetClassMeta()) )
	{
		...
		if ( sizeof( MATRIX ) != sizeof( D3DXMATRIX ) ) exit(0);
		if ( sizeof( VEC3D ) != sizeof( D3DXVECTOR3 ) ) exit(0);

		i3mem::Copy( &mat, pStack->GetTop(), sizeof( D3DXMATRIX ) );

		i3Geometry* pGeo = (i3Geometry *)pNode;
		INT32 i32Count = pGeo->GetGeometryAttrCount();

		for( INT32 i = 0; i < i32Count; i++)
		{
			i3GeometryAttr * pGeoAttr = (i3GeometryAttr *) pGeo->GetGeometryAttr( i);
			...
			i32TriCount = resolver.getTriangleCount();

			for( INT32 j = 0; j < i32TriCount; j++ )
			{
				resolver.getPositions( j, vec );

				D3DXVec3TransformCoord( (D3DXVECTOR3*)&vec[0], (D3DXVECTOR3*)&vec[0], &mat );
				D3DXVec3TransformCoord( (D3DXVECTOR3*)&vec[1], (D3DXVECTOR3*)&vec[1], &mat );
				D3DXVec3TransformCoord( (D3DXVECTOR3*)&vec[2], (D3DXVECTOR3*)&vec[2], &mat );
				
				fwrite( vec, sizeof( VEC3D ), 3, fp );
			}
		}
	}
...
void i3Viewer2Framework::MakeNMByManual( i3Node* pRoot )
{
	i3MatrixStack stack;
	stack.Reset();
	...
	FILE* fp;
	char strBuf[256];
	i3String::Format( strBuf, 256, "D:\\Project\\PointBlank\\Tools\\AStarAlgorism\\AStarAlgorism\\Test.nm" );
	fopen_s( &fp, strBuf, "wb" );
	if ( fp == NULL ) return;
	i3Scene::Traverse( m_pSgInfo->getInstanceSg(), (I3_SG_TRAVERSE_PROC)g_WriteFile, fp, &stack);
	fclose(fp); 
}
#endif
