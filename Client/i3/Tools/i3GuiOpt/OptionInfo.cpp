...
{
	#if defined( I3_WINDOWS) && !defined( I3G_OGL)
...
REAL32	COptionInfo::_GetREAL32( const char * pszEntry, REAL32 def)
{
	...
	i3::snprintf( conv, sizeof( conv), "%f", def);
...
void	COptionInfo::_SetREAL32( const char * pszEntry, REAL32 val)
{
	...
	i3::snprintf( conv, sizeof( conv), "%f", val);
...
void	COptionInfo::_GetString( const char * pszEntry, char * pszStr, INT32 len, const char * pszDef)
{
	...
	#else
	i3::string_ncopy_nullpad( pszStr, pszDef, len);
	#endif
}
...
void	COptionInfo::_GetVEC3D( const char * pszEntry, VEC3D * pVec, REAL32 x, REAL32 y, REAL32 z)
{
	...
	i3::snprintf( conv, sizeof( conv), "%f %f %f", x, y, z);
...
void	COptionInfo::_SetVEC3D( const char * pszEntry, VEC3D * pVec)
{
	...
	i3::snprintf( conv, sizeof( conv), "%f %f %f", pVec->x, pVec->y, pVec->z);
...
void	COptionInfo::_GetVEC4D( const char * pszEntry, VEC4D * pVec, REAL32 x, REAL32 y, REAL32 z, REAL32 w)
{
	...
	i3::snprintf( conv, sizeof( conv), "%f %f %f %f", x, y, z, w);
...
void	COptionInfo::_SetVEC4D( const char * pszEntry, VEC4D * pVec)
{
	...
	i3::snprintf( conv, sizeof( conv), "%f %f %f %f", pVec->x, pVec->y, pVec->z, pVec->w);
...
void COptionInfo::Load(void)
{
	...
	//m_SoftShadowType	= (I3G_SOFTSHADOW_TYPE) _GetINT32( "SoftShadowType", I3G_SOFTSHADOW_VSM);
	m_bGridShow			= _GetINT32( "Grid", TRUE);
	
	_GetString( (const char *) "LightProbe", m_szLightProbe, sizeof(m_szLightProbe), "");

	_GetVEC4D( "Ambient",		(VEC4D *) &m_Ambient,		0.2f, 0.2f, 0.2f, 1.0f);
	_GetVEC4D( "LightDiffuse",	(VEC4D *) &m_LightColor,	2.0f, 2.0f, 2.0f, 1.0f);
	...
	for( i = 0; i < MAX_MRU; i++)
	{
		...
		i3::snprintf( szEntry, sizeof( szEntry), "MRU%d", i);
	...
	for( i = 0; i < MAX_RECCAM; i++)
	{
		...
		i3::snprintf( szEntry, sizeof( szEntry), "CamPos%d", i);
		_GetVEC3D( szEntry, &m_CamPos[i], 0.0f, 0.0f, 0.0f);

		i3::snprintf( szEntry, sizeof( szEntry), "CamRot%d", i);
		_GetVEC4D( szEntry, &m_CamRot[i], 1.0f, 0.0f, 0.0f, 0.0f);
	}

	m_bRenderableAxis = _GetINT32( "RenderableAxisEnable", FALSE);
	m_bBoneView = _GetINT32( "BoneViewEnable", FALSE);
}
...
void COptionInfo::Save(void)
{
	...
	//_SetINT32( "SoftShadowType", m_SoftShadowType);
	_SetINT32( "Grid", m_bGridShow);

	_SetVEC4D( "Ambient",	(VEC4D *) &m_Ambient);
	_SetVEC4D( "LightDiffuse", (VEC4D *) &m_LightColor);
	...
	_SetString( "LightProbe", m_szLightProbe);
	...
	_SetINT32( "RenderableAxisEnable", m_bRenderableAxis);
	_SetINT32( "BoneViewEnable", m_bBoneView);
}
...
void COptionInfo::addMRU( const char * pszPath)
{
	...
	// 최상위 List에 넣는다.
	i3::string_ncopy_nullpad( m_MRU[0], pszPath, sizeof( m_MRU[0]));
}
