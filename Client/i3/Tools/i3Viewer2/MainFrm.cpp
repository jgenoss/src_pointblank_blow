...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWndEx)
	...
	ON_COMMAND(ID_TOOLS_CHANGETEX, &CMainFrame::OnToolsChangeTex)
	ON_UPDATE_COMMAND_UI(ID_TOOLS_CHANGETEX, &CMainFrame::OnUpdateToolsChangeTex)
	...
	ON_COMMAND(ID_TOOLS_BATCH_RENATTR_OPTI, &CMainFrame::OnToolsBatchRenAttrOpti)
	ON_UPDATE_COMMAND_UI(ID_TOOLS_BATCH_RENATTR_OPTI, &CMainFrame::OnUpdateToolsBatchRenAttrOpti)

	ON_COMMAND(ID_TOOLS_PHYSIXTOOL, &CMainFrame::OnToolsPhysics)

	ON_COMMAND(ID_TOOLS_TOTALBLEND, &CMainFrame::OnToolsTotalBlend)
	ON_UPDATE_COMMAND_UI(ID_TOOLS_TOTALBLEND, &CMainFrame::OnUpdateTotalBlend)

	ON_COMMAND(ID_TOOLS_TOTALCOLOR, &CMainFrame::OnToolsTotalColor)
	ON_UPDATE_COMMAND_UI(ID_TOOLS_TOTALCOLOR, &CMainFrame::OnUpdateTotalColor)
	...
	ON_CONTROL( 0,	ID_EDIT_LIGHTINTENSITY,			&CMainFrame::OnChanged_LightIntensity)
	ON_UPDATE_COMMAND_UI( ID_OPTIONS_LIGHTINTENSITY,&CMainFrame::OnUpdate_LightIntensity)

	ON_UPDATE_COMMAND_UI( ID_SLIDER_LIGHTPOSRHO,	&CMainFrame::OnUpdate_LightPosRho)
	ON_UPDATE_COMMAND_UI( ID_SLIDER_LIGHTPOSTHETA,	&CMainFrame::OnUpdate_LightPosTheta)

	ON_BN_CLICKED(ID_CHECK_LIGHTPOS,				&CMainFrame::OnClicked_LightPosVisible)
	ON_UPDATE_COMMAND_UI(ID_CHECK_LIGHTPOS,			&CMainFrame::OnUpdate_LightPosVisible)
	...
	ON_COMMAND(ID_CAMERA_FOV,						&CMainFrame::OnChange_CameraFOV)

	ON_BN_CLICKED(ID_BTN_OPT1,							&CMainFrame::OnClicked_Opt1)
	ON_BN_CLICKED(ID_BTN_OPT2,							&CMainFrame::OnClicked_Opt2)
	ON_BN_CLICKED(ID_BTN_OPT3,							&CMainFrame::OnClicked_Opt3)
	ON_BN_CLICKED(ID_BTN_OPT4,							&CMainFrame::OnClicked_Opt4)
END_MESSAGE_MAP()

// CMainFrame 생성/소멸

CMainFrame::CMainFrame() : m_dlgChangeTex(NULL), m_pPanel_Spec(NULL)
{
...
CMainFrame::~CMainFrame()
{
	if( m_pPanel_Spec )
	{
		delete m_pPanel_Spec;
		m_pPanel_Spec = NULL;
	}
}

class ChangeTexCallback : public i3TDKChangeTexCB
{
public:
	virtual bool OnSelectedi3sFilename(const i3::string& str_i3s_fullpath)
	{
		// 아마도 로딩이 이루어져야할것임	
		CMainFrame* frame = (CMainFrame*)AfxGetMainWnd();
		Ci3Viewer2Doc * pDoc = (Ci3Viewer2Doc *) frame->GetActiveDocument();
		pDoc->CloseI3S();
		return pDoc->LoadI3S( str_i3s_fullpath.c_str() );			
	}
	virtual bool OnUnselectedi3sFilename(const i3::string& str_i3s_fullpath)
	{
		CMainFrame* frame = (CMainFrame*)AfxGetMainWnd();
		Ci3Viewer2Doc * pDoc = (Ci3Viewer2Doc *) frame->GetActiveDocument();
		pDoc->CloseI3S();
		if (str_i3s_fullpath.empty() )
		{
			frame->GetTDKDlgChangeTex()->SetFilenameAndSgInfo("", NULL);
			return false;
		}
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_dlgChangeTex = new i3TDKDlgChangeTex;
	m_dlgChangeTex->Create( i3TDKDlgChangeTex::IDD);	
	m_dlgChangeTex->SetCallback( new ChangeTexCallback);
...
{
	{
		...
		if( !m_pPanel_Spec )
			m_pPanel_Spec = new Panel_Spec;

		if( m_pPanel_Spec->Create( IDD_PANEL_SPEC, &m_Dock_Spec) == FALSE)
		{
			I3TRACE( "Error\n");
		}
