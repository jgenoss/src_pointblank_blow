...
#include <lua/luabind/exception_handler.hpp>
#include <lua/luabind/make_function.hpp>
...
namespace
{
  ...
  int function::call(lua_State* L) const
  {
	  ...
	  int best_score = INT_MAX;//std::numeric_limits<int>::max();
      ...
      if (!best)
      {
          if (best_score != INT_MAX)//std::numeric_limits<int>::max())
          {
              // Ambiguous
