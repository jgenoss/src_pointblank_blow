...
I3_CLASS_INSTANCE( i3UIProgressBarSplit);//, i3UICaptionControl);

void i3UIProgressBarSplit::_UpdateShapeState( void)
{
	for( INT32 i=0; i< I3UI_PROGRESSBAR_SPLIT_SHAPE_COUNT; i++)
	...
	REAL32 fWidth = 0.0f;
	for (INT32 i = (INT32)I3UI_PROGRESSBAR_SPLIT_SHAPE_BAR_LEFT; i <= (INT32)I3UI_PROGRESSBAR_SPLIT_SHAPE_BAR_RIGHT; ++i)
	{
		i3UIShape * pShape	= getShape(i);
		fWidth += pShape->getWidth();
	}
	
	REAL32 size = fWidth - m_pRects[I3UI_PROGRESSBAR_SPLIT_SHAPE_BAR_LEFT].right 
		- m_pRects[I3UI_PROGRESSBAR_SPLIT_SHAPE_BAR_RIGHT].right;

	m_rTextUV = m_rProgress;
	m_pRects[I3UI_PROGRESSBAR_SPLIT_SHAPE_BAR_CENTER].right = size * m_rProgress;
	m_pRects[I3UI_PROGRESSBAR_SPLIT_SHAPE_BAR_RIGHT].left = 
		m_pRects[I3UI_PROGRESSBAR_SPLIT_SHAPE_BAR_CENTER].right + m_pRects[I3UI_PROGRESSBAR_SPLIT_SHAPE_BAR_CENTER].left;

	addState( I3UI_STATE_NEED_UPDATE);
}

void i3UIProgressBarSplit::UpdateRenderObjects()
{
	OnUpdateTextPos();
		...
		for(i=0; i< m_nShapeCount; i++)
		{
			...
			if( g_pUIRenderer->getEnableSprite( m_hSprites[i]))
			{
				...
				if( !m_bUseCustomTexCoord)
				{
					...
					/*if (i == (INT32)I3UI_PROGRESSBAR_SPLIT_SHAPE_BAR_CENTER)
					{
...
void i3UIProgressBarSplit::setProgress( REAL32 progress)
{
...
void i3UIProgressBarSplit::OnUpdate( REAL32 rDeltaSeconds)
{
...
void i3UIProgressBarSplit::OnEnabled( bool bEnable)
{
	i3UICaptionControl::OnEnabled( bEnable);
}

void i3UIProgressBarSplit::OnLButtonDown( UINT32 nFlag, POINT point)
{
	if( !m_bReadOnly)
	{
		m_bDragging = true;
...
void i3UIProgressBarSplit::OnLButtonUp( UINT32 nFlag, POINT point)
{
	if( !m_bReadOnly)
	{
		m_bDragging = false;
...
void i3UIProgressBarSplit::OnMouseMove( UINT32 nFlag, POINT point)
{
	if( !m_bReadOnly)
	{
		if( m_bDragging)
...
INT32 i3UIProgressBarSplit::OnCommand(LuaState * pState)
{
	char szCommand[ 260];
...
void i3UIProgressBarSplit::OnSize( REAL32 cx, REAL32 cy)
{
...
UINT32 i3UIProgressBarSplit::OnSave( i3ResourceFile * pResFile)
{
	UINT32 Rc, Result = 0;
...
UINT32 i3UIProgressBarSplit::OnLoad( i3ResourceFile * pResFile)
{
	UINT32 Rc, Result = 0;
	pack::UIPROGRESSBAR data;
...
bool i3UIProgressBarSplit::OnSaveXML( i3XMLFile * pFile, i3XMLElement * pXML)
{
	bool bResult = false;
...
bool i3UIProgressBarSplit::OnLoadXML( i3XMLFile * pFile, i3XMLElement * pXML)
{
	bool bResult = false;
...
void i3UIProgressBarSplit::CopyTo( i3ElementBase * pDest, I3_COPY_METHOD method)
{
	i3UICaptionControl::CopyTo( pDest, method );

	I3ASSERT( i3::same_of<i3UIProgressBarSplit*>(pDest));

	i3UIProgressBarSplit* pProgBar = (i3UIProgressBarSplit*)pDest;
...
#if defined( I3_DEBUG)
void i3UIProgressBarSplit::GetTDKGridProperties(i3::vector<PROPERTYINFO*>& List)
{
	i3UICaptionControl::GetTDKGridProperties( List);	

	AddMFCProperty_BeginGroup( List, "i3UIProgressBarSplit");
...
void i3UIProgressBarSplit::OnTDKPropertyChanged( const char* szName)
{
...
//////////////////////////////////////////////////////////////////////////////////
extern "C" int i3UIPSplitBar_setValue(lua_State *L)
{
	i3UIProgressBarSplit * pCtrl = (i3UIProgressBarSplit*)i3Lua::GetUserDataArg( L, 1);
	if( pCtrl != nullptr)
	{
...
extern "C" int i3UIPSplitBar_getValue(lua_State *L)
{
	i3UIProgressBarSplit * pCtrl = (i3UIProgressBarSplit*)i3Lua::GetUserDataArg( L, 1);
	if( pCtrl != nullptr)
	{
...
extern "C" int i3UIPSplitBar_setReadOnly( lua_State* L)
{
	i3UIProgressBarSplit * pCtrl = (i3UIProgressBarSplit*)i3Lua::GetUserDataArg( L, 1);
	if( pCtrl != nullptr)
	{
...
extern "C" int i3UIPSplitBar_getReadOnly( lua_State* L)
{
	i3UIProgressBarSplit* pCtrl = (i3UIProgressBarSplit*)i3Lua::GetUserDataArg( L, 1);
	if( pCtrl != nullptr)
	{
...
LuaFuncReg i3UIProgressSplitGlue[] = 
{
	{"uiPSplitBar_setValue",			i3UIPSplitBar_setValue},
	{"uiPSplitBar_getValue",			i3UIPSplitBar_getValue},	

	{"uiPSplitBar_setReadOnly",		i3UIPSplitBar_setReadOnly},
	{"uiPSplitBar_getReadOnly",		i3UIPSplitBar_getReadOnly},

	{nullptr, nullptr}
};

void i3UIProgressBarSplit::RegisterLuaFunction(LuaState * pState)
{
	if( pState == nullptr)
		return;

	for(int i=0; i3UIProgressSplitGlue[i].name; i++)
	{
		i3Lua::RegisterFunc( pState, i3UIProgressSplitGlue[i].name, i3UIProgressSplitGlue[i].func);
	}
}