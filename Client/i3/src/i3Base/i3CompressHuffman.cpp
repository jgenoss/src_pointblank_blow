...
struct TREE
{
	...
	struct TREE *prev_right_tree;
	struct TREE *prev_left_tree;
	ID_DATA tree_id;
...
static SORT_TABLE *hmInitLinkTable( TOP_TREE  *pTopTree, DATA_INFO *pDataInfo)
{
	...
	if(sort_table == nullptr) return nullptr;
...
static FLAG_TABLE *hmInitFlagTable( DATA_INFO *pDataInfo)
{
	...
	if(flag_table == nullptr) return nullptr;
...
static TOP_TREE *hmInitTopTree( DATA_INFO *pDataInfo)
{
	...
	if(top_tree == nullptr) return nullptr;

	for(loop = 0; loop < pDataInfo->num; loop++) 
	{
		top_tree[loop].tree_link = (TREE *) i3MemAlloc( sizeof(TREE));

		top_tree[loop].tree_link->next_tree       = nullptr;
		top_tree[loop].tree_link->prev_right_tree = nullptr;
		top_tree[loop].tree_link->prev_left_tree  = nullptr;
...
static TREE_TABLE *hmDestroyTree( TOP_TREE  *pTopTree, DATA_INFO *pDataInfo, UINT32     Size)
{
	...
	if(tree_table == nullptr) return nullptr;
	...
	for(loop = 0; loop < pDataInfo->num; loop++) 
	{
		...
		while(tree != nullptr)
		{
			...
			if(tree->next_tree != nullptr)
			{
				if(tree->bit) tree->next_tree->prev_left_tree  = nullptr;
				else          tree->next_tree->prev_right_tree = nullptr;
			}

			if(tree->prev_left_tree != nullptr)
			{
				tree->prev_left_tree->next_tree = nullptr;
			}

			if(tree->prev_right_tree != nullptr)
			{
				tree->prev_right_tree->next_tree = nullptr;
			}
...
static UINT8 *hmCopyDecodeData2Buf( UINT32 AccessUnit, UINT8 *pData, DECODE_TABLE *pDecodeTable)
{
	...
	if(header->m_UnitDecSize == 0) return nullptr;
...
UINT8 * i3CompressHuffman::Decode( UINT8 *pData, UINT32  AccessUnit)
{
	...
	if(header->m_AccessTblCount-1 <= AccessUnit) return nullptr;
