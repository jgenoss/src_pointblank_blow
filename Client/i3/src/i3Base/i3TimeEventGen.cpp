...
I3_CLASS_INSTANCE( i3TimeEventGen);
...
INT32 i3TimeEventGen::AddEvent( i3TimeEventInfo * pInfo)
{
	...
	i3::vector_multiset<i3TimeEventInfo*>::iterator it =	m_EventList.insert( pInfo);
...
void i3TimeEventGen::RemoveEvent( INT32 idx)
{
	...
	m_EventList.erase( m_EventList.begin() + idx);
...
{
	i3::sort( m_EventList.begin(), m_EventList.end(), i3::less<i3TimeEventInfo*>() );
...
void i3TimeEventGen::OnUpdate( REAL32 tm)
{
	...
	if( m_CurEvent < (INT32) getEventCount())
	{
		...
		while( pEvent->getStartTime() <= m_timeLocal)
		{
			bOwnProcess = false;

			// 시간이 만료된 Event들을 발생시켜준다.
			if( pEvent->getAction() != nullptr)
			{
...
void	i3TimeEventGen::CopyTo( i3ElementBase * pDest, I3_COPY_METHOD method)
{
	...
	I3ASSERT( i3::same_of<i3TimeEventGen*>(pDest) );
...
UINT32 i3TimeEventGen::OnSave( i3ResourceFile * pResFile)
{
	...
	rc = SaveFromListToResourceFile(m_EventList.get_vector(), pResFile);		 // m_EventList.SaveTo( pResFile);
	I3_CHKIO( rc);
	result += rc;
...
UINT32 i3TimeEventGen::OnLoad( i3ResourceFile * pResFile)
{
	...
	{
		i3::vector<i3TimeEventInfo*> v;
		rc = LoadFromResourceFileToList(v, pResFile, true);

		m_EventList.clear();
		m_EventList.insert(v.begin(), v.end());
	}
