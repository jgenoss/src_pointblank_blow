...
I3_CLASS_INSTANCE( i3Udp);
I3_CLASS_INSTANCE( i3Tcp);
...
{
#if defined( I3_WINDOWS )// || defined( I3_WINCE )
	struct	hostent *pHosten = nullptr;
	char	szHostName[30];
...
char * i3Net::GetMyAddressList( INT32 * pOutCount)
{
	...
	if( SOCKET_ERROR != gethostname(szHostName,30) )
	{
		...
		if( pHosten)
		{
			...
			while( pHosten->h_addr_list[ count] != nullptr)
				count++;

			char * pAddress = (char*) i3MemAlloc( sizeof(char) * 4 * count);
			char * pTemp = pAddress;

			while( pHosten->h_addr_list[ i] != nullptr)
			{	
...
void i3Net::GetName( INT32 Socket, SOCKADDR_IN * plocaladdr )
{
	...
	int err = ::getsockname( Socket, &addr, &len );
	if( err == SOCKET_ERROR)
	{
		I3PRINTLOG(I3LOG_WARN,  "getsockname err1!");
		return;
	}
...
INT32 i3Udp::CreateSocket(void)
{
	...
	if( m_Socket != INVALID_SOCKET)
	{
		...
		int err = ::setsockopt( m_Socket, IPPROTO_UDP, SO_RCVBUF, (const char *) &bufsz, sizeof(int));
		if( err == SOCKET_ERROR)
		{
		...
		err = ::setsockopt( m_Socket, IPPROTO_UDP, SO_SNDBUF, (const char *) &bufsz, sizeof(int));
		if( err == SOCKET_ERROR)
		{
			I3PRINTLOG(I3LOG_WARN,  "sockopt err2!!");
			::closesocket( m_Socket);
			return 0;
		}
	}
...
i3Tcp::i3Tcp()
{
	INT32 currSock = CreateSocket(); 
	I3ASSERT(currSock != INVALID_SOCKET);
}
