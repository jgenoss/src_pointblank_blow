...
#include "string/ext/integral_to_comma_str.h"
...
static I3MEMBLOCK		*	s_pAllocBlockHead	= nullptr;
static I3MEMBLOCK		*	s_pFreeBlockHead	= nullptr;
static LPI3MEM				s_pBuffer			= nullptr;
static LPI3MEM				s_pPointer[2]		= { nullptr, nullptr };
static LPI3MEM				s_pMovePointer[2]	= { nullptr, nullptr };
...
namespace i3mem
{
	I3_EXPORT_BASE void *	(*Alloc)(INT32 nBytes,	INT32 State, INT32 Align, const char * FileName ,INT32 Line) = nullptr;
	I3_EXPORT_BASE void *	(*Realloc)(void * pPointer,	INT32 nBytes,INT32 Align, const char * FileName, INT32 Line) = nullptr;
	I3_EXPORT_BASE void		(*Free)(const void * p, const char * pszFile, INT32 line ) = nullptr;
	I3_EXPORT_BASE void		(*Compact)(void) = nullptr;
};
...
{
	//Set Alloc Block
	if(s_pAllocBlockHead == nullptr)
	{			
		pBlock->m_Next	= nullptr;
		pBlock->m_Prev	= nullptr;
	}else
	{		
...
inline void PopAllocList(I3MEMBLOCK * pBlock)
{
	...
	if(s_pAllocBlockHead == pBlock)
	{
		...
		if( s_pAllocBlockHead != nullptr)
			s_pAllocBlockHead->m_Next = nullptr;
	}else
	{		
		if(pBlock->m_Prev != nullptr)
		{
			pTempBlock			= pBlock->m_Prev; 
			pTempBlock->m_Next	= pBlock->m_Next; 
		}

		if(pBlock->m_Next != nullptr)
		{
...
//----------------------------------------------------------------------------*/
	...
	static FILE_MAPPING_HEADER	* s_pMappingHeader	= nullptr;
	static MEM_NET_ALLOC_INFO	* s_pNetAllocInfo	= nullptr;
	static HANDLE				s_hFile;

	void _NetworkDump_Malloc_Disable(void* , std::size_t , const char* , int ){}
	void  _NetworkDump_Free_Disable(void*, const char *, int){}

	void (*NetworkDump_Malloc)(void* , std::size_t , const char* , int ) = _NetworkDump_Malloc_Disable;
	void (*NetworkDump_Free)(void*, const char *, int)   = _NetworkDump_Free_Disable;

	//static i3Mutex				s_DumpCs; 	

	void DumpWriteFileBuffer(MEM_NET_ALLOC_INFO * pAllocInfo )
	{
		if( s_pMappingHeader == nullptr) return;
//		s_DumpCs.Enter(); 
		{	            
...
//		s_DumpCs.Leave(); 
	...
	//
	// DumpWriteFileBuffer는 크리티컬섹션이 없기 때문에, 바깥에 걸어야될 필요가 있음..
	// MemAlloc과 MemFree에서 s_MemoryCS를 쓰기 때문에, s_MemoryCS를 써야한다..
	//
	// 메모리할당에서 크리티컬 섹션은 빈번할때 속도저하의 요인이 되는데,
	// 릴리즈모드에서만큼은 메모리할당의 크리티컬섹션을 모두 제거하는게 가능한 루틴이어야했다..
	//

	void 	_NetworkDump_Malloc(void* data, std::size_t size, const char* file, int line)
	{
		MEM_NET_ALLOC_INFO SendBuffer; 
		I3ASSERT( size > 0);
		SendBuffer._Command	= MEMDUMP_CMD_MALLOC;
		SendBuffer._Size	= (UINT32)size;
		SendBuffer._Address = (UINT32)data;
		
		char szTemp[101];
		i3::generic_string_ncopy( szTemp, file, 100);
		szTemp[100] = 0;
		::sprintf( SendBuffer._String, "%s - %d", szTemp, line);
	...
	void	_NetworkDump_Free(void* data, const char * file, int line)
	{
		if( data != nullptr)
		{
			MEM_NET_ALLOC_INFO SendBuffer; 
			SendBuffer._Command	= MEMDUMP_CMD_FREE;
			SendBuffer._Size	= 0;
			SendBuffer._Address = (UINT32) data;
			::sprintf( SendBuffer._String, "%s - %d", file, line);
...
void i3mem::SetDumpEnable(bool bEnable)
{
	...
	if (bEnable)
	{
		NetworkDump_Malloc = _NetworkDump_Malloc;
		NetworkDump_Free   = _NetworkDump_Free;
	}
	else
	{
		NetworkDump_Malloc = _NetworkDump_Malloc_Disable;
		NetworkDump_Free   = _NetworkDump_Free_Disable;
	}
#endif
}
...
//#endif
...
#endif
		//Set Free Block 
		s_pAllocBlockHead	= nullptr;
		s_pFreeBlockHead	= nullptr;

		//Make Free Block 
		s_pFreeBlockHead	= (I3MEMBLOCK *)s_pBuffer; 
		s_pFreeBlockHead->m_pPointer	= nullptr;
	...
	else
	{
		...
		s_pPointer[0]		= s_pMovePointer[0] = nullptr;
		s_pPointer[1]		= s_pMovePointer[1] = nullptr;
		s_Size				= 0;
#if defined( I3_DEBUG)
		s_AllocSize			= 0;		
		s_AllocCount		= 0; 
...
void i3mem::Destroy(void)
{
	...
	s_Size				= -1;
	s_pBuffer			= nullptr;
	s_pPointer[0]		= s_pPointer[1]		= nullptr;
	s_pMovePointer[0]	= s_pMovePointer[1] = nullptr;
...
void * i3mem::AllocGame(INT32 nBytes, INT32 State, INT32 Align, const char * FileName, INT32 Line)
{
	...
	//First Fit 
	while(pBlockPointer != nullptr)
	{		
		if( (pBlockPointer->m_pPointer == nullptr) &&  (pBlockPointer->m_Size >= nBytes) )
		{
	...
	if(pBlockPointer == nullptr)
	{
		...
		{			
			Dump(); 
			I3PRINTLOG(I3LOG_FATAL,  "[i3mem::Alloc] Not Enough Memory Size = %d", nBytes);
			s_MemoryCs.Leave(); 
			return nullptr;
		}
	}
	...
	if( pBlockPointer->m_Size > (INT32)( ( nBytes + sizeof(I3MEMBLOCK) + INFO_SIZE) + DIVIDE_SIZE ) )
	{
		...
		pNewFreeBlock->m_pPointer	= nullptr;
		pNewFreeBlock->m_Prev		= nullptr;
		pNewFreeBlock->m_Next		= nullptr;
...
// Desc: 
//----------------------------------------------------------------------------*/
void i3mem::FreeGame(const void * pPointer, const char * pszFile, INT32 line )
{	
...
void i3mem::CompactGame(void)
{
	...
	//하나만 있기 때문에 할 필요없다. 
	if(s_pFreeBlockHead->m_Next == nullptr)return;
	...
	//Sort by Pointer 	
	{
		...
		{
			I3ASSERT( pSelPreBlock != nullptr);

			pSelPreBlock->m_Next = pStartBlock->m_Next;
		}
		pStartBlock->m_Next = nullptr;
...
void i3mem::Dump(INT32 Type)
{
	...
	{		
		...
		{
			I3TRACE("====================== SUM MEMORY ==================================\n"); 
			i3::integral_to_comma_str((s_pPointer[1] - s_pPointer[0]), TempChar, 128); 
...
void * i3mem::AllocTool(INT32 nBytes, INT32 State, INT32 Align, const char * FileName, INT32 Line)
{
	...
	I3ASSERT(nBytes != 0); 
		...
		if(s_bNetDump)
		{
			...
			SendBuffer._Command = MEMDUMP_CMD_MALLOC;
			SendBuffer._Size	= nBytes;
			SendBuffer._Address = (UINT32)pRv; 			
			sprintf( SendBuffer._String, "%s - %d", FileName, Line); 
...
// Desc: 
//----------------------------------------------------------------------------*/
void i3mem::FreeTool(const void * pPointer, const char * pszFile, INT32 line ) 
{
	s_MemoryCs.Enter(); 
	I3ASSERT( pPointer != nullptr);
	I3MEMBLOCK * pBlock;
...
{	
	if(pPointer == nullptr) return Alloc(nBytes, 0, Align, FileName, Line);

	s_MemoryCs.Enter(); 
	void * pRv; 
	I3MEMBLOCK * pNewBlock = nullptr;
	INT32 allocSz = nBytes;
	I3MEMBLOCK * pBlock;
#if defined( I3_DEBUG)
