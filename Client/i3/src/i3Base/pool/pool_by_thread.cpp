...
#pragma init_seg(lib)
	...
	pool_by_thread::pool_by_thread() : pool_tls_base<pool_by_thread>( init_pool_memory_size ), 
		m_ap_pool_by_size(nullptr), m_ap_free_block_by_size(nullptr)
	{
	...
	void*				pool_by_thread::allocate( size_t size)
	{
	//	size = align_up(calculate_adjusted_size(size), calculate_adjusted_align(align));		// 정렬인수를 제거..(2012.08.24..할당 풀에서 정렬주소를 취하고 있음..)
...
//			return i3::tls_safe_aligned_malloc( i3::shared_tls<pool_by_thread>::get_thread_id(), size, align );		// 
		...
		if ((result = detail::allocate_from_active_blocks( ref_active_block )) != nullptr)
			return result;

		//  active_block == nullptr임.. (모든 블록의 prev/next 링크가 모두 깨져있어야함..)  tss의 프리 리스트를 뒤져볼것.!!
		if ((result = detail::allocate_from_tss_blocks(ref_active_block, &m_ap_free_block_by_size[size_index] )) != nullptr)
			return result;
...
//		널값비교를 좀더 상위에 두도록 변경...if (!data) return;
		...
		if ( block->threadID == threadID )		// 동일 쓰레드..
			...
			get_raw_by_thread_id_unsafe( block->threadID )->deallocate_other_thread(  block, data  );	// 타쓰레드....pool_by_thread의 다른 인스턴스에 호출한다..
	}
