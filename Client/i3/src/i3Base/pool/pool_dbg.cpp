...
namespace i3
{
	...
	{
		dbg_state() : first(nullptr), last(nullptr), request_count(0), accum_size(0), curr_alloc_size(0), max_alloc_size(0),
			max_size_per_alloc(0), breakAlloc(DWORD(-1)), critical_section(1024) { }
	...
	{
		return sizeof( i3::dbg_block ) + size;
	}

	
	size_t		calculate_aligned_dbg_data_size( size_t size, size_t adjusted_align)
	{
		return sizeof( i3::dbg_block) + size + adjusted_align + sizeof( i3::aligned_dummy );
	}

	void*		apply_dbg_data_impl( const char* szFilename, int line, 
		void* raw_data, size_t orig_size, size_t align)
	{
		...
		DWORD request_number = state.request_count;
		...
		header->request_number = request_number;
	
	
		// 앞뒤에 무인도바이트..(뒷쪽은 폐기했음.2012.08.24.수빈)
...
//		NetworkDump_Malloc(raw_data, orig_size + sizeof(dbg_block), szFilename, line);

		return orig_start;
	}
	
	void*		apply_dbg_data( const char* szFilename, int line, 
		void* raw_data, size_t orig_size )
	{
		return apply_dbg_data_impl(szFilename, line, raw_data, orig_size, 0);
	}
		...
		size_t adjusted_align, size_t orig_size)
	{
		...
		void* raw_data2 = apply_dbg_data_impl(szFilename, line, raw_data, orig_size, adjusted_align);
...
//		NetworkDump_Free(raw_data, szFilename, line);
//		NetworkDump_Malloc(raw_data2, orig_size + sizeof(dbg_block), szFilename, line);
...
//		NetworkDump_Free( header, __FILE__, __LINE__);
...
//		BYTE* data_start = (BYTE*)( mem + 1);
	...
	{								// tls쪽 dll에서 callback이 되도록 조정할것..
		...
		if (state.first)
		{
			...
			for ( dbg_block* head = state.first; head != nullptr; head = head->next )
			{
				...
				real_data_ptr = reinterpret_cast<BYTE*>(head + 1);
				real_data_size = head->data_size;

				if (align > 0)
				{
					real_data_size -= DWORD( align + sizeof(aligned_dummy) );
					real_data_ptr = reinterpret_cast<BYTE*>( ( uintptr_t(real_data_ptr) + sizeof(aligned_dummy) + align - 1 ) & ~(align - 1) );
				}
						
				_RPT2(_CRT_WARN, "normal block at 0x%p, %Iu bytes long.\n", real_data_ptr, real_data_size);
	...
	I3_EXPORT_BASE DWORD	set_break_alloc(DWORD alloc_number)
	{
		dbg_state& state = *dbg_state::i();	
