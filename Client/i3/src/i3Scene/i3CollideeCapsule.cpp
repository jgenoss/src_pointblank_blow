...
I3_CLASS_INSTANCE( i3CollideeCapsule);

void i3CollideeCapsule::Transform( MATRIX * pMatrix, i3Collidee * pCol)
{
	i3CollideeCapsule * pDest = (i3CollideeCapsule *) pCol;

	I3ASSERT( i3::same_of<i3CollideeCapsule*>(pDest));
...
namespace pack
{
	struct ALIGN4 COLLIDEE_CAPSULE
	{
...
UINT32 i3CollideeCapsule::OnSave( i3ResourceFile * pResFile)
{
	UINT32 Rc, Result = 0;
	i3Stream * pStream = pResFile->GetStream();
	pack::COLLIDEE_CAPSULE Data;
	...
	if( Rc == STREAM_ERR)
	{
		i3Log( "i3CollideeCapsule::OnSave()", "Could not read data.");
		return STREAM_ERR;
	}
	Result += Rc;
...
UINT32 i3CollideeCapsule::OnLoad( i3ResourceFile * pResFile)
{
	UINT32 Rc, Result = 0;
	i3Stream * pStream = pResFile->GetStream();
	pack::COLLIDEE_CAPSULE Data;
	...
	if( Rc == STREAM_ERR)
	{
		i3Log( "i3CollideeCapsule::OnLoad()", "Could not read data.");
		return STREAM_ERR;
	}
	Result += Rc;
