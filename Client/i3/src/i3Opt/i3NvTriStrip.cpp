...
bool TestTriangle(const unsigned short v0, const unsigned short v1, const unsigned short v2, const std::vector<NvFaceInfo>* in_bins, const int NUMBINS)
{
	...
	bool isLegit = false;
	int ctr = v0 % NUMBINS;
	for (int k = 0; k < (int) in_bins[ctr].size(); ++k)
	{
		//check triangles in this bin
		if (SameTriangle( (unsigned short) in_bins[ctr][k].m_v0, (unsigned short) in_bins[ctr][k].m_v1, (unsigned short) in_bins[ctr][k].m_v2, 
			v0, v1, v2))
		{
			isLegit = true;
			break;
					...
					PrimitiveGroup** primGroups, unsigned short* numGroups, bool validateEnabled)
{
		...
		*numGroups = 1;
		...
		for(i = 0; i < (int) tempStrips.size(); i++)
		{
			for(int j = 0; j < (int) tempStrips[i]->m_faces.size(); j++)
			{
				...
				{
					primGroupArray[0].indices[indexCtr++] = (unsigned short) tempStrips[i]->m_faces[j]->m_v0;
					primGroupArray[0].indices[indexCtr++] = (unsigned short) tempStrips[i]->m_faces[j]->m_v1;
					primGroupArray[0].indices[indexCtr++] = (unsigned short) tempStrips[i]->m_faces[j]->m_v2;
				}
				else
				{
		...
		for(i = 0; i < (int) tempFaces.size(); i++)
		{			
			primGroupArray[0].indices[indexCtr++] = (unsigned short) tempFaces[i]->m_v0;
			primGroupArray[0].indices[indexCtr++] = (unsigned short) tempFaces[i]->m_v1;
			primGroupArray[0].indices[indexCtr++] = (unsigned short) tempFaces[i]->m_v2;
		}
	}
	else
	{
		...
		*numGroups = (unsigned short) numSeparateStrips; //for the strips
		if(tempFaces.size() != 0)
		...
		for(unsigned int stripCtr = 0; stripCtr < numSeparateStrips; stripCtr++)
		{
			...
			if(!bStitchStrips)
			{
				...
				for( i = startingLoc; i < (int) stripIndices.size(); i++)
				{
					if(stripIndices[i] == -1)
						break;
				}
			...
			for( i = startingLoc; i < stripLength + startingLoc; i++)
				primGroupArray[stripCtr].indices[indexCtr++] = (unsigned short) stripIndices[i];
		...
		if(tempFaces.size() != 0)
		{
			...
			for( i = 0; i < (int) tempFaces.size(); i++)
			{
				primGroupArray[faceGroupLoc].indices[indexCtr++] = (unsigned short) tempFaces[i]->m_v0;
				primGroupArray[faceGroupLoc].indices[indexCtr++] = (unsigned short) tempFaces[i]->m_v1;
				primGroupArray[faceGroupLoc].indices[indexCtr++] = (unsigned short) tempFaces[i]->m_v2;
			}
		}
	}
	...
	if (validateEnabled)
	{
		...
		for( i = 0; i < (int)in_numIndices; i += 3)
		{
	...
	for(int i = 0; i < numGroups; i++)
	{
		...
		for( unsigned int j = 0; j < numIndices; j++)
		{
			...
			if(cachedIndex == -1) //we haven't seen this index before
			{
				...
				(*remappedGroups)[i].indices[j] = (unsigned short) indexCtr;
			...
			else
			{
				...
				(*remappedGroups)[i].indices[j] = (unsigned short) cachedIndex;
			}
		}
	}
