...
{
	for(int i = 0; i < (int) faceInfos.size(); ++i)
	{
									 ...
									 const unsigned short maxIndex)
{
	...
	{	
		...
		if(IsDegenerate((unsigned short) v0, (unsigned short) v1, (unsigned short) v2))
			continue;
		...
		else 
		{
			...
			{
				I3TRACE("BuildStripifyInfo: > 2 triangles on an edge... uncertain consequences\n");
			}
			else
			{
...
inline int NvStripifier::GetNextIndex(const WordVec &indices, NvFaceInfo *face){
	...
	if (fv0 != v0 && fv0 != v1){
		if ((fv1 != v0 && fv1 != v1) || (fv2 != v0 && fv2 != v1)){
			I3TRACE("GetNextIndex: Triangle doesn't have all of its vertices\n");
			I3TRACE("GetNextIndex: Duplicate triangle probably got us derailed\n");
		}
		return fv0;
	}
	if (fv1 != v0 && fv1 != v1){
...
void NvStripInfo::Build(NvEdgeInfoVec &edgeInfos, NvFaceInfoVec &faceInfos)
{
	...
	scratchIndices.push_back( (WORD) v0);
	scratchIndices.push_back( (WORD) v1);
	int v2 = NvStripifier::GetNextIndex(scratchIndices, m_startInfo.m_startFace);
	scratchIndices.push_back( (WORD) v2);
	...
	while (nextFace != nullptr && !IsMarked(nextFace))
	{
		...
		if( (nextNextFace == nullptr) || (IsMarked(nextNextFace)) )
		{
			...
			if( ((testNextFace != nullptr) && !IsMarked(testNextFace)) )
			{
				...
				scratchIndices.push_back( (WORD) nv0);
				testnv0 = nv0;
		...
		scratchIndices.push_back( (WORD) testnv1);
	...
	for(int i = 0; i < (int) forwardFaces.size(); i++)
...
void NvStripifier::RemoveSmallStrips(NvStripInfoVec& allStrips, NvStripInfoVec& allBigStrips, NvFaceInfoVec& faceList)
{
	...
	for(int i = 0; i < (int) allStrips.size(); i++)
	{
		if( (int) allStrips[i]->m_faces.size() < minStripLength)
		{
			//strip is too small, add faces to faceList
			for(int j = 0; j < (int) allStrips[i]->m_faces.size(); j++)
	...
	if(tempFaceList.size())
	{
		...
		while( bLoop)	// avoid to warning
		{
			...
			for(int i = 0; i < (int) tempFaceList.size(); i++)
			{
				if(bVisitedList[i])
					continue;
							...
							NvStripInfoVec &outStrips, NvFaceInfoVec& outFaceList)
{
	...
	cacheSize = i3Math::Max(1, in_cacheSize - CACHE_INEFFICIENCY);
...
//    outStrips.push_back(tempStrips[i]);
	...
	if(tempStrips2.size() != 0)
	{
		...
		for(i = 0; i < (int) tempStrips2.size(); i++)
		{
			int numNeighbors = 0;
			
			//find strip with least number of neighbors per face
			for(int j = 0; j < (int) tempStrips2[i]->m_faces.size(); j++)
			{
		...
		while( bLoop)	// avoid to warning
		{
			...
			for(int i = 0; i < (int) tempStrips2.size(); i++)
			{
...
{
	for(int i = 0; i < (int) strip->m_faces.size(); ++i)
	{
		...
		for (i = 0; i < numExperiments; i++)
		{
			if (i != bestIndex)
			{
				...
				for (int j = 0; j < numStrips; j++)
				{
					...
					for (int k = 0; k < (int) currStrip->m_faces.size(); ++k)
					{
