...
I3_CLASS_INSTANCE(i3NetworkClientSocket2);
...
bool i3NetworkClientSocket2::OnCreate(char* serverIp,UINT16 serverPort)
{	
...
bool i3NetworkClientSocket2::OnCreate(UINT32 serverIp,UINT16 serverPort)
{
	return _Create(serverIp, serverPort);
}

void i3NetworkClientSocket2::OnReceive(void)
{
	// 패킷을 받는다.
...
void i3NetworkClientSocket2::SelectEvent(void)
{
...
void	i3NetworkClientSocket2::ReceiveProcess(INT32 receivedByte)
{
	...
	{
		// 패킷 사이즈의 버퍼 오버플로우 검사
		INT32 packetSize = PacketParsing( m_receiveBuffer+i32StartIdx, m_receivedPacketSize-i32StartIdx );
...
bool i3NetworkClientSocket2::_Create(UINT32 serverIp,UINT16 serverPort)
{
	// 리시브 이벤트 생성
	m_recvEvent = ::WSACreateEvent();
