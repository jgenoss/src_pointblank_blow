...
:DoMakeInstaller
call %_PBTOOL_MAKEINSTALLER_% "%cd%\Client\i3\NsisToolsSetup\PBTools.nsi"
...
if %_PBTOOL_SVN_COMMIT_% equ 1 (
  svn commit "%cd%\NsisToolsSetup\PBToolsSetup.exe" -m "Auto commit PBToolsSetup.exe(SVNREV:%BIN_SVNREV%)"
  if not %ERRORLEVEL% equ 0 (
    echo "ERR - failed to svn commit PBToolsSetup.exe"
    goto exitWithErr
  )
)
