...
{	
	...
	// case 3 :		provided from the user.
		...
		// 1st Light
				...
				#if defined( I3L_NORMAL_MAP)
				float3 ambient = diffColor * g_vAmbient;
				diffColor.rgb = (max(litted,0.01f) * diffColor) + ambient;
				#endif

				diffColor.rgb *= luxColor; // * g_vLightDiffuse0;		Light 및 Material의 영향을 받지 않도록...이진균
		...
		// 2nd Light
			...
			#if (I3L_LIGHTTYPE_1 == I3L_LIGHTTYPE_DIRECTIONAL)
				oColor.rgb += GetLighingColor_Dir(	g_vLightDiffuse1, g_vLightSpecular1,
	...
	#if defined( I3L_EMISSIVE_MAP)
		oColor.rgb += tex2D( g_texEmissive, i.Tex0).rgb * g_vMtlEmissive.rgb * 4;
	#endif
	...
	#if defined( I3L_XRAYCOLOR )
		oColor.rgb = g_XrayColor.rgb;
	#endif
