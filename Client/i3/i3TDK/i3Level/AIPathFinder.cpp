...
// 경고 제거를 위한 매크로 제거.... (이런건 매크로 쓰지 말것...)
namespace
{
	// 시작노드 -> 현재노드 까지의 거리
	inline REAL32 GET_G(const D3DXVECTOR3& a, const D3DXVECTOR3& b)
	{
		D3DXVECTOR3 a_m_b = a;	a_m_b -= b;
		return D3DXVec3Length(&a_m_b);
	}
	
	// 현재노드 -> 목표노드 까지의 추정치 거리(휴리스틱)
	inline REAL32 GET_H(const D3DXVECTOR3& a, const D3DXVECTOR3& b)
	{
		D3DXVECTOR3 a_m_b = a;	a_m_b -= b;
		return D3DXVec3LengthSq(&a_m_b)*10;
	}
	...
	while(1)
	{
		...
		for(INT32 i = 0; i < 3; i++)
		{
			...
			if (pMeshNb != nullptr)
			{
				...
				{
					assert( i32Idx < 0xFFFF);
					pNodeChild->m_ui16ParentMesh = static_cast<UINT16>(i32Idx);
				...
				{
					pNodeChild->m_ui16ParentMesh = UINT16(i32Idx);
					pNodeChild->m_i32Byun = i;
					pNodeChild->m_pParentMesh = &m_pAIStage->m_vtMesh[i32Idx];
					pNodeChild->m_pParentNode = pNodeParent;
...
void CAIPathFinder::ClearState()
{
	for(UINT32 i = 0; i < m_pAIStage->m_vtMesh.size(); i++)
	{
		...
		pNode->m_i32State = 0;
		pNode->m_ui16ParentMesh	= UINT16(-1);
		pNode->m_r32G = 0.f;
		pNode->m_r32F = 0.f;
		pNode->m_r32H = 0.f;
		pNode->m_pMyMesh = &m_pAIStage->m_vtMesh[i];
		pNode->m_ui16MyMesh = UINT16(i);
	}
}
...
BOOL CAIPathFinder::FindPath(CAIPathData* pPathData, CAIStage* pStage, INT32 i32BeginIdx, INT32 i32GoalIdx, D3DXVECTOR3 dxv3BeginPos, D3DXVECTOR3 dxv3GoalPos)
{
	...
	if (bWayFound)
	{
		...
		// 끝 노드 삽입(1개)
		pPathData->InsertNode(dxv3GoalPos, pNode->m_ui16MyMesh, nullptr);
