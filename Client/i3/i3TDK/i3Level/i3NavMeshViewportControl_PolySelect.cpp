...
I3_CLASS_INSTANCE( i3NavMeshViewportControl_PolySelect); //, i3LevelViewportControl );

void	i3NavMeshViewportControl_PolySelect::DrawRect( HWND hWnd, const CPoint& pt1, const CPoint& pt2 )
{
	HDC dc;
	CRect rect;
...
bool	i3NavMeshViewportControl_PolySelect::OnLButtonDown( I3_LEVEL_EDIT_CONTEXT * pCtx, UINT nFlags, CPoint point )
{	
...
bool	i3NavMeshViewportControl_PolySelect::OnLButtonUp( I3_LEVEL_EDIT_CONTEXT * pCtx, UINT nFlags, CPoint point )
{
	...
	if( nullptr != pScene )
	{
		...
		if( nullptr != pNav )
		{
			if( false == isSelBand )
			{				
				pNav->SelectPolygon( point, mode );
			}
			else
			{
				pNav->SelectBandPolygon( pCtx->m_DragStartPt, point, mode );
			}
		}
	}	
...
bool	i3NavMeshViewportControl_PolySelect::OnMouseMove( I3_LEVEL_EDIT_CONTEXT * pCtx, UINT nFlags, CPoint point )
{
	if( i3Level::GetNavMesh() )
		i3Level::GetNavMesh()->UpdateMouseMove( point, false, true );	
