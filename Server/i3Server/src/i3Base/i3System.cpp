...
static I3_TERMINATE_INFO *	s_pHead = NULL;
...
void i3System::GetCPUInfo( char * pszTemp)
{
	...
	i3String::Concat( pszTemp, buffer);
#endif
}
...
void i3System::GetGPUInfo( char * pszTemp)
{
	...
	if( lResult == ERROR_SUCCESS ) 
	{
		...
		i3String::Concat( szResult, szTemp);
	...
	i3String::Concat( pszTemp, szResult);
#endif
}
...
void i3System::GetOSInfo( char * pszTemp)
{
	...
	if( !bOsVersionInfoEx)
	{
		...
		{
			i3String::Concat( pszTemp, "Unknown O/S");		// OS확인 불가
			return;
		}
	}
	...
	if( osvi.dwMajorVersion >=5){ // win200이상
		...
		{
			case 0: // 2000
				i3String::Concat( pszTemp, "Windows 2000");
				return;
			case 1: // xp
				i3String::Concat( pszTemp, "Windows XP");
				return;
			case 2: // 2003
				i3String::Concat( pszTemp, "Windows 2003");
				return;
			default:
				break;          
		}
		i3String::Concat( pszTemp, "Windows 2000이상의 O/S");		// 2000이상
	}
	else
	{   
		// wind NT
		i3String::Concat( pszTemp, "Windows NT4.0이하의 O/S");		// nt 4.0 이하 
		return; 
	}
	...
	case VER_PLATFORM_WIN32_WINDOWS:        //95계열 윈도우 시리즈
		...
		{
			//윈도우 95
			i3String::Concat( pszTemp, "Windows 95");
			return;
		} 
		...
		{
			//윈도우 98
			i3String::Concat( pszTemp, "Windows 98");
			return;
		} 
		...
		{
			//윈도우 ME
			i3String::Concat( pszTemp, "Windows Me");
			return;
		} 
		i3String::Concat( pszTemp, "Wind32");
		return; // wind32
	break;

	case VER_PLATFORM_WIN32s:      //윈도우 32s
		i3String::Concat( pszTemp, "Win32s");
		return;   // 
	default:
		i3String::Concat( pszTemp, "Unknown O/S. Linux O/S라고 생각되어짐!");
		return;
	} 
#endif
}
...
void i3System::GetMemoryInfo( char * pszTemp)
{
	...
	//{ 
	//DWORD dwLength;			// 이 구조체의 사이즈
	//DWORD dwMemoryLoad;		// 현재 메모리 사용을 나타내는 전반적인 메모리 부하 매개변수
	//DWORD dwTotalPhys;		// 메인 메모리 총 크기
	//DWORD dwAvailPhys;		// 메인 메모리 가용 크기
	//DWORD dwTotalPageFile;	// 총 페이징 파일 사이즈 ( XP , ME 용)
	//DWORD dwAvailPageFile;	// 가용 페이징 파일 사이즈
	//DWORD dwTotalVirtual;		// 가상 메모리 총 사이즈
	//DWORD dwAvailVirtual;		// 가용 가상 메모리 총 사이즈
	//
	...
	i3String::Concat( pszTemp, szTemp);
}
...
void i3System::GetMultiMediaInfo( char * pszTemp)
{
	...
	i3String::Concat( pszTemp, "Multimedia = ");
	...
	{
		// 사운드 카드가 설치되지 않음.
		i3String::Concat( pszTemp, "사운드 카드가 설치되지 않았습니다");
		return;
	}

	i3String::Concat( pszTemp, m_CompanyName);
#endif
...
void i3System::GetDXInfo(char * pszTemp)
{
	...
	if( lResult == ERROR_SUCCESS ) 
	{
		...
		i3String::Concat( szTemp, "DirectX = ");
		i3String::Concat( szTemp, buffer);
	}
...
void i3System::SendMailBugReport( char * pszTemp, char* pszFileName )
{
	...
	if( mail.ConnectServer( "mail.zepetto.com" ) ) 
	{
		...
		i3String::SplitFileName( szPath, szRunApp, FALSE);
		...
		if( pszFileName )
		{
			...
			i3String::Concat(szDirPath, "\\");
			i3String::Concat(szDirPath, pszFileName);
...
void i3System::AddTerminateProc( INT32 prio, I3_TERMINATE_PROC pProc, void * pUserData)
{
	...
	s_CSSystem.Enter();
	{
		I3_TERMINATE_INFO * pPrev = NULL;
		pTemp = s_pHead;

		while((pTemp != NULL) && (pTemp->m_Priority >= prio))
		{
			pPrev = pTemp;
			pTemp = pTemp->m_pNext;
		}
...
BOOL i3System::GetDrive( const char* strPath, char* strDrive, INT32 i32OutSize )
{
	INT32 i32Size = 0;
	while( '\0' != strPath[i32Size] && '\\' != strPath[i32Size] )
	{
		strDrive[i32Size] = strPath[i32Size];
		i32Size++;
		if( i32OutSize <= i32Size )	return FALSE;
	}
	strDrive[i32Size] = '\0';
	return TRUE;
}

BOOL i3System::GetDrive( const wchar_t* wstrPath, wchar_t* wstrDrive, INT32 i32OutSize )
{
	INT32 i32Size = 0;
	while( '\0' != wstrPath[i32Size] && '\\' != wstrPath[i32Size] )
	{
		wstrDrive[i32Size] = wstrPath[i32Size];
		i32Size++;
		if( i32OutSize <= i32Size )	return FALSE;
	}
	wstrDrive[i32Size] = '\0';
...
// 작성자		: 이진균
// 작성일자		: 1999.6.1
BOOL i3System::IsDirectory( const char* strPath )
{
	BOOL Result = FALSE;

    char strOldPath[ MAX_PATH];
    
    GetCurrentDirectoryA( MAX_PATH, strOldPath );

    if( SetCurrentDirectoryA( strPath ) == TRUE)
        Result = TRUE;

    SetCurrentDirectoryA( strOldPath );

    return Result;
}

BOOL i3System::IsDirectory( const wchar_t * wstrPath )
{
	BOOL Result = FALSE;

    wchar_t wstrOldPath[ MAX_PATH];
    
    GetCurrentDirectoryW( MAX_PATH, wstrOldPath);

    if( SetCurrentDirectoryW( wstrPath ) == TRUE)
        Result = TRUE;

    SetCurrentDirectoryW( wstrOldPath );
...
void	i3System::GetExecPath( char * pszPath)
{
	...
	if( GetModuleFileName( GetModuleHandle( NULL), szTemp, MAX_PATH) == 0)
	{
...
void i3System::GetCurrentPath( char * pszPath, INT32 pathLen)
{
#if defined( I3_WINDOWS)
	::GetCurrentDirectory( pathLen, pszPath);
#else
	if( getcwd( pszPath, bufSize) == NULL)
...
// 작성자		: 이진균
// 작성일자		: 1999.6.1	
BOOL i3System::PrepareDirectory( char* strPath )
{
 	char	strOldPath[ MAX_PATH ];
	...
	while( *pPath ) 
	{
		...
		if( FALSE == IsDirectory( strDir ) ) 
		{
			if( FALSE == CreateDirectoryA( strDir, NULL ) ) 
			{
			    return FALSE;
			}
		}
	...
	return SetCurrentDirectoryA( strOldPath );
}

BOOL i3System::PrepareDirectory( wchar_t* wstrPath )
{
	wchar_t	wstrOldPath[ MAX_PATH ];
	wchar_t	wstrDir[ MAX_PATH ];
	wchar_t	*pPath;
	INT32	i32Idx;
	...
	memset( wstrDir, 0, sizeof(wstrDir) );
	...
	while( *pPath ) 
	{
		...
		if( FALSE == IsDirectory( wstrDir ) ) 
		{
			if( FALSE == CreateDirectoryW( wstrDir, NULL ) ) 
			{
			    return FALSE;
			}
		}
	...
	return SetCurrentDirectoryW( wstrOldPath );
}
...
BOOL i3System::RenameFile( const char *szOldFileName, const char *szNewFileName )
{
	...
	return ::MoveFile( szOldFileName, szNewFileName );
