...
BOOL CPacketLocker::Create(UINT32 ui32BufferCount,UINT32 ui32BufferSize,
						   UINT32 ui32DataSize,UINT32 ui32MaxIdx)
{
	...
	if( NULL == m_ppTrackBuffer ) return FALSE;

	for(UINT32 i = 0; i < ui32BufferCount; i++)
	{
		m_ppTrackBuffer[i] = CTrackBuffer::NewObject();
		if( NULL == m_ppTrackBuffer[i] ) return FALSE;

		UINT32 createSize = ui32DataSize;
		m_ppTrackBuffer[i]->Create(ui32BufferSize, createSize, ui32MaxIdx);
	}
