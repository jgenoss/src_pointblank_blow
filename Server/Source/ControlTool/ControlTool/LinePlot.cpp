...
CRect CLinePlot::DrawFramework(CDC *pDC, CRect rcBounds, int nFontHeight, bool oIsPrinting)
{
  ...
  rcPlotArea.left += nCaptionHeight + 80; // + nSpacer;
  rcPlotArea.bottom -= 20;
  rcPlotArea.right -= nCaptionHeight - 12;
  rcPlotArea.top += nSpacer + 10;

  //  calculate the widest rect for the y scale.
  CString szTestNumMin;
  szTestNumMin.Format( _T("%0.f"), m_eYMin);
  int nNumCharsMin = szTestNumMin.GetLength();
  CString szTestNumMax;
  szTestNumMax.Format( _T("%0.f"), m_eYMax);
  ...
  CString szXFormat;
  szXFormat.Format( _T("%2dH"), m_uiXPrecision);
  CString szYFormat;
  szYFormat.Format( _T("%%0.%uf"), m_uiYPrecision);
  ...
  */
  ...
  if (rcPlotArea.left < rcPlotArea.right)
  {
    ...
    {
		if(ii != 0)
			szScale.Format( _T("-%02dH"), ii);
		else
			szScale.Format( _T("%02dH"), ii);

      ptXScale.x = (int)(rcStartScale.right+(float)(ii)*eScreenXInterval);
	  ptXScale.y = rcStartScale.bottom + siScale.cy + 5;
      pDC->TextOut(ptXScale.x + 10, ptXScale.y, szScale);
      pDC->MoveTo(ptXScale.x, rcPlotArea.top - (siScale.cy + 5));
    ...
    for (int jj=0; jj<m_nYIntervals+1; jj++)
    {
      ...
      ptYScale.x = rcStartScale.right - 5;
      ptYScale.y = (int)(rcStartScale.top-(float)(jj)*eScreenYInterval) - nSpacer/2;
      pDC->TextOut(ptYScale.x, ptYScale.y + 10, szScale);
	...
	*/
    ...
    if (nCount>0 && m_nSelected>=0 && m_nSelected<nCount)
    {
      if (m_nCursor >= 0 && (UINT)(m_nCursor) < m_lstPlotData[m_nSelected].m_uiPointCount)
      {
        ...
        szPosition.Format( _T("Position: %0.3f    Value: %0.3f"), ptDisplay.x, ptDisplay.y);
        pDC->SetTextAlign(TA_LEFT | TA_BOTTOM);
    //    pDC->TextOut(rcStats.left+1, rcStats.bottom-1, szPosition);
      ...
      szPosition.Format( _T("Mean: %0.3f    Min: %0.3f    Max: %0.3f"), eYMean, eYMin, eYMax);
      pDC->SetTextAlign(TA_RIGHT | TA_BOTTOM);
...
void CLinePlot::OnPaint() 
{
  ...
  //  get the area to draw in.
	...
	int nFontHeight = (GetSystemMetrics(SM_CYCAPTION) / 2) + 3;

	//  create a memory dc for double buffering.
	CUserMemDC dcMem(&dc, &rcClient);
  ...
  if(dlgPrint.DoModal() == IDOK)
  {
    ...
    if (hdcPrinter!=NULL)
    {
      ...
      else //  there was a printer error of some sort....
      {
        ...
        AfxMessageBox( _T("Cannot Start Print Job.") );
      }
      //  clean up
      dcPrinter.Detach();
...
int CLinePlot::Add(CString szName, COLORREF crColor, enumPlotStyle nStyle, FLOATPOINT *pptData, UINT uiPointCount)
{
  ...
  if (m_oIsYLocked == false)
  {
	...
	{
		INT32 i32CheckData = 0;
		if((INT32)(PlotData.m_eYDataMax / 100000) != 0)
		{
			i32CheckData = (INT32)(PlotData.m_eYDataMax / 100000);
			switch(i32CheckData)
			{
			case 9:
				m_eYMax = 1200000;
		...
		else if((INT32)(PlotData.m_eYDataMax / 10000) != 0)
		{
			i32CheckData = (INT32)(PlotData.m_eYDataMax / 10000);
			switch(i32CheckData)
			{
			case 9:
				m_eYMax = 120000;
		...
		else if((INT32)(PlotData.m_eYDataMax / 1000) != 0)
		{
			i32CheckData = (INT32)(PlotData.m_eYDataMax / 1000);
			switch(i32CheckData)
			{
			case 9:
				m_eYMax = 12000;
		...
		else if((INT32)(PlotData.m_eYDataMax / 100) != 0)
		{
			i32CheckData = (INT32)(PlotData.m_eYDataMax / 100);
			switch(i32CheckData)
			{
			case 9:
				m_eYMax = 1200;
		...
		else if((INT32)(PlotData.m_eYDataMax / 10) != 0)
		{
			i32CheckData = (INT32)(PlotData.m_eYDataMax / 10);
			switch(i32CheckData)
			{
			case 9:
				m_eYMax = 120;
...
void CLinePlot::HideEditWindow()
{
  ...
  if (m_edtDataEntry.IsWindowVisible()==TRUE)
  {
    ...
    float eValue  = (float)(_tcstod(szText.GetBuffer(1), NULL));
