...
DWORD i3scolFile::Read(NxShapeSet* pShapeSet)
{
	...
	{
		// set file pointer
		if(INVALID_SET_FILE_POINTER == SetFilePointer(m_hFile, sizeof(m_head) + m_index * sizeof(desc), 0, FILE_BEGIN)) 
		{
			return 0;
		}
		...
		if( desc.tsize > 0 )
		{
			//pShapeSet->m_pvTerrain			= (NxVec3*) malloc( desc.tsize );
			pShapeSet->m_pI3Terrain			= (I3_PHYSIX_TERRAIN*) malloc( desc.tsize );
			//pShapeSet->m_numTerrainVectors	= desc.tsize / desc.tstep;
