...
{
	BOOL    (*m_pfnCreateThread)(DS_THREAD_TYPE Type, INT32 i32ThreadIdx);	// 쓰레드 생성 함수
	HANDLE			m_hevtQuit;					// 종료 이벤트
	HANDLE*			m_phThreads;				// thread handles : 모니터 대상
	INT32			m_i32ThreadCount;			// count 
	DS_THREAD_TYPE	m_CreateType;				// thread type for recreation

	MONITOR_THREAD_PARAM()
	{
		m_pfnCreateThread	= NULL;
		m_hevtQuit			= NULL;
		m_phThreads			= NULL;
		m_i32ThreadCount	= 0;
		m_CreateType		= THREAD_TYPE_UNKNOWN;
	}
};
