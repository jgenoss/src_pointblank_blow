...
S2List::S2List( INT32 i32AllocUnit )
{
	...
	NSM_nsMemory::FillZero(_defaultBuf, sizeof(_defaultBuf));
...
{
	if( m_bIsUseMalloc )	NSM_SAFE_FREE(Items);
	NSM_SAFE_FREE( m_pStackBuffer);
	m_nStackDepth		= 0;
	m_nStackTop			= -1;
}
...
BOOL S2List::ResizeBuffer( INT32 nCount )
{
	...
	if( m_i32MaxCount != NewCount )
	{
		if (m_bIsUseMalloc)
		{
			Items = (void **) NSM_nsMemory::Realloc( Items,  NewCount * sizeof(void *), 0, 0);
			if(NULL == Items)
			{	
				// I3Log 남길것				
			#ifdef UNLOCK_LOG
		...
		else
		{
			...
			{
				Items = (void **) NSM_ALLOC( (NewCount * sizeof(void *)));
				NSM_nsMemory::Copy(Items, _defaultBuf, sizeof(_defaultBuf));
				m_bIsUseMalloc = TRUE;
				m_i32MaxCount = NewCount;
			}
		}
...
{
	m_nStackTop++;
	_ASSERT( m_nStackTop >= 0 && m_nStackTop < m_nStackDepth);
...
{
	if( m_nStackDepth < m_i32Count) NSM_SAFE_FREE( m_pStackBuffer);
	...
	{	
		...
		m_pStackBuffer = (QSTACK*) NSM_ALLOC( sizeof( QSTACK) * depth);
		m_nStackDepth = depth;
	}

	NSM_nsMemory::FillZero( m_pStackBuffer, sizeof( QSTACK) * m_nStackDepth);
...
void S2List::Copy( const S2List *List)
{
	...
	ResizeBuffer( m_i32Count);
	memcpy( Items, List->Items, sizeof(void *) * m_i32Count);
}
