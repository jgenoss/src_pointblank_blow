...
BOOL CModuleZLog::Create( UINT32 ui32IP, UINT16 ui16Port )
{
	// ZLog 모듈이 꺼져있습니다.
	if( FALSE == g_pConfig->m_bLogDActive ) return TRUE;
	
	m_ui32IP	= ui32IP;
	m_ui16Port	= ui16Port;
	i3String::Copy( m_strLogPath, g_pConfig->m_strZLogFilePath, MAX_STRING_COUNT );

	// Socket Open
	m_pZLogSocket = CModuleZLogSocket::NewObject(); 
	if( FALSE == m_pZLogSocket->OnCreate( m_ui32IP, m_ui16Port ) )
	{
...
{
	g_pLog->AddLogThread( L"ZLOG_SEND", 0, m_ThreadID );
	...
	while( m_bRunning )
	{
		...
		if(!m_pZLogSocket->IsConnected()) 
		{
			...
			if( m_pZLogSocket->OnCreate( m_ui32IP, m_ui16Port ) )
			{
				g_pLog->WriteLog( WARNING_TYPE_NORMAL, WARNING_LEVEL_NETWORK, L"SUCCESS LOG_MODULE RECONNECT 1" );
				m_eUpdateState = ZLOG_UPDATE_STATE_FIND;
			}
			else
			{
				m_pZLogSocket->OnDestroy(); 
				g_pLog->WriteLog( WARNING_TYPE_NOTICE, WARNING_LEVEL_NETWORK, L"FAIL LOG_MODULE RECONNECT 1" );
			}
...
BOOL CModuleZLog::_OpenFile()
{
	...
	{
		g_pLog->WriteLog(L"Set File Poniter Fail. Error Code : %x", GetLastError());
...
{
	i3NetworkPacket	SendPacket(PROTOCOL_ZLOG_FILEINFO_SEND);
...
BOOL CModuleZLog::_UploadFile()
{
	...
	while(!m_bFinish)
	{
		...
		if (TotalSendSize == m_ui32FileSize)
		{
			wchar_t wstrUploadFile[MAX_STRING_COUNT];
			MultiByteToWideChar(CP_ACP, NULL, m_strUploadFile, -1, wstrUploadFile, MAX_STRING_COUNT);

			g_pLog->WriteLog(L"ZLog File Transfer Finished / FileName : %s / Total Size : %d / Write : %d / Send : %d ", wstrUploadFile, TotalSendSize, i32Size, i32ret);
			m_bFinish = TRUE;
		}
