...
public:
	...
	// 빌링 로그인 요청
	bool UserLogin(	DWORD dwAccountGUID,			// 계정 GUID
	...
	// 빌링 로그아웃 요청
	bool UserLogout(long dwBillingGUID);			// 빌링 유니크 아이디(로그인 성공 시 OnUserStatus 함수를 통해 전달된 값)

	// 빌링 로그인 정보 조회 요청
	bool InquireUser(long dwBillingGUID);			// 빌링 유니크 아이디(로그인 성공 시 OnUserStatus 함수를 통해 전달된 값)

	// PC 방이 멀티 유저 지원 여부 조회
	bool InquireMultiUser(	long AccountGUID,		// 계정 GUID
							long RoomGUID );		// PC 방 GUID

	// 특정 PC방의 게임 별 정량 보유 여부 조회
	bool InquirePCRoomPoint(long AccountGUID,		// 계정 GUID
							long RoomGUID,			// PC 방 GUID
							long GameCode);			// 게임 코드

	// PC방 퍼블리싱
	// PC방 IP 여부 확인
	bool CheckPCRoomIPAddr(	DWORD dwIPAddress,		// 클라이언트 아이피 주소
...
protected:
	...
	virtual void OnUserStatus(	DWORD dwAccountGUID,				// 계정 GUID
								...
								short nResultCode ) = 0;			// 처리 결과

	// 특정 PC방의 게임 별 정량 보유 여부 조회 결과
	virtual void OnInquirePCRoomPoint(	long AccountGUID,  			// 계정 GUID
										long RoomGUID,				// PC 방 GUID
										long GameCode,				// 게임 코드
										long ResultCode ) = 0;		// 처리 결과

	// PC 방 멀티 유저 조회 결과 
	//		Result : 1 (가능), 2 (불가), -1 (PC방 정보 없음)
	virtual void OnInquireMultiUser(long AccountID,					// 계정 GUID
									long RoomGUID,					// PC 방 GUID
									long Result) = 0;				// 처리 결과
	...
	virtual void OnCheckLoginUser(	long AccountID,					// 계정 GUID
									long BillingGUID ) = 0;			// 빌링 유니크 아이디(로그인 성공 시 OnUserStatus 함수를 통해 전달된 값)

	// PC방 퍼블리싱
	// PC방 IP 여부 확인 결과
	// PC방 IP일 경우 PC방 GUID를 반환한다.
	virtual void OnCheckPCRoomIPAddr(	DWORD dwIPAddress,			// 클라이언트 아이피 주소
										DWORD dwAccountGUID,		// 계정 GUID
	...
	// 정량 보유중일 경우에만 발생한다.
	virtual void OnPCRoomDeductUpdate(	long RoomGUID,				// PC 방 GUID
										long GameCode ) = 0;		// 게임 코드
