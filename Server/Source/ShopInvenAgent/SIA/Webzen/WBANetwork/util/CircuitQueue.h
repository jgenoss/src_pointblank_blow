...
namespace WBANetwork
{
	...
	public:
		bool Create(DWORD bufSize)
		{
			...
			m_buffer		= _new_dbg_ _Ty[bufSize + m_surplusSize];
		...
		bool IsValidWritePtr(_Ty* pos, DWORD size)
		{
			DWORD bufStart = 0;
		...
		bool Enqueue(_Ty* data, DWORD size)
		{
			...
			if(NULL != data)
			{
				if(m_posHead <= m_posTail)
				{
					...
					if (remainSize >= size)
					{
						memcpy(m_buffer + m_posTail, data, sizeof( _Ty ) * size);
					}
					else
					{
						//  버퍼에 여유가 없다면 데이터를 두개로 나누어
						//  버퍼의 앞/뒤에 복사한다.
						memcpy(m_buffer + m_posTail, data, sizeof(_Ty) * remainSize);
						memcpy(m_buffer, data + remainSize, sizeof(_Ty) * (size - remainSize));
					}
				}
				else
				{
		...
		bool Dequeue(_Ty* dest, DWORD size)
		{
		...
		bool Peek(_Ty* dest, DWORD size)
		{
			...
			if(NULL != dest)
			{
				if(m_posHead < m_posTail)
				{
					memcpy(dest, m_buffer + m_posHead, sizeof(_Ty) * size);
				}
				else
				{
					...
					else
					{
						...
						memcpy(dest, m_buffer + m_posHead, sizeof(_Ty) * cutDataSize);
						memcpy(dest + cutDataSize, m_buffer, sizeof(_Ty) * (size - cutDataSize));
					}
				}
			}
		...
		_Ty* GetReadPtr()	{ return (m_buffer + m_posHead); }
		_Ty* GetWritePtr()	{ return (m_buffer + m_posTail); }
