...
#define TOKEN_RSA_PUB_KEY_FILE_SM			"publickey.pem"

CLoginWorker_SM::CLoginWorker_SM()
{
}
...
BOOL CLoginWorker_SM::_Create()
{
	char strKeyFile[MAX_PATH];
	i3String::Format( strKeyFile, MAX_PATH, "%s\\%s", g_strCCurrentPath, TOKEN_RSA_PUB_KEY_FILE_SM );

	if( false == m_OpensslRSA.Init( strKeyFile ) )					return FALSE;

	g_pLog->WriteLog( WARNING_TYPE_NORMAL, WARNING_NO, L"[LOAD] CLoginWorker_SM_Garena Create Success - Check Key File" ); 

	return TRUE;
}

void CLoginWorker_SM::_Destroy()
{
}

BOOL CLoginWorker_SM::_WorkProcess( LOGIN_INPUT* pInput, LOGIN_OUTPUT* pOutput )
{
...
BOOL CLoginWorker_SM::_DecryptSessionKey( LOGIN_INPUT* pInput, LOGIN_OUTPUT* pOutput )
{
