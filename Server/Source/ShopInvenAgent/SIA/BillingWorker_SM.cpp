...
CBillingWorker_SM::CBillingWorker_SM()
{
}
...
BOOL CBillingWorker_SM::_WorkProcess( BILLING_INPUT * pInput, BILLING_OUTPUT* pOutput, UINT8 ui8GoodsIdx )
{
	...
	case TYPE_SHOP_GOODS_BUY:
		{			
			...
			{
				g_pLog->WriteLog( WARNING_TYPE_NORMAL, WARNING_AUTH, L"[CBillingWorker_SM::InputProcess] BUY SP Fail : %s ", wstrSP ); 
	...
	case TYPE_SHOP_TYPE_GET_CASH:
		{
			...
			if( FALSE == m_pDB->ExecuteSP(wstrSP) )
			{
				g_pLog->WriteLog( WARNING_TYPE_NORMAL, WARNING_AUTH, L"[CBillingWorker_SM::InputProcess] GET CASH SP Fail : %s ", wstrSP);
				CBillingDB::ReConnect(wstrSP);
				pOutput->m_ai32Rv[ui8GoodsIdx]	= EVENT_ERROR_DATABASE_EXECUTE_SP;
				return FALSE;
			}
			...
			if( 1 != i32Count ) 
			{
				...
				g_pLog->WriteLog( WARNING_TYPE_NORMAL, WARNING_NO, L"[CBillingWorker_SM::InputProcess] GET CASH SP Fail / Record Count / UID : %I64d / Count : %d", pInput->m_TUID, i32Count);
				return FALSE;
			}
...
void CBillingWorker_SM::_ParsingBuyItem( BILLING_INPUT * pInput, BILLING_OUTPUT * pOut, UINT8 ui8GoodsIdx  )
{
	INT32 i32Count = m_pDB->GetRecordCount();

	if( 1 != i32Count )
	{
		g_pLog->WriteLog( WARNING_TYPE_NORMAL, WARNING_AUTH, L"[CBillingWorker_SM::InputProcess] RecordCount is Wrong / RecordCount :  %d", i32Count);
		return;
	}
	...
	case -1:	// 존재하지 않는 PB 유저
		{
			g_pLog->WriteLog( WARNING_TYPE_NORMAL, WARNING_AUTH, L"[CBillingWorker_SM::InputProcess] no Exist User(%I64d)'s request ", pInput->m_TUID);
	...
	case -3:	// 보유 캐쉬가 Goods 가격보다 적음
		{
			g_pLog->WriteLog( WARNING_TYPE_NORMAL, WARNING_AUTH, L"[CBillingWorker_SM::InputProcess] Buy Fail / Not Enough Cash / User : %I64d, Cash : %I64d ", pInput->m_TUID, ui64CurCash);
			pOut->m_ui32CurrentUserCash		= (UINT32)ui64CurCash;
			pOut->m_ui32CurrentUserCashType	= TYPE_CURRENT_USER_CASH_USE;
