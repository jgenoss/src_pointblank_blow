...
#define TOKEN_RSA_PUB_KEY_FILE_THAI			"publickey.pem"

CLoginWorker_Thai::CLoginWorker_Thai()
{
}
...
BOOL CLoginWorker_Thai::_Create()
{
	char strKeyFile[MAX_PATH];
	i3String::Format( strKeyFile, MAX_PATH, "%s\\%s", g_strCCurrentPath, TOKEN_RSA_PUB_KEY_FILE_THAI );

	if( false == m_OpensslRSA.Init( strKeyFile ) )					return FALSE;

	g_pLog->WriteLog( WARNING_TYPE_NORMAL, WARNING_NO, L"[LOAD] CLoginWorker_Thai_Garena Create Success - Check Key File" ); 

	return TRUE;
}

void CLoginWorker_Thai::_Destroy()
{
}

BOOL CLoginWorker_Thai::_WorkProcess( LOGIN_INPUT* pInput, LOGIN_OUTPUT* pOutput )
{
...
BOOL CLoginWorker_Thai::_DecryptSessionKey( LOGIN_INPUT* pInput, LOGIN_OUTPUT* pOutput )
{
