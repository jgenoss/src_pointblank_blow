...
{	
	//Set Current Directory; 
	{
		...
		strcpy_s( Temp, sizeof(Temp), argv[0] );
		StrLen = (INT32) strlen( Temp ); 
	...
	{
		i3String::Format( pExeFile, sizeof(pModuleFile), "%s",pModuleFile);
		pModuleFile[dwSize-4] = 0;	
	}
	strcpy(pServiceName,"_PBDediServerService");
	...
	{
		printf("Dedication Server Help!\n\n"); 
		printf("Dedicationss.exe -i : Service Install \n"); 
		printf("Dedicationss.exe -u : Service Uninstall. \n"); 
		printf("Dedicationss.exe -s : Service Start. \n");
		printf("Dedicationss.exe -e : Service End. \n");
		printf("Dedicationss.exe : It does not use. \n");
	}
...
BOOL RunService(char* pName)
{
	...
	if (schSCManager==0) 
	{
		...
		char pTemp[128];
		i3String::Format(pTemp, sizeof(pTemp), "OpenSCManager failed, error code = %d\n", nError);
		printf("%s\n",pTemp); 
	}
	else
	{
		...
		if (schService==0) 
		{
			long nError = GetLastError();
			char pTemp[128];
			i3String::Format(pTemp, sizeof(pTemp), "OpenService failed, error code = %d\n", nError);
			printf("%s\n",pTemp); 
		}
		else
		{
			...
			else
			{
				long nError = GetLastError();
				char pTemp[128];
				i3String::Format( pTemp, sizeof(pTemp), "StartService failed, error code = %d\n", nError);
				printf("%s\n",pTemp); 
			}
			CloseServiceHandle(schService); 
		}
	...
	else
	{
		...
		else
		{
			...
			else
			{
				long nError = GetLastError();
				char pTemp[128];
				i3String::Format(pTemp, sizeof(pTemp), "ControlService failed, error code = %d\n", nError);
				printf("%s\n",pTemp);
			}
			CloseServiceHandle(schService); 
		}
...
{	
	...
	if(Rv != EVENT_ERROR_SUCCESS )
	{
		...
		case SERVER_ERROR_SYSTEMLOG:		DSERROR("SystemLog Error!");		break;
		case SERVER_ERROR_CONTROL:			DSERROR("Control Info Error!");		break;
		case SERVER_ERROR_CONTROLSERVER:	DSERROR("Control Server Error!");	break;
		//case SERVER_ERROR_UDPMODULE:		DSERROR("UdpModule Error!");		break;
		//case SERVER_ERROR_GROUPMAKER:		DSERROR("GroupMaker Error!");		break;
		default:							DSERROR("UnKnown Error!");			break;	
		}
...
void Install(char* pPath, char* pName)
{
	...
	else
	{
		...
		if (schService == 0) 
		{
			long nError =  GetLastError();
			char pTemp[128];
			i3String::Format( pTemp, sizeof(pTemp), "Failed to create service %s, error code = %d\n", pName, nError);
			printf("%s\n",pTemp); 
		}
		else
		{
			...
			i3String::Copy( Des, "Point Blank Dedication Server", MAX_STRING_COUNT );
			SERVICE_DESCRIPTION lpDes;
			lpDes.lpDescription = Des;
			ChangeServiceConfig2(schService, SERVICE_CONFIG_DESCRIPTION, &lpDes);

			char pTemp[128];
			i3String::Format(pTemp, sizeof(pTemp), "Service %s installed\n", pName);
			printf("%s\n",pTemp);
			CloseServiceHandle(schService); 
		}
		CloseServiceHandle(schSCManager);
...
void UnInstall(char* pName)
{
	...
	else
	{
		...
		else
		{
			...
			{
				char pTemp[128];
				i3String::Format( pTemp, sizeof(pTemp), "Failed to delete service %s\n", pName);
				printf("%s\n",pTemp); 
			}
			else 
			{
				char pTemp[128];
				i3String::Format( pTemp, sizeof(pTemp), "Service %s removed\n",pName);
				printf("%s\n",pTemp);
			}
			CloseServiceHandle(schService); 
		}
