...
int PrintAddress(SOCKADDR *sa, int salen)
{
    ...
    if (rc != 0)
    {
        DSERROR("[PrintAddress] %s: getnameinfo failed: %d\n", __FILE__, rc);
        return rc;
    }
...
int FormatAddress(SOCKADDR *sa, int salen, char *addrbuf, int addrbuflen)
{
	...
	if (rc != 0)
	{
		DSERROR("[FormatAddress] %s: getnameinfo failed: %d\n", __FILE__, rc);
		return rc;
	}
	if ( (strlen(host) + strlen(serv) + 1) > (unsigned)addrbuflen)
		return WSAEFAULT;
	if (sa->sa_family == AF_INET)
		i3String::Format( addrbuf, addrbuflen,  "%s:%s", host, serv);
...
struct addrinfo *ResolveAddress(char *addr, char *port, int af, int type, int proto)
{
    ...
    *res = NULL;
    ...
    if (rc != 0)
    {
        DSERROR("[ResolveAddress] Invalid address %s, getaddrinfo failed: %d\n", addr, rc);
        return NULL;
    }
    return res;
}
