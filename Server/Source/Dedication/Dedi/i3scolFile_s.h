...
public:
	...
	void	GetChecksum( UINT32* ui32Checksum );
	...
	BOOL IsValid()
	{
		switch( m_head.version )
		{
		case I3SCOL_VERSION1:
			{
				if( sizeof(DS_I3SCOL_HEADER) != m_head.size )	return FALSE;
			}
			break;
		case I3SCOL_VERSION2:
			{
				if( sizeof(DS_I3SCOL_HEADER2) != m_head.size )	return FALSE;
			}
			break;
		default:
			{
		...
		if( m_head.count == 0 )									return FALSE;
