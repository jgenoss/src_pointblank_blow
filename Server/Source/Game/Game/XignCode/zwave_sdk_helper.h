/*--
	Copyright (C) 2007-2014 Wellbia.com Co., Ltd.

	YoungJin Shin <codewiz@wellbia.com>
	2009-06-22

	XIGNCODE3 Helper Interface
...
#define LoadHelperLib			LoadHelperLibW
#define LoadHelperLib2			LoadHelperLib2W
#else
...
#define LoadHelperLib			LoadHelperLibA
#define LoadHelperLib2			LoadHelperLib2A
#endif
...
#define XNEUTRONAPI extern "C" __attribute__((__visibility__("default")))
#else
#define XNEUTRONAPI 
#endif

typedef xbool (XCALL *Send32)(xpvoid uid, xpvoid meta, xpcch buf, xulong size);
typedef xvoid (XCALL *Verify32A)(xpvoid uid, xpvoid meta, xint code, xcstr report);
typedef xvoid (XCALL *Verify32W)(xpvoid uid, xpvoid meta, xint code, xcwstr report);

typedef xbool (XCALL *Send64)(xuint64 uid, xpvoid meta, xpcch buf, xulong size);
typedef xvoid (XCALL *Verify64A)(xuint64 uid, xpvoid meta, xint code, xcstr report);
typedef xvoid (XCALL *Verify64W)(xuint64 uid, xpvoid meta, xint code, xcwstr report);

XNEUTRONAPI
xbool 
XCALL 
ICreateXigncodeServerW(IXigncodeServer **u
						, Send32 send_cb
						, Verify32W verify_cb);
...
xbool 
XCALL 
ICreateXigncodeServerA(IXigncodeServer **u
						, Send32 send_cb
						, Verify32A verify_cb);
...
xbool 
XCALL 
ICreateXigncodeServer2W(IXigncodeServer2 **u
							, Send64 send_cb
							, Verify64W verify_cb);
...
xbool 
XCALL 
ICreateXigncodeServer2A(IXigncodeServer2 **u
							, Send64 send_cb
							, Verify64A verify_cb);
...
xvoid
XCALL
PushPathW(xcwstr path);

XNEUTRONAPI
xvoid
XCALL
PushPathA(xcstr path);
...
inline CreateXigncodeServerW LoadHelperLibW(xcwstr pszPath)
{
	xwch szT[X_MAX_PATH];
	xwch Base[X_MAX_PATH];
	...
	CreateXigncodeServerW f = ICreateXigncodeServerW;
	if ( f ) 
		return f;
	else
		return NULL;
...
inline CreateXigncodeServerA LoadHelperLibA(xcstr pszPath)
{
	xch szT[X_MAX_PATH];
	xch Base[X_MAX_PATH];
	...
	CreateXigncodeServerA f = ICreateXigncodeServerA;
	if ( f ) 
		return f;
	else
		return NULL;
...
inline CreateXigncodeServer2W LoadHelperLib2W(xcwstr pszPath)
{
	xwch szT[X_MAX_PATH];
	xwch Base[X_MAX_PATH];
	...
	CreateXigncodeServer2W f = ICreateXigncodeServer2W;
	if ( f ) 
		return f;
	else
		return NULL;
...
inline CreateXigncodeServer2A LoadHelperLib2A(xcstr pszPath)
{
	char szT[X_MAX_PATH];
	char Base[X_MAX_PATH];
	...
	CreateXigncodeServer2A f = ICreateXigncodeServer2A;
	if ( f ) 
		return f;
	else
		return NULL;
...
typedef xbool (XCALL *ZSWAVE_SendCommandVaT)(xuint cid, va_list ap);
...
_ZSWAVE_FetchFunction(xpvoid *fn, xcstr name)
{
	...
	if(!module)
	{
		SetLastError(ZSWAVE_E_NOT_INITIALIZED);
		return X_FALSE;
	}

	*fn = GetProcAddress(module, name);
	if(!*fn)
	{
		SetLastError(ZSWAVE_E_NOT_INITIALIZED);
		return X_FALSE;
...
#define SAFE_CAST(t, obj) ((t)(xpvoid)(obj))
...
ZSWAVE_RsaFreeBuffer(xhandle H, xpuch Buffer)
{
	...
	if(!_ZSWAVE_FetchFunction(SAFE_CAST(xpvoid *, &pfn), "ZSWAVE_RsaFreeBuffer"))
		return X_FALSE;
...
ZSWAVE_RsaCreate()
{
	...
	if(!_ZSWAVE_FetchFunction(SAFE_CAST(xpvoid *, &pfn), "ZSWAVE_RsaCreate"))
		return X_FALSE;
...
ZSWAVE_RsaClose(xhandle H)
{
	...
	if(!_ZSWAVE_FetchFunction(SAFE_CAST(xpvoid *, &pfn), "ZSWAVE_RsaClose"))
		return X_FALSE;
							...
							, xsize *OutputSize)
{
	...
	if(!_ZSWAVE_FetchFunction(SAFE_CAST(xpvoid *, &pfn), "ZSWAVE_RsaPublicDecrypt"))
		return X_FALSE;
							...
							, xsize *OutputSize)
{
	...
	if(!_ZSWAVE_FetchFunction(SAFE_CAST(xpvoid *, &pfn), "ZSWAVE_RsaPublicEncrypt"))
		return X_FALSE;
							...
							, xsize *OutputSize)
{
	...
	if(!_ZSWAVE_FetchFunction(SAFE_CAST(xpvoid *, &pfn), "ZSWAVE_RsaPrivateDecrypt"))
		return X_FALSE;
							...
							, xsize *OutputSize)
{
	...
	if(!_ZSWAVE_FetchFunction(SAFE_CAST(xpvoid *, &pfn), "ZSWAVE_RsaPrivateEncrypt"))
		return X_FALSE;
...
ZSWAVE_RsaSetPublicKey(xhandle H, xpcuch Key, xsize KeySize)
{
	...
	if(!_ZSWAVE_FetchFunction(SAFE_CAST(xpvoid *, &pfn), "ZSWAVE_RsaSetPublicKey"))
		return X_FALSE;
...
ZSWAVE_RsaSetPrivateKey(xhandle H, xpcuch Key, xsize KeySize)
{
	...
	if(!_ZSWAVE_FetchFunction(SAFE_CAST(xpvoid *, &pfn), "ZSWAVE_RsaSetPrivateKey"))
		return X_FALSE;
							...
							, int Exponent)
{
	...
	if(!_ZSWAVE_FetchFunction(SAFE_CAST(xpvoid *, &pfn), "ZSWAVE_RsaGenerateKey"))
		return X_FALSE;
					...
					, CryptType Type)
{
	...
	if(!_ZSWAVE_FetchFunction(SAFE_CAST(xpvoid *, &pfn), "ZSWAVE_Decrypt"))
		return X_FALSE;
					...
					, CryptType Type)
{
	...
	if(!_ZSWAVE_FetchFunction(SAFE_CAST(xpvoid *, &pfn), "ZSWAVE_Encrypt"))
		return X_FALSE;
...
xbool
XCALL
ZSWAVE_SendCommand(xuint cid, ...)
{
	va_list ap;
	xbool r;

	static ZSWAVE_SendCommandVaT pfn = NULL;

	if(!_ZSWAVE_FetchFunction((xpvoid *) &pfn, "ZSWAVE_SendCommandVa"))
		return X_FALSE;
