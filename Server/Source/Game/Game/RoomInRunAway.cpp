...
I3_CLASS_INSTANCE( CRoomInRunAway, CRoomBase );

CRoomInRunAway::CRoomInRunAway()
{
...
BOOL CRoomInRunAway::OnCreate( TYPE_ROOM_INFO* pRoomInfo )
{ 
...
void CRoomInRunAway::OnDestroy()
{
}

BOOL CRoomInRunAway::SetRoomOption( UINT32 ui32StageID )
{
	m_pInfo->_i32StageType			= STAGE_MODE_RUN_AWAY;
...
BOOL CRoomInRunAway::OnUpdatePreBattle( UINT32 ui32RTState, UINT32 ui32RTServer )
{
	...
	{	// 2라운드 부터는 폭파모드와 동일하게 자동 공수교대 시간 체크
		...
		{
			CRoomBase::OnUpdatePreBattle( ui32RTState, ui32RTServer);
		}
	}
	return TRUE;
}

void CRoomInRunAway::SetMatchStart()
{
	i3mem::FillZero( m_ui8AccTouchDownCount, sizeof(UINT8)*SLOT_MAX_COUNT );
}

void CRoomInRunAway::SetRoundStart()
{

}

BOOL CRoomInRunAway::CheckRoundEnd( UINT32 ui32RTServer )
{
	//전적 올리기
...
BOOL CRoomInRunAway::CheckMatchEnd()
{
...
INT32 CRoomInRunAway::GetWinTeam()
{
...
// 추후 정규모드로 전환시 적용예정.
//INT16 CRoomInRunAway::GetMissionExp( INT32 i32SlotIdx )
//{
...
void CRoomInRunAway::GetBasicExpPoint( INT32 i32Win, TYPE_ROOM_GET_EXPPOINT_V1* pstGetExpPoint )
{
...
REAL32 CRoomInRunAway::GetSubPercentOfUnusualEnd( INT32 i32Win )
{
...
UINT16 CRoomInRunAway::GetMatchEndExp( INT32 i32Win, INT32 i32SlotIdx, UINT16 ui16AccExp, REAL32 r32AdjustPercent, REAL32 r32AsymPercent, BOOL bNormalEnd, UINT32 ui32RTBattle )
{
	return (UINT16)(ui16AccExp * r32AdjustPercent);
}

void CRoomInRunAway::AddStartBattlePacket( PACKET_BATTLE_STARTBATTLE_ACK* pPacket, CUserSession * pSession, UINT32 ui32RTBattle )
{
...
void CRoomInRunAway::GetRoomNowScore( ROOM_NOW_SCORE* pRoomNowScore, UINT32 ui32RTServer )
{
...
BOOL CRoomInRunAway::MissionTouchDown( CUserSession * pSession, INT32 i32SlotIdx, UINT32 ui32RTBattle )
{
...
void CRoomInRunAway::CheckDefaultWeapon(ITEM_INFO* aWeaponList, INT32& i32CheckSkipFlag,  INT32& i32WeaponUseStop, INT32 i32SlotIdx  ) 
{
		...
		{				
			...
			case STAGE_UID_HOLIDAY:
			{
				...
				case 301058:				// Chinese Cleaver. 판매용 중식칼 장착시 일반 중식칼로 바꿔줍니다. 기획의도.
					...
					break;
				default:
					if ((i32SlotIdx % 2) == i32AtkDefChange) aWeaponList[CHAR_EQUIPMENT_WEAPON_MELEE].m_TItemID = 301055;	//	레드팀이면 HairDryer 
...
UINT16 CRoomInRunAway::GetMatchAccExp(INT32 i32SlotIdx, UINT16 ui16AccExp)
{
...
UINT16 CRoomInRunAway::GetMatchAccPoint(INT32 i32SlotIdx, UINT16 ui16AccPoint)
{
	TYPE_ROOM_KILL_EXPPOINT	*pstKillExpPoint;
	pstKillExpPoint = GetKillExpPoint();
