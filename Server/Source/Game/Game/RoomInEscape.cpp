...
BOOL CRoomInEscape::OnCreate( TYPE_ROOM_INFO* pRoomInfo )
{
	...
	// 보너스 경험치/포인트 (공통 설정사항)
	m_stGetExpPointV1._i16WinExp		= 270;
	m_stGetExpPointV1._i16LoseExp		= 170;
	m_stGetExpPointV1._i16DrawExp		= 200;
	m_stGetExpPointV1._i16WinPoint		= 150;
	m_stGetExpPointV1._i16LosePoint		= 90;
	m_stGetExpPointV1._i16DrawPoint		= 110;
...
{
	m_pInfo->_i32StageType			= STAGE_MODE_ESCAPE;
...
void CRoomInEscape::UserKillBounsV1( INT32 i32SlotIdx, UINT32 ui32DeathType, UINT32 ui32DeathMyState, UINT32 ui32DeathEnemyState, UINT32 ui32RTBattle )
{
	// Assist를 했는지
	if( CHARA_DEATH_MY_STATE_ASSIST & ui32DeathMyState )
	{
		...
		{
			m_ai16MatAssist[ m_pInfo->_MiDinoInfo._uiTRexSlotIdx ]++;		// 티라노 기여도
		}
		m_ai16MatAssist[ i32SlotIdx ]++;									// 어시스트 킬
	}

	if(m_ui8DinoTeam == (i32SlotIdx % TEAM_COUNT))
	{
...
UINT16 CRoomInEscape::UserKillBounsV2( INT32 i32SlotIdx, UINT32 ui32DeathType, UINT32 ui32DeathMyState, UINT32 ui32DeathEnemyState, UINT32 ui32RTBattle )
...
BOOL CRoomInEscape::OnUpdatePreBattle( UINT32 ui32RTState, UINT32 ui32RTServer )
{
...
{
	i3mem::FillZero( m_ai16MatAssist, sizeof(INT16)*SLOT_MAX_COUNT );
}

void CRoomInEscape::SetRoundStart()
{
	i3mem::FillZero( m_ai16RouTouchDownCount, sizeof(INT16)*SLOT_MAX_COUNT );

	_InitUserState();
}

void CRoomInEscape::RoundStart( REAL32 r32RTBattle )
{
	TouchdownBlockTimeReset( r32RTBattle );
}

BOOL CRoomInEscape::CheckRoundEnd( UINT32 ui32RTServer )
{
	return TRUE;
}
...
void CRoomInEscape::GetBasicExpPoint( INT32 i32Win, TYPE_ROOM_GET_EXPPOINT_V1* pstGetExpPoint )
{
...
UINT16 CRoomInEscape::GetMatchAccExp( INT32 i32SlotIdx, UINT16 ui16AccExp )
{
	...
	// 1.0 과 1.5 의 소스가 틀립니다.. 풀경우 확인 필요. 20150105 김현우
	return ui16Exp;
}
...
UINT16 CRoomInEscape::GetMatchEndExp( INT32 i32Win, INT32 i32SlotIdx, UINT16 ui16AccExp, REAL32 rAdjustPercent, REAL32 rAsymPercent, BOOL bNormalEnd, UINT32 ui32RTBattle )
{
	...
	if( 0 == g_pContextMain->GetContentsVersion ( CONTENTS_CALCULATE_POINT ) )
	{
		...
		if( (m_pInfo->_InfoAdd._NowRoundCount) < m_ui32RoundCount || m_pInfo->_InfoAdd._NowIngTime < g_TeamTimeTable[ui8EndTimeIdx] )
		{
			...
			UINT32 ui32PlayTime = ui32RTBattle - m_pInfo->_pSession[i32SlotIdx]->m_ui32RTBattleStart;
			if( ui32MaxPlayTime < ui32PlayTime )		ui32PlayTime = ui32MaxPlayTime;

			UINT16 ui16TimeSubRate;

			ui16TimeSubRate = (UINT16)((ui32PlayTime*100)/ui32MaxPlayTime);
	...
	{
		INT8 i8ScoreDiff = (INT8)(m_pInfo->_aui16TotalRoundCount[i32SlotIdx%TEAM_COUNT] - m_pInfo->_aui16TotalRoundCount[(i32SlotIdx+1)%TEAM_COUNT]);
		...
		ui16Exp = CRoomBase::GetMatchEndExp( i32Win, i32SlotIdx, ui16Exp, rAdjustPercent, 1.0f, bNormalEnd, ui32RTBattle );
	}

	return ui16Exp;
}

UINT16 CRoomInEscape::GetMatchEndPoint( INT32 i32Win, INT32 i32SlotIdx, UINT16 ui16AccPoint, REAL32 r32AdjustPercent, REAL32 r32AsymPercent, BOOL bNormalEnd, UINT32 ui32RTBattle )
{
	if( ExistRecord(i32SlotIdx ) == false ) return 0;
	...
	if( ( m_pInfo->_InfoAdd._NowRoundCount ) < m_ui32RoundCount || m_pInfo->_InfoAdd._NowIngTime < g_TeamTimeTable[ ui8EndTimeIdx ] )
	{
		...
		UINT16 ui16TimeSubRate = (UINT16)( ( ui32PlayTime * 100 ) / ui32MaxPlayTime );
		if( 0 < ui16TimeSubRate )
		{
			ui16TimeSubRate = ui16TimeSubRate * ui16TimeSubRate;
		}
...
void CRoomInEscape::AddStartBattlePacket( PACKET_BATTLE_STARTBATTLE_ACK* pPacket, CUserSession * pSession, UINT32 ui32RTBattle )
{
	pPacket->m_ui16RoundStartUser	= m_pInfo->_ui16RoundStartUser;
	pPacket->m_ui8NowRoundCount		= m_pInfo->_InfoAdd._NowRoundCount;
	pPacket->m_ui8BlockBonusRemain	= (UINT8)(m_r32RTBlockBonus - ui32RTBattle);
	pPacket->m_ui16TotalRoundCount.SetValue( m_pInfo->_aui16TotalRoundCount, 0, TEAM_COUNT );
	i3mem::Copy( &pPacket->m_MiDinoInfo,			&m_pInfo->_MiDinoInfo,				sizeof(DINO_ROOM_INFO) );
	
	//I3TRACE("***** ADDSTARTBATTLEPACKET ***\n");
...
void CRoomInEscape::_InitUserState()
{
	...
	for( INT32 i = 0 ; i < SLOT_MAX_COUNT ; i++ )
	{
		...
		if( TRUE == m_pInfo->_aSlotInfo[i]._bIsGmObserverMode ) continue;
	...
	INT32 i32TargetUserCount	= m_ui8DinoTeam ? i32BlueUserCount : i32RedUserCount;
	UINT32 ui32TRex				= 255;
	UINT32 ui32Idx				= 0;

		if( 2 > i32TargetUserCount)
		{
			m_pInfo->_MiDinoInfo._uiTRexSlotIdx = 255;
		}
		else
		{
			ui32TRex = (rand() % (i32TargetUserCount));
		}
		...
		for( INT32 i = m_ui8DinoTeam; i < SLOT_MAX_COUNT ; i+=2 )
		{
			...
			if( ui32TRex == ui32Idx )	
			{
				m_pInfo->_MiDinoInfo._uiTRexSlotIdx				= (UINT8)i;
				m_pInfo->_aui32DInoCharInfo[i] = MAKE_ITEM_ID( ITEM_TYPE_DINO,	CHARACTER_TEAM_NONE,	CHARA_RES_ID_DINO_TREX);
			}
			else
			{
...
bool CRoomInEscape::CheckTrex( void )
{
	...
	{
		//새로 들어온 유저처리 
		if(ai8PlayUserCount[m_ui8DinoTeam] > 1)m_bNewTrex = true;
	}
	else 
	{
		//나간 유저 처리 
		if( SLOT_STATE_BATTLE != m_pInfo->_aSlotInfo[pDinoInfo->_uiTRexSlotIdx]._State )
		{
...
{
	m_aui8A_TouchDownChainCount[i32Idx] = 0;
	...
	{
		//Trex로 선택 되었습니다. 
		pDinoInfo->_uiTRexSlotIdx = (UINT8)i32Idx; 
		m_bNewTrex = false; 
		...
		m_pInfo->_aui32DInoCharInfo[i32Idx] = MAKE_ITEM_ID(ITEM_TYPE_DINO,	CHARACTER_TEAM_NONE,	CHARA_RES_ID_DINO_TREX);
	}
	else 
	{
...
void CRoomInEscape::periodicUpdateRoomState( REAL32 r32RTServer )
{
	if( m_pInfo->_InfoBasic._State == ROOM_STATE_BATTLE && m_pInfo->_i32MiStateType == MISSION_STATE_BATTLE )
	{
		...
		// 방어 점수
		if( m_r32RTBlockBonus <= r32RTServer)
		{
			m_pInfo->_aui16TotalRoundCount[m_ui8DinoTeam] += m_ui8ScoreBlock;
			...
			TouchdownBlockTimeReset( r32RTServer );
...
BOOL CRoomInEscape::MissionTouchDown( CUserSession * pSession, INT32 i32SlotIdx, UINT32 ui32RTBattle )
{
	...
	// 터치다운 시간
	TouchdownBlockTimeReset( (REAL32)ui32RTBattle );

	// 개인 정보만 갱신하고 전체 카운트는 메인스레드에서 변경
	m_pInfo->_aui8AccMissionCount[ i32SlotIdx ]++;
	m_ai16RouTouchDownCount[ i32SlotIdx ]++;
	m_pInfo->_aui8A_Mission[i32SlotIdx]++;
	m_aui8A_TouchDownChainCount[i32SlotIdx]++;

	if( m_aui8A_TouchDownChainCount[i32SlotIdx] > 4 )	m_aui8A_TouchDownChainCount[i32SlotIdx] = 4;		// 최대 4회 (Game Maker)

	// Score 갱신
	switch( m_aui8A_TouchDownChainCount[i32SlotIdx] )
	{
	case 4:
	case 3:
	...
	// ZLog 에 탈출을 남김 - 서동권
	g_pModuleContextNC->LogSendMessage( pSession->m_WorkThreadIdx, LOG_MESSAGE_U_R_ESCAPE, pSession, &m_ai16RouTouchDownCount[i32SlotIdx], &m_aui8A_TouchDownChainCount[i32SlotIdx], &m_pInfo->m_ui64BattleUID );
...
BOOL CRoomInEscape::CheckVoteTime( UINT32 ui32RTServer )
{
	if ( (UINT32)(m_pInfo->_InfoAdd._NowRoundCount) + 1 == m_ui32RoundCount )
	{
		UINT8 ui8EndTimeIdx = (m_pInfo->_InfoAdd._SubType & 0xF0) >> 4; 
		if( (m_pInfo->_ui32RTRoundStart + g_TeamTimeTable[ui8EndTimeIdx]) <= ui32RTServer + VOTE_FINISH_TIME )
		{
			return FALSE;
		}
	}
...
void CRoomInEscape::GetRoomNowScore( ROOM_NOW_SCORE* pRoomNowScore, UINT32 ui32RTServer )
{
...
void CRoomInEscape::CommandIncreaseKillCount( INT32 i32SlotIdx, INT32 i32KillCount )
{
	if(m_ui8DinoTeam == (i32SlotIdx % TEAM_COUNT))
	{
