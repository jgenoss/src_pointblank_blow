...
BOOL CModuleControl::OnCreate()
{
	...
	if( FALSE == m_pControlSocket->OnCreate( g_pConfig->m_ui32ControlIP, g_pConfig->m_ui16ControlServerPort, m_i32LogIdx))	return FALSE;
...
INT32 CModuleControl::_ControlConnect()
{
	...
	if( m_pControlSocket->OnCreate( g_pConfig->m_ui32ControlIP, g_pConfig->m_ui16ControlServerPort, m_i32LogIdx ) )
	{
		m_ui32SystemInfoSendTime	= i3ThreadTimer::GetServerTime();
...
{
	m_i32LogIdx = g_pLog->AddLogThread(L"CONT", 0, m_ThreadID);
		...
		if( m_pControlSocket->IsActive() )
		{
			...
			if( (m_ui32SystemInfoSendTime + m_pControlSocket->m_ui32ControlSendTime) < i3ThreadTimer::GetServerTime() )
			{
				...
				if ( SERVER_STATE_START == g_eServerState )
				{
					...
					PacketData._i32Arg1		= g_pConfig->GetClanCount();
				}
				else
				{
...
BOOL CModuleControl::IsActive(void)
{
	if (!m_pControlSocket) return FALSE;
	return m_pControlSocket->IsActive();
}