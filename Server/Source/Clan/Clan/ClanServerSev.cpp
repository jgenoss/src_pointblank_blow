// Clanss.cpp : Defines the entry point for the console application.
...
{	
	...
	i3String::Copy(g_strServiceName, "_PBClanServerService", 512);
	...
	{
		printf("Clan Server Help!\n\n"); 
		printf("Clanss.exe -i : Service Install \n"); 
		printf("Clanss.exe -u : Service Uninstall. \n"); 
		printf("Clanss.exe -s : Service Start. \n");
		printf("Clanss.exe -e : Service End. \n");
		printf("Clanss.exe : It does not use. \n");
	}
...
{	
	...
	if(i32Rv != EVENT_ERROR_SUCCESS )
	{
		...
		case SERVER_ERROR_LOG				: 	if(g_pLog != NULL)	g_pLog->WriteLog( L"NC LOG-D Error!");		break;
		case SERVER_ERROR_CONTROL			:	if(g_pLog != NULL)	g_pLog->WriteLog( L"Control Info Error!");	break;
		case SERVER_ERROR_CONTROLSERVER		:	if(g_pLog != NULL)	g_pLog->WriteLog( L"ControlServer Error!");	break;
		default								:	if(g_pLog != NULL)	g_pLog->WriteLog( L"UnKnown Error!");		break;
		//g_InitServer에서 g_StartServer로 작업위치가 변경되었습니다.
		//case SERVER_ERROR_DATABASE		: 	if(g_pLogFile != NULL)	g_pLogFile->Write_Log_Main("DataBase Error!");			break;
		//case SERVER_ERROR_CONTEXT			: 	if(g_pLogFile != NULL)	g_pLogFile->Write_Log_Main("Context Error!");			break;
		//case SERVER_ERROR_PATH			: 	if(g_pLogFile != NULL)	g_pLogFile->Write_Log_Main("CurrentPath Error!");		break;
		//case SERVER_ERROR_CLAN			: 	if(g_pLogFile != NULL)	g_pLogFile->Write_Log_Main("ClanContext Error!");		break;
		//case SERVER_ERROR_GIP				:	if(g_pLogFile != NULL)	g_pLogFile->Write_Log_Main("GIP Error!");				break;
		//case SERVER_ERROR_GATEWAY			:	if(g_pLogFile != NULL)	g_pLogFile->Write_Log_Main("GateWay Error!");			break;
		//case SERVER_ERROR_TASKPROCESSOR	:	if(g_pLogFile != NULL)	g_pLogFile->Write_Log_Main("TaskProcessor Error!");		break;
		}
		Sleep(500);
...
void Install(char* pstrPath, char* pstrName)
{
	...
	else
	{
		...
		else
		{
			...
			i3String::Copy( strDes, "Point Blank Clan Server", MAX_STRING_COUNT );
