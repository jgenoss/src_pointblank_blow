...
////////////////////////////////////////////////////////////////////////
	...
	public:
		...
		size_type			bucket(const key_type& key) const { return hash_fun()(key) % num_bucket(); }
		...
		{
			Bucket* pBucket = get_bucket(key);
			Node*   it = find_node(pBucket, k);
			if (it)
			{
				iterator first(pBucket, it);
		...
		i3::pair<const_iterator, const_iterator> equal_range(const key_type& key) const
		{
			...
			if (it)
			{
				const_iterator first(pBucket, it);
	...
	protected:
		...
		size_type			hash_function(const key_type& v) const { return hash_fun()(v); }
		bool				key_function(const key_type& l, const key_type& r) const { return equal_fun()(l,r); }
		size_type			bucket_from_hash_value(size_type hash_value) const {	return hash_value %  num_bucket(); }
		Bucket*				get_bucket(const key_type& key) const { return buckets() + static_cast<difference_type>(bucket(key)); }
		Bucket*				get_bucket_from_hash_value(size_type hash_value) const 
			{ return buckets() + static_cast<difference_type>(bucket_from_hash_value(hash_value) ); }
		Node*				find_node(const key_type& k) const {	return find_node( get_bucket(k), k ); }
		Node*				find_node(Bucket* pBucket, const key_type& k) const
		{
...
////		
	...
	private:	
		...
		Node**				find_for_erase(Bucket* pBucket, key_type const& k) const
		{
...
//			other.allocate_new_buckets(pNewBuckets, pNewBucket_Begin, numBucket_New);
		...
		{
			if ( pBucket1 == bucket_begin() && pBucket1->pHead == NULL )
...
//		size_type					m_numSize;
	...
	public:
		...
		i3::pair<iterator, bool> insert(const value_type& val)
		{
			...
			Bucket*		pBucket = get_bucket_from_hash_value(hash_value);
			...
			if (  reserve_for_insert( size() + 1 ) )
				pBucket = get_bucket_from_hash_value(hash_value);
		...
		void	insert(InputIterator i, InputIterator j)
		{
			...
			for (; i != j; ++i) 
			{
				...
				size_type hash_value = hash_function(key(*i));
				Bucket* bucket = get_bucket_from_hash_value(hash_value);
				...
				if (!pos )		// 못찾았다면...이미 있는 경우 인서트 스킵..
				{
					...
					if( size() + 1 >= max_load() ) 
					{
						if ( reserve_for_insert( size() + static_cast<size_type> (i3::distance(i, j) ) ) )
							bucket = get_bucket_from_hash_value(hash_value);
					}
