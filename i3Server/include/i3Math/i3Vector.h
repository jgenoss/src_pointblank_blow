...
namespace i3Vector
{
	...
	inline
	bool	isAlike( VEC2D * pV1, VEC2D * pV2, REAL32 eps = I3_EPS)
	{
	...
	inline
	void	Mul( VEC2D * pOut, const VEC2D * p1, const VEC2D * p2)
	{
		pOut->x = p1->x * p2->x;
		pOut->y = p1->y * p2->y;
	}
	...
	inline
	bool	isValid( VEC3D * pSrc)
	{
		return (i3Math::isValid( GetX( pSrc)) == true) &&
			(i3Math::isValid( GetY( pSrc)) == true) &&
			(i3Math::isValid( GetZ( pSrc)) == true);
	}
