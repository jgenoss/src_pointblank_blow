...
BOOL i3NetworkPacket::WriteData( const void* buffer, UINT16 size)
{
	if( 0 == size )
	{
...
void i3NetworkPacket::WriteData( UINT16 ui16Pos,  const void* buffer, UINT16 size)
{
	if( 0 == size )	return;

	if( &m_pPacketBuffer[PACKETHEADERSIZE+ui16Pos] + size > m_pEndOfDataField )
	{
		I3TRACE("[i3NetworkPacket::WriteData] Write Failed \n"); 
		return;
	}
	...
	if( m_Size < ui16NewSize ) 
	{	// 데이터 사이즈 변경이 일어났다면 사이즈 증가시켜줍니다.		
		m_pWritePosition = &m_pPacketBuffer[PACKETHEADERSIZE+ui16Pos];
		i3mem::Copy( m_pWritePosition, buffer, size );
	...
	{
		i3mem::Copy( &m_pPacketBuffer[PACKETHEADERSIZE+ui16Pos], buffer, size );
	}
